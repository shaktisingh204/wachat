...
const envSchema = z.object({
  ...
  // SabNode main app origin (login fallback) + shared session JWT secret
  SABNODE_APP_URL: z.string().url().optional(),
...
// Empty-string env vars (common in shared .env files like SabNode's) must
// behave like unset ones, otherwise enum/url validations reject them.
const emptyToUndefined = (value: string | undefined) =>
  value === "" ? undefined : value;

const rawEnvWithEmpties = {
  ...
  SABNODE_APP_URL: process.env.SABNODE_APP_URL,
...
const rawEnv = Object.fromEntries(
  Object.entries(rawEnvWithEmpties).map(([key, value]) => [
    key,
