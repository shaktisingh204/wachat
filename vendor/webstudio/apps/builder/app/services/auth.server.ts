...
export const findAuthenticatedUser = async (request: Request) => {
  ...
  if (user == null) {
    // SabNode unified auth: createContext resolves the SabNode session
    // cookie into a user authorization (auto-provisioning the account).
    try {
      const context = await createContext(request);
      if (context.authorization.type === "user") {
        return await getUserById(context, context.authorization.userId);
      }
    } catch {
      // canvas requests and broken sessions fall through to null
    }
    return null;
  }
  const context = await createContext(request);
