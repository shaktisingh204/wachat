...
// Best-effort: inside the SabNode process the inherited DEBUG state can
// produce patterns the debug package cannot re-parse — never fatal.
try {
  createDebugRaw.enable(
    `${createDebugRaw.disable()},-OAuth2Strategy,-*OAuth2Strategy*`
  );
} catch {
  // keep whatever debug state the host app configured
}
