...
import { verifySabNodeSession } from "~/services/sabnode-auth.server";
import { createOrLoginWithSabNode } from "./db/user.server";
...
/**
 * Native builder session, falling back to SabNode unified auth (the SabNode
 * session cookie present on this origin) with user auto-provisioning.
 * Dashboard-origin requests only — builder (p-*) origins use the ws OAuth flow.
 */
export const getAuthenticatedSessionData = async (request: Request) => {
  const sessionData = await authenticator.isAuthenticated(request);
  if (sessionData) {
    return sessionData;
  }
  const identity = verifySabNodeSession(request);
  if (identity === undefined) {
    return null;
  }
  try {
    const user = await createOrLoginWithSabNode(
      { postgrest: createPostgrestContext() } as AppContext,
      identity.email,
      identity.name
    );
    return { userId: user.id, createdAt: Date.now() };
  } catch (error) {
    console.error("SabNode unified auth failed", error);
    return null;
  }
};
...
const createAuthorizationContext = async (
  ...
  const { authToken, isServiceCall, sessionData: nativeSessionData } =
  ...
  // SabNode unified auth: no native builder session, but the SabNode
  // session cookie is present on this origin (the builder runs inside the
  // SabNode app). Verify it and auto-provision the builder user. Builder
  // (p-*) origins never see that cookie and keep the ws OAuth flow.
  let sessionData = nativeSessionData;
  if (sessionData == null && isBuilder(request) === false) {
    const identity = verifySabNodeSession(request);
    if (identity) {
      try {
        const user = await createOrLoginWithSabNode(
          { postgrest } as AppContext,
          identity.email,
          identity.name
        );
        sessionData = { userId: user.id, createdAt: Date.now() };
      } catch (error) {
        console.error("SabNode unified auth failed", error);
      }
    }
