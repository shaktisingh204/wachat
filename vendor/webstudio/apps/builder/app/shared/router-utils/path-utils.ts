...
export const builderUrl = (props: {
  ...
  // The app is mounted at /sites inside SabNode; builder URLs are absolute
  // (p-<projectId> subdomain) so the prefix must be embedded explicitly.
...
export const dashboardUrl = (props: { origin: string }) => {
  ...
  return `${authServerOrigin}/sites/dashboard`;
};
...
export const cloneProjectUrl = (props: {
  ...
  return `${authServerOrigin}/sites/dashboard?${searchParams.toString()}`;
};
...
export const restLogoutPath = () => "/sites/dashboard-logout";
...
export const planSubscriptionPath = (subscriptionId?: string) => {
  ...
  return `/sites/builder-payments/billing-portal/sessions?${urlSearchParams.toString()}`;
};
...
export const restAssetsUploadPath = ({
  ...
  if (urlSearchParams.size > 0) {
    return `/sites/rest/assets/${name}?${urlSearchParams.toString()}`;
  }

  return `/sites/rest/assets/${name}`;
};
...
export const restResourcesLoader = () => `/sites/rest/resources-loader`;
