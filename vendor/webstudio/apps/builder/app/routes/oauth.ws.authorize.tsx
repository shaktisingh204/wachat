...
import { getAuthenticatedSessionData } from "~/shared/context.server";
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  try {
    ...
    if (
      ...
      new URL(redirect_uri).pathname !== "/sites/auth/ws/callback" ||
    ...
    const sessionData = await getAuthenticatedSessionData(request);
    ...
    if (sessionData) {
      ...
      // Compare origins only — builderUrl carries the /sites mount path.
      if (
        false ===
        compareUrls(
