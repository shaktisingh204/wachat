...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    // SabNode: the builder is mounted inside the SabNode Next.js app at /sites.
    // Next normalizes away trailing slashes, so the built basename must be
    // "/sites" (the vite dev server instead requires it to start with `base`).
    base: "/sites/",
    plugins: [
      remix({
        basename: command === "build" ? "/sites" : "/sites/",
        future: {
