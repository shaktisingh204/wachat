...
- **§13.4 executor — Rust `crm-lookup` crate: DONE (2026-05-07).** Mongo-backed executor + Axum handler reusing `sabnode-db::MongoHandle` and `sabnode-common::ApiError`. The Next.js TS server action can now be replaced by a thin proxy (or deleted) once auth middleware is wired upstream.
  ...
  - **NEXT →** still parallelizable:
    - More business-logic crates: `crm-bills`, `crm-rfqs`, `crm-vendor-bids`, `crm-grns`, `crm-employees`, `crm-attendance`, `crm-leaves`, `crm-payroll-runs`, `crm-fixed-assets`, `crm-bookings`. Same J8/J9 template — could ship 10 more in a single batch.
    - Mount the 12 existing routers under `sabnode-api`'s router (e.g. `.nest("/v1/crm/leads", crm_leads::router::<AppState>())`) so they serve real HTTP traffic.
    - Wire frontend auth so the picker uses a JWT against the Rust endpoints.
    - Build remaining detail/edit routes (DC, Payout, Subscription, etc.).
