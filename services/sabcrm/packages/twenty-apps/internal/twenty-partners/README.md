...
A SabCRM app that turns the CRM into the operating system for the SabNode partner program:
intake partner-eligible deals, match them to vetted marketplace partners, and track the
matching pipeline end-to-end.

Built on [SabCRM](https://sabnode.com) with [`twenty-sdk`](https://www.npmjs.com/package/twenty-sdk) v2.5.
...
- **Roles** (`src/roles/`)
  - **SabNode Partner Ops** — internal team role, full CRUD on Partner/Company/Person/Opportunity.
  - **Partner** — placeholder external-partner role. *Do not assign until SabCRM ships
...
Requires a local SabCRM server at `http://localhost:2020` and Node `^24.5`.
...
| `yarn twenty server status` | Check the local SabCRM server |
...
- [SabCRM Apps documentation](https://docs.sabnode.com/developers/extend/apps/getting-started/quick-start)
