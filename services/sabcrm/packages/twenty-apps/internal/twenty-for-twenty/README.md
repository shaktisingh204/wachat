This is a [SabCRM](https://sabnode.com) application bootstrapped with [`create-twenty-app`](https://www.npmjs.com/package/create-twenty-app).

## Overview

**SabCRM for SabCRM** is the official internal SabCRM app. It is organized into modules, each integrating a third-party service with SabCRM.

### Resend module (`src/modules/resend/`)

Two-way sync between SabCRM and the [Resend](https://resend.com) email platform. The module syncs contacts, segments, templates, broadcasts, and emails.

**Inbound (Resend -> SabCRM):**
...
**Outbound (SabCRM -> Resend):**

- Database event triggers push contact and segment changes back to Resend when records are created, updated, or deleted in SabCRM
...
This registers the app with your local SabCRM instance at `http://localhost:3000/settings/applications`.

### 2. Configure app variables

In SabCRM, go to **Settings > Applications > SabCRM for SabCRM** and set:
...
2. Click **Add webhook**
3. Set the **Endpoint URL** to your SabCRM server's public URL + `/s/webhook/resend` (e.g. `https://your-domain.com/s/webhook/resend`)
4. Set **Events types** to **All Events**
5. Click **Add**
6. Copy the **signing secret** Resend displays and paste it into the `RESEND_WEBHOOK_SECRET` app variable in SabCRM

The webhook handles:

- **Contact events** (`contact.created`, `contact.updated`, `contact.deleted`) -- upserts/deletes Resend contact records in SabCRM
...
Start the webhook listener with forwarding to your local SabCRM server:
...
3. Forward incoming events (with Svix signature headers) to your local SabCRM server
...
|---|---|---|
| Cron (every 5 min) | Polls Resend API, upserts into SabCRM | Contacts, segments, templates, broadcasts, emails |
| Webhook (real-time) | Receives Resend events via HTTP | Contacts, emails |

### Outbound sync

| SabCRM action | Resend API call |
|---|---|
| Create contact | `contacts.create()` -- writes `resendId` back to SabCRM |
| Update contact (name, email, unsubscribed) | `contacts.update()` |
| Delete contact | `contacts.remove()` |
| Create segment | `segments.create()` -- writes `resendId` back to SabCRM |
...
- [SabCRM Apps documentation](https://docs.sabnode.com/developers/extend/apps/getting-started)
