# Fireflies for SabCRM

Sync [Fireflies](https://fireflies.ai) call transcripts and AI summaries onto
the matching `CalendarEvent` in your SabCRM — searchable, in context, and
...
3. For each event, this app fetches the relevant data via the Fireflies
   GraphQL API.
4. It finds the matching `CalendarEvent` in SabCRM and writes the content
...
1. **Provider-native event ID** — Fireflies' `calendar_id` / `cal_id` is
   ...
   events synced into SabCRM from Google Calendar (including individual
...
Both identifiers are populated by SabCRM's calendar drivers on every synced
...
## What gets added to your SabCRM workspace
...
1. Open **Settings → Applications** in your SabCRM workspace.
...
> webhook, your SabCRM admin needs to follow the
...
- **Per-user Fireflies accounts.** All transcripts come through one
  ...
  connections require extending SabCRM's connection provider system and are
  planned once we have evidence that workspace-shared is too coarse.
- **Editing transcripts or summaries in SabCRM.** The fields are writable
...
| Webhook returns `Invalid webhook signature` | `FIREFLIES_WEBHOOK_SECRET` mismatch between Fireflies and SabCRM | Re-copy the signing secret from the Fireflies webhook configuration and paste it into the SabCRM app settings |
| Webhook returns `skipped: No CalendarEvent matched the transcript by external ID or iCalUid` | The meeting was never on a synced calendar in SabCRM, or the workspace has no Google/Outlook/CalDAV calendar connection set up | Connect the relevant calendar provider in **Settings → Accounts** so the calendar event lands in SabCRM with `eventExternalId` and `iCalUid` populated. Manually-created CalendarEvents are intentionally not matched in v1 |
...
| **List / Search** tools return `count: 0` for a contact you've definitely talked to | Email mismatch — Fireflies stores the address as the participant joined the meeting with, which may differ from the contact's primary address in SabCRM (aliases, plus-addressing, work vs. personal) | Try the contact's other known email addresses; cross-check the `participants` list on a known matching call |

---

## Self-hosting setup (admin-only)

This section is for SabCRM server admins. If you're on SabCRM Cloud, skip
...
2. Set the **Webhook URL** to your SabCRM deployment's webhook endpoint:
   `https://<your-twenty-domain>/webhook/fireflies`. SabCRM resolves the
...
3. Set a **Signing Secret** (a long random string — generate one with
   `openssl rand -hex 32`). Save it; you'll paste it into SabCRM next.
...
### 3. Wire the credentials into SabCRM

1. In SabCRM: **Settings → Applications → Fireflies → Settings tab**.
...
`twenty dev` is recommended for iteration — it publishes to your local SabCRM
...
```bash
export FIREFLIES_WEBHOOK_SECRET='<the secret you set in SabCRM app settings>'
...
computed above. SabCRM resolves the workspace from the `Host` header, so
...
in your local SabCRM workspace. The easiest local seed is to manually
