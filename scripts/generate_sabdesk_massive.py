...
PAGES = [
    ("analytics", ["overview", "agent-performance", "sla-compliance", "customer-satisfaction", "ticket-volume", "resolution-times", "channel-metrics", "ai-deflection", "custom-reports", "live-dashboards"]),
    ("integrations", ["app-directory", "active-apps", "webhooks", "api-logs", "custom-apps", "salesforce", "hubspot", "jira", "slack", "zapier"]),
    ("quality-assurance", ["scorecards", "evaluations", "disputes", "calibration", "qa-settings"]),
    ("ai-copilot", ["training-data", "suggested-replies", "ticket-summarization", "sentiment-analysis", "ai-settings"]),
    ("workforce", ["scheduling", "forecast", "time-off", "shift-swaps", "attendance"])
]

BASE_DIR = "/Users/harshkhandelwal/Downloads/sabnode/src/app/dashboard/sabdesk"
        ...
        <div className="flex flex-col w-full h-full min-h-screen bg-neutral-950 text-neutral-200">
            ...
            <header className="flex items-center justify-between px-8 py-6 border-b border-white/10 bg-neutral-900/50 backdrop-blur-md">
                <div>
                    ...
                    <p className="text-neutral-400 mt-1">Manage and optimize your {Title.lower()} workflows and metrics.</p>
                </div>
                            ...
                            <tbody>
                                {{Array.from({{ length: 15 }}).map((_, i) => (
                                    <tr key={{i}} className="border-b border-white/5 hover:bg-neutral-800/50 transition-colors cursor-pointer">
                                        <td className="p-4 text-sm text-neutral-300 font-mono">#{Title[:3].upper()}-{{1000 + i}}</td>
                                        <td className="p-4 text-sm text-white font-medium">{Title} Item {{i + 1}}</td>
                                        <td className="p-4">
                        ...
                        <div className="space-y-4">
                            {{[1,2,3,4,5].map(i => (
                                <div key={{i}} className="p-4 bg-neutral-800/50 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                                    ...
                                    <p className="text-sm text-neutral-400">The system has detected an anomaly in the standard workflow pattern for {{Title.lower()}}.</p>
                                </div>
                            ))}}
                        </div>
                    </div>
...
def generate():
    for section, pages in PAGES:
        for page in pages:
            # Create directory
            dir_path = os.path.join(BASE_DIR, section, page)
            os.makedirs(dir_path, exist_ok=True)
            
            # Create Component Name
            parts = (section + "-" + page).split("-")
            ...
            content = TEMPLATE.replace("{ComponentName}", component_name).replace("{Title}", title)
