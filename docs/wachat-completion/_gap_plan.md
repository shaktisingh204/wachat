I have fully validated the wiring chain (workspace member → api Cargo dep → router.rs `let` + `.nest` → `FromRef` impl + AppState field → rust-client module + index.ts barrel → api-manifest module). Now I'll produce the master plan.
