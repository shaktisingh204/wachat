...
| 4 | `docs/adr/benches/sabflow-ws/` | Bench harness directory: Node WS gateway vs Rust (tokio + tungstenite) at N=2 / 10 / 50 / 200 clients/doc; emits a verdict consumed by ADRs 3 and 5. |
...
| 9 | `docs/adr/benches/sabflow-executor/` + `docs/adr/sabflow-executor-rust-bench.md` | Bench harness + write-up for the hot expression/transform path: n8n queue-mode (Bull + Redis + Node workers) baseline vs a Rust executor; verdict consumed by Track B Phase 1. |
...
- **CRDT library:** Yjs.
- **WebSocket gateway:** Node-based, modeled on n8n's push module. Service-location decision (Next.js route handler vs standalone `services/sabflow-ws`) defers to Phase 3 once the bench verdict is in.
