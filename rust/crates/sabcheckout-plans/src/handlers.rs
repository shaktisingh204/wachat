//! HTTP handlers for SabCheckout Plans.
...
use crate::types::SabcheckoutPlan;

const COLL: &str = "sabcheckout_plans";
...
fn plan_from_create(input: CreatePlanInput, user_id: ObjectId) -> Result<SabcheckoutPlan> {
...
#[instrument(skip_all, fields(user_id = %user.user_id, plan_id = %plan_id))]
pub async fn get_plan(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Path(plan_id): Path<String>,
) -> Result<Json<SabcheckoutPlan>> {
    let user_id = user_oid(&user)?;
    ...
    let row = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcheckout_plans.find_one"))
        })?
        .ok_or_else(|| ApiError::NotFound("sabcheckout_plan".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn update_plan(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcheckout_plans.update"))
        })?;
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("sabcheckout_plan".to_owned()));
    }
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcheckout_plans.refetch"))
        })?
        .ok_or_else(|| ApiError::NotFound("sabcheckout_plan".to_owned()))?;
    Ok(Json(after))
}
...
pub async fn delete_plan(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcheckout_plans.archive"))
        })?;
...
mod tests {
    ...
    fn create_rejects_bad_interval() {
        ...
        assert!(plan_from_create(input, ObjectId::new()).is_err());
    }
    ...
    fn create_defaults_count_and_status() {
        ...
        let p = plan_from_create(input, ObjectId::new()).unwrap();
