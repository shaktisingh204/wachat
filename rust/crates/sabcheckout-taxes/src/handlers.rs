//! HTTP handlers for SabCheckout SabcheckoutTaxs.
...
use crate::dto::{
    CreateSabcheckoutTaxInput, CreateSabcheckoutTaxResponse, DeleteSabcheckoutTaxResponse,
    ListQuery, UpdateSabcheckoutTaxInput,
};
use crate::types::SabcheckoutTax;

const COLL: &str = "sabcheckout_taxes";
...
fn entity_from_create(
    input: CreateSabcheckoutTaxInput,
    user_id: ObjectId,
) -> Result<SabcheckoutTax> {
    Ok(SabcheckoutTax::from_input(input, user_id))
}

fn build_update_doc(patch: UpdateSabcheckoutTaxInput) -> Document {
...
pub async fn list_taxes(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<SabcheckoutTax>(COLL);
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context(format!("{COLL}.find")))
        })?;
    let mut rows: Vec<SabcheckoutTax> = cursor.try_collect().await.map_err(|e| {
...
pub async fn create_tax(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateSabcheckoutTaxInput>,
) -> Result<Json<CreateSabcheckoutTaxResponse>> {
    let user_id = user_oid(&user)?;
    ...
    Ok(Json(CreateSabcheckoutTaxResponse {
        id: new_id.to_hex(),
        entity,
    }))
}
...
pub async fn update_tax(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
    Json(patch): Json<UpdateSabcheckoutTaxInput>,
) -> Result<Json<SabcheckoutTax>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabcheckoutTax>(COLL);
...
pub async fn delete_tax(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<DeleteSabcheckoutTaxResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabcheckoutTax>(COLL);
    let result = coll
    ...
    Ok(Json(DeleteSabcheckoutTaxResponse {
