...
fn entity_from_create(input: CreateThemeInput, user_id: ObjectId) -> Result<SabshopTheme> {
    ...
    Ok(SabshopTheme {
        ...
        config_json: input.config_json.map(value_to_bson).unwrap_or(Bson::Document(Document::new())),
        system: false,
...
fn build_update_doc(patch: UpdateThemeInput) -> Document {
    ...
    if let Some(v) = patch.config_json { set.insert("configJson", value_to_bson(v)); }
    doc! { "$set": set }
}

fn doc_for_audit(e: &SabshopTheme) -> Document { bson::to_document(e).unwrap_or_default() }
...
pub async fn list_themes(user: AuthUser, State(mongo): State<MongoHandle>, Query(q): Query<ListQuery>) -> Result<Json<ListResponse>> {
    ...
    let cursor = coll.find(filter).with_options(opts).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabshop_themes.find")))?;
    let mut rows: Vec<SabshopTheme> = cursor.try_collect().await
...
pub async fn get_theme(user: AuthUser, State(mongo): State<MongoHandle>, Path(theme_id): Path<String>) -> Result<Json<SabshopTheme>> {
    let user_id = user_oid(&user)?;
...
#[instrument(skip_all, fields(user_id = %user.user_id))]
pub async fn create_theme(user: AuthUser, State(mongo): State<MongoHandle>, Json(input): Json<CreateThemeInput>) -> Result<Json<CreateThemeResponse>> {
    let user_id = user_oid(&user)?;
    ...
    Ok(Json(CreateThemeResponse { id: new_id.to_hex(), entity }))
}

#[instrument(skip_all, fields(user_id = %user.user_id, theme_id = %theme_id))]
pub async fn update_theme(user: AuthUser, State(mongo): State<MongoHandle>, Path(theme_id): Path<String>, Json(patch): Json<UpdateThemeInput>) -> Result<Json<SabshopTheme>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&theme_id)?;
    let coll = mongo.collection::<SabshopTheme>(COLL);
    let before = coll.find_one(ownership(user_id, oid)).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabshop_themes.find_one")))?
        .ok_or_else(|| ApiError::NotFound("theme".to_owned()))?;
    let update = build_update_doc(patch);
    let r = coll.update_one(ownership(user_id, oid), update).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabshop_themes.update")))?;
    if r.matched_count == 0 { return Err(ApiError::NotFound("theme".to_owned())); }
    let after = coll.find_one(ownership(user_id, oid)).await
...
#[instrument(skip_all, fields(user_id = %user.user_id, theme_id = %theme_id))]
pub async fn delete_theme(user: AuthUser, State(mongo): State<MongoHandle>, Path(theme_id): Path<String>) -> Result<Json<DeleteThemeResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&theme_id)?;
    let coll = mongo.collection::<SabshopTheme>(COLL);
    ...
    if r.deleted_count == 0 { return Err(ApiError::NotFound("theme".to_owned())); }
...
mod tests {
    ...
    fn rejects_empty_name() {
        let u = ObjectId::new();
        assert!(entity_from_create(CreateThemeInput { name: " ".into(), ..Default::default() }, u).is_err());
    }
}
