...
use crate::types::MailRule;

const COLL: &str = "mail_rules";
const ENTITY_KIND: &str = "mail_rule";
...
pub async fn list_rules(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<MailRule>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.find")))?;
    let mut rows: Vec<MailRule> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.collect")))?;
...
pub async fn get_rule(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.find_one")))?
        .ok_or_else(|| ApiError::NotFound("mail_rule".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn create_rule(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_rule(
    ...
    coll.update_one(ownership(user_id, oid), doc! { "$set": set })
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.update")))?;
    let after = coll
        .find_one(ownership(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.refetch")))?
        .ok_or_else(|| ApiError::NotFound("mail_rule".to_owned()))?;
...
pub async fn delete_rule(
    ...
    let res = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_rules.archive")))?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("mail_rule".to_owned()));
    }
