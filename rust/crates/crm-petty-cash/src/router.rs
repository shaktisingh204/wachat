//! Mountable router. Mount under `/v1/crm/petty-cash`.
...
{
    Router::new()
        .route("/", get(handlers::list_floats).post(handlers::create_float))
        .route(
            "/{floatId}",
            get(handlers::get_float)
                .patch(handlers::update_float)
                .delete(handlers::delete_float),
        )
}
