...
fn list_filter(user_id: ObjectId, status: Option<&str>, client: Option<ObjectId>) -> Document {
    ...
    match status.unwrap_or("open") {
        "all" => {}
        s @ ("open" | "filled" | "closed") => { filter.insert("status", s); }
        _ => { filter.insert("status", doc! { "$ne": "closed" }); }
    }
...
pub async fn list_jobs(
    ...
    let mut rows: Vec<SabworkerlyJob> = cursor.try_collect().await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e)))?;
...
pub async fn create_job(
    user: AuthUser, State(mongo): State<MongoHandle>, Json(input): Json<CreateJobInput>,
    ...
    let coll = mongo.collection::<SabworkerlyJob>(COLL);
    let inserted = coll.insert_one(&job).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e)))?;
    let new_id = inserted.inserted_id.as_object_id()
        .ok_or_else(|| ApiError::Internal(anyhow::anyhow!("inserted_id was not ObjectId")))?;
    job.id = Some(new_id);
    Ok(Json(CreateJobResponse { id: new_id.to_hex(), entity: job }))
}

#[instrument(skip_all)]
pub async fn update_job(
    user: AuthUser, State(mongo): State<MongoHandle>,
    Path(id): Path<String>, Json(patch): Json<UpdateJobInput>,
) -> Result<Json<SabworkerlyJob>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let mut set = doc! { "updatedAt": BsonDateTime::from_chrono(Utc::now()) };
    if let Some(v) = patch.client_id { set.insert("clientId", oid_from_str(&v)?); }
    ...
    if let Some(v) = patch.shift_pattern { set.insert("shiftPattern", v); }
    ...
    if let Some(v) = patch.start_date { set.insert("startDate", BsonDateTime::from_chrono(v)); }
    ...
    let result = coll.update_one(own(user_id, oid), doc! { "$set": set }).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e)))?;
    if result.matched_count == 0 { return Err(ApiError::NotFound("job".to_owned())); }
