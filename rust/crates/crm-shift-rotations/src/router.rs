//! Mountable routers for the ShiftRotation entity.
//!
...
//!   `/v1/crm/shift-rotations`. Behaviour is unchanged.
    ...
    Arc<AuthConfig>: FromRef<S>,
{
    Router::new()
        .route(
            "/",
            get(handlers::list_rotations).post(handlers::create_rotation),
        )
        .route(
            "/{rotationId}",
            get(handlers::get_rotation)
                .patch(handlers::update_rotation)
                .delete(handlers::delete_rotation),
        )
}

/// Legacy `userId`-scoped router — mount under `/v1/crm/shift-rotations`.
...
/// `/v1/sabcrm/people/shift-rotations`. Same handlers, same
/// `crm_shift_rotations` collection; every request must carry
