...
pub trait ComplianceStore: Send + Sync {
    ...
    async fn check_opt_status(&self, phone_number: &str, sender: Option<&str>) -> Result<OptStatus>;
    
    /// Update the opt-status (e.g. when STOP or START is received)
    async fn update_opt_status(&self, phone_number: &str, sender: Option<&str>, status: OptStatus) -> Result<()>;
}
