...
async fn test_tcpa_gdpr_opt_out() {
    ...
    assert_eq!(result.unwrap_err().to_string(), "Recipient has opted out (Suppression list)");
}
...
async fn test_tcpa_gdpr_quiet_hours() {
    ...
    let result = rule.evaluate(&ctx, store).await;
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().to_string(), "Message violates TCPA/GDPR rules: Message blocked due to quiet hours restrictions.");
}
