...
pub async fn list_pages(
    ...
    let cursor = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.find"))
        })?;
    let mut rows: Vec<SabmonitorStatusPage> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.collect"))
    })?;
...
pub async fn get_page(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<SabmonitorStatusPage>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    mongo
        .collection::<SabmonitorStatusPage>(COLL)
        .find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.find_one"))
        })?
        .map(Json)
...
pub async fn create_page(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateStatusPageInput>,
) -> Result<Json<CreateStatusPageResponse>> {
    let user_id = user_oid(&user)?;
    ...
    let r = coll.insert_one(&entity).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.insert"))
    })?;
    let id = r
        .inserted_id
        .as_object_id()
    ...
    entity.id = Some(id);
    Ok(Json(CreateStatusPageResponse {
        id: id.to_hex(),
        entity,
    }))
}
...
pub async fn update_page(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
    Json(patch): Json<UpdateStatusPageInput>,
) -> Result<Json<SabmonitorStatusPage>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    ...
    if let Some(v) = patch.show_historical_uptime {
        set.insert("showHistoricalUptime", v);
    }
    if let Some(v) = patch.custom_header {
        set.insert("customHeader", v);
    }
    ...
    let r = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.update"))
        })?;
    if r.matched_count == 0 {
        return Err(ApiError::NotFound("status_page".into()));
    }
    coll.find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.refetch"))
        })?
        .map(Json)
        .ok_or_else(|| ApiError::NotFound("status_page".into()))
}
...
pub async fn delete_page(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<DeleteStatusPageResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabmonitorStatusPage>(COLL);
    let r = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.delete"))
        })?;
...
pub async fn public_get_by_slug(
...
) -> Result<Json<PublicStatusPageView>> {
    let coll = mongo.collection::<SabmonitorStatusPage>(COLL);
    let page = coll
        .find_one(doc! { "slug": &slug, "status": "live" })
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_pages.public_find"))
        })?
        .ok_or_else(|| ApiError::NotFound("status_page".into()))?;
    ...
    if !page.check_ids.is_empty() {
        let check_oids: Vec<bson::Bson> = page
            .check_ids
            .iter()
        ...
        let cursor = mongo
            .collection::<bson::Document>(CHECKS_COLL)
            .find(filter)
            .await
            .map_err(|e| {
                ApiError::Internal(
                    anyhow::Error::new(e).context("sabmonitor_status_pages.public_find_checks"),
                )
            })?;
        let docs: Vec<bson::Document> = cursor.try_collect().await.map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_pages.public_find_checks_collect"),
            )
        })?;
        for d in docs {
            let id = d
