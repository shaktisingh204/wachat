//! Mountable routers for the §12.3 RFQ endpoints.
//!
...
//!   `/v1/crm/rfqs`. Behaviour unchanged.
//! - [`project_router`] — SabCRM Supply suite surface, scoped by a
//!   required `projectId`. Mount under `/v1/sabcrm/supply/rfqs`.
//!
//! Routes (relative):
//!
//! ```text
...
/// Legacy `userId`-scoped router — mount under `/v1/crm/rfqs`.
...
/// `/v1/sabcrm/supply/rfqs`. Same handlers, same collection; every
