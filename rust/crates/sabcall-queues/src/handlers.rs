...
pub async fn list_queues(
    ...
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.find"))
        })?;
    let mut rows: Vec<VoiceQueue> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.collect"))
    })?;
...
pub async fn get_queue(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.find_one")))?
...
pub async fn create_queue(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_queue(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.update")))?;
    ...
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.refetch")))?
        .ok_or_else(|| ApiError::NotFound("voice_queue".to_owned()))?;
...
pub async fn delete_queue(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcall_queues.archive"))
        })?;
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("voice_queue".to_owned()));
    }
