//! HTTP handlers for the hosted Mail domain entity.
//!
...
use crate::types::MailDomain;

const COLL: &str = "mail_domains";
const ENTITY_KIND: &str = "mail_domain";
...
fn domain_from_create(input: CreateDomainInput, user_id: ObjectId) -> MailDomain {
    MailDomain {
        id: None,
        user_id,
...
fn doc_for_audit(d: &MailDomain) -> Document {
...
pub async fn list_domains(
    ...
    let coll = mongo.collection::<MailDomain>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.find")))?;
    let mut rows: Vec<MailDomain> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.collect")))?;
...
pub async fn get_domain(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.find_one")))?
        .ok_or_else(|| ApiError::NotFound("mail_domain".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn create_domain(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_domain(
    ...
    coll.update_one(ownership_filter(user_id, oid), build_update_doc(patch))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.update")))?;
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.refetch")))?
        .ok_or_else(|| ApiError::NotFound("mail_domain".to_owned()))?;
...
pub async fn delete_domain(
    ...
    let res = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_domains.archive")))?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("mail_domain".to_owned()));
    }
