...
const COLL: &str = "sabcheckout_preorders";
...
fn entity_from_create(input: CreateInput, user_id: ObjectId) -> Result<SabcheckoutPreorder> {
    Ok(SabcheckoutPreorder {
        id: None,
        user_id,
...
fn build_update_doc(patch: UpdateInput) -> Document {
    ...
    if let Some(v) = patch.target_quantity {
        set.insert("targetQuantity", v);
    }
...
pub struct ListResponse {
    pub items: Vec<SabcheckoutPreorder>,
    pub page: u32,
    pub limit: u32,
    pub has_more: bool,
}
...
pub async fn list(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<SabcheckoutPreorder>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("list.find")))?;
    let mut rows: Vec<SabcheckoutPreorder> = cursor
        .try_collect()
        .await
...
pub async fn get(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<SabcheckoutPreorder>> {
    let user_id = user_oid(&user)?;
    ...
    let row = coll
        ...
        .ok_or_else(|| ApiError::NotFound("sabcheckout_preorders".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn update(
    ...
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("sabcheckout_preorders".to_owned()));
    }
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
