//! On-disk shape of a `crm_shifts` document.

use bson::{DateTime as BsonDateTime, oid::ObjectId};
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct CrmShift {
    #[serde(rename = "_id", skip_serializing_if = "Option::is_none")]
    pub id: Option<ObjectId>,
    #[serde(rename = "userId")]
    pub user_id: ObjectId,

    /// SabCRM tenant scope. Stamped on rows created through the
    /// project-scoped mount (`/v1/sabcrm/people/shifts`); absent on
    /// legacy user-scoped rows — which are therefore invisible on the
    /// project mount (accepted clean-start per people-suite §2.1.7; no
    /// `userId` fallback, that would cross-tenant-leak).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub project_id: Option<ObjectId>,

    /// Display name, e.g. "Morning", "Evening", "Night".
    pub name: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub code: Option<String>,

    /// `HH:MM` 24-hour string, e.g. `"09:00"`.
    pub start_time: String,
    /// `HH:MM` 24-hour string, e.g. `"18:00"`.
    pub end_time: String,

    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub break_minutes: Option<i32>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub grace_minutes: Option<i32>,

    #[serde(default)]
    pub is_night_shift: bool,

    /// Day codes such as `"Mon"`, `"Tue"`, `"Wed"`…
    #[serde(default)]
    pub working_days: Vec<String>,

    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub color: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub description: Option<String>,

    #[serde(default)]
    pub is_default: bool,

    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub department_ids: Vec<ObjectId>,

    #[serde(default = "default_true")]
    pub is_active: bool,

    /// `"active"` | `"archived"`.
    pub status: String,

    #[serde(rename = "createdAt")]
    pub created_at: BsonDateTime,
    #[serde(rename = "updatedAt", default, skip_serializing_if = "Option::is_none")]
    pub updated_at: Option<BsonDateTime>,
}

fn default_true() -> bool {
    true
}
