//! HTTP handlers for the mailbox account entity.
...
const COLL: &str = "mail_accounts";
const DOMAINS_COLL: &str = "mail_domains";
const ENTITY_KIND: &str = "mail_account";
...
fn doc_for_audit(a: &MailAccount) -> Document {
...
pub async fn list_accounts(
    ...
    let coll = mongo.collection::<MailAccount>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.find")))?;
    let mut rows: Vec<MailAccount> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.collect")))?;
...
pub async fn get_account(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.find_one")))?
        .ok_or_else(|| ApiError::NotFound("mail_account".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn create_account(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_account(
    ...
    coll.update_one(ownership_filter(user_id, oid), build_update_doc(patch))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.update")))?;
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.refetch")))?
        .ok_or_else(|| ApiError::NotFound("mail_account".to_owned()))?;
...
pub async fn delete_account(
    ...
    let res = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_accounts.archive")))?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("mail_account".to_owned()));
    }
