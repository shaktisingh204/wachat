...
pub async fn list_timesheets(
    ...
    if let Some(s) = q.placement_id.as_deref() {
        if !s.is_empty() {
            filter.insert("placementId", oid_from_str(s)?);
        }
    }
    ...
    let mut rows: Vec<SabworkerlyTimesheet> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e)))?;
...
pub async fn create_timesheet(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateTimesheetInput>,
    ...
    t.id = Some(new_id);
    Ok(Json(CreateTimesheetResponse {
        id: new_id.to_hex(),
        entity: t,
    }))
}
...
pub async fn update_timesheet(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
    Json(patch): Json<UpdateTimesheetInput>,
) -> Result<Json<SabworkerlyTimesheet>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    ...
    if let Some(v) = patch.total_hours {
        set.insert("totalHours", v);
    }
    ...
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("timesheet".to_owned()));
    }
    let after = coll
        .find_one(own(user_id, oid))
        .await
...
pub async fn delete_timesheet(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e)))?;
    Ok(Json(DeleteTimesheetResponse {
...
pub async fn submit_timesheet(
    ...
    if result.matched_count == 0 {
        return Err(ApiError::Validation(
            "timesheet not in draft/rejected".to_owned(),
        ));
    }
    let after = coll
        .find_one(own(user_id, oid))
...
pub async fn approve_timesheet(
    ...
    if result.matched_count == 0 {
        return Err(ApiError::Validation(
            "timesheet not in submitted state".to_owned(),
        ));
    }
    let after = coll
        .find_one(own(user_id, oid))
