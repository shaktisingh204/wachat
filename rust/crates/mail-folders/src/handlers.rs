//! HTTP handlers for mail folders.
...
use crate::types::MailFolder;

const COLL: &str = "mail_folders";
const ENTITY_KIND: &str = "mail_folder";
...
pub async fn list_folders(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<MailFolder>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.find")))?;
    let mut rows: Vec<MailFolder> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.collect")))?;
...
pub async fn get_folder(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.find_one")))?
        .ok_or_else(|| ApiError::NotFound("mail_folder".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn create_folder(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_folder(
    ...
    coll.update_one(ownership(user_id, oid), doc! { "$set": set })
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.update")))?;
    let after = coll
        .find_one(ownership(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.refetch")))?
        .ok_or_else(|| ApiError::NotFound("mail_folder".to_owned()))?;
...
pub async fn delete_folder(
    ...
    let res = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_folders.archive")))?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("mail_folder".to_owned()));
    }
