//! HTTP handlers for SabCheckout SabvaultSecrets.
...
use crate::dto::{
    CreateSabvaultSecretInput, CreateSabvaultSecretResponse, DeleteSabvaultSecretResponse,
    ListQuery, UpdateSabvaultSecretInput,
};
use crate::types::SabvaultSecret;

const COLL: &str = "sabvault_secrets";
...
fn secret_from_create(
    input: CreateSabvaultSecretInput,
    user_id: ObjectId,
) -> Result<SabvaultSecret> {
...
fn build_update_doc(patch: UpdateSabvaultSecretInput) -> Document {
...
pub async fn list_secrets(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<SabvaultSecret>(COLL);
    let cursor =
...
pub async fn create_secret(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateSabvaultSecretInput>,
) -> Result<Json<CreateSabvaultSecretResponse>> {
    let user_id = user_oid(&user)?;
    let mut entity = secret_from_create(input, user_id)?;
    let coll = mongo.collection::<SabvaultSecret>(COLL);
    ...
    Ok(Json(CreateSabvaultSecretResponse {
        id: new_id.to_hex(),
        entity,
    }))
}
...
pub async fn update_secret(
    ...
    State(mongo): State<MongoHandle>,
    Path(secret_id): Path<String>,
    Json(patch): Json<UpdateSabvaultSecretInput>,
) -> Result<Json<SabvaultSecret>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&secret_id)?;
    let coll = mongo.collection::<SabvaultSecret>(COLL);
    ...
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvault_secrets.refetch")))?
        .ok_or_else(|| ApiError::NotFound("sabvault_secret".to_owned()))?;
    Ok(Json(after))
}
...
pub async fn delete_secret(
    ...
    State(mongo): State<MongoHandle>,
    Path(secret_id): Path<String>,
) -> Result<Json<DeleteSabvaultSecretResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&secret_id)?;
    let coll = mongo.collection::<SabvaultSecret>(COLL);
    let result = coll
    ...
    Ok(Json(DeleteSabvaultSecretResponse {
...
mod tests {
    ...
    fn create_rejects_bad_interval() {
        let input = CreateSabvaultSecretInput {
            name: "Pro".into(),
            interval_unit: "decade".into(),
            amount_minor: 1000,
            ..Default::default()
        };
        assert!(secret_from_create(input, ObjectId::new()).is_err());
    }
    ...
    fn create_defaults_count_and_status() {
        ...
        let p = secret_from_create(input, ObjectId::new()).unwrap();
