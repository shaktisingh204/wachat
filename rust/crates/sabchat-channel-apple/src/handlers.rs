...
use crate::state::SabChatChannelAppleState;
...
pub async fn ingest(
    State(state): State<SabChatChannelAppleState>,
...
pub async fn delivered(
    ...
    let result = messages
        .update_one(
            doc! {
                "inboxId": inbox_id,
                "providerMetadata.updateId": &body.provider_message_id,
            },
            doc! {
...
async fn resolve_inbox(mongo: &MongoHandle, account_id: &str) -> Result<Document> {
    ...
    .ok_or_else(|| {
        ApiError::NotFound(format!(
            "No apple inbox configured for accountId {account_id}."
        ))
    })
}
