...
fn build_update_doc(patch: UpdateCheckInput) -> Document {
    ...
    if let Some(v) = patch.headers_json {
        set.insert("headersJson", v);
    }
    if let Some(v) = patch.body_json {
        set.insert("bodyJson", v);
    }
    if let Some(v) = patch.expected_status {
        set.insert("expectedStatus", v);
    }
    if let Some(v) = patch.expected_body_contains {
        set.insert("expectedBodyContains", v);
    }
    if let Some(v) = patch.expected_body_regex {
        set.insert("expectedBodyRegex", v);
    }
    if let Some(v) = patch.ssl_expiry_warn_days {
        set.insert("sslExpiryWarnDays", v);
    }
    if let Some(v) = patch.synthetic_script_id {
        if let Ok(oid) = ObjectId::parse_str(&v) {
            set.insert("syntheticScriptId", oid);
        }
    }
    if let Some(v) = patch.api_transaction_id {
        if let Ok(oid) = ObjectId::parse_str(&v) {
            set.insert("apiTransactionId", oid);
        }
    }
...
pub async fn list_checks(
    ...
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.find"))
        })?;
    let mut rows: Vec<SabmonitorCheck> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.collect"))
    })?;
...
pub async fn get_check(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.find_one"))
        })?
...
pub async fn create_check(
    ...
    let coll = mongo.collection::<SabmonitorCheck>(COLL);
    let res = coll.insert_one(&entity).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.insert"))
    })?;
    let id = res
        .inserted_id
        .as_object_id()
...
pub async fn update_check(
    ...
    let coll = mongo.collection::<SabmonitorCheck>(COLL);
    let r = coll
        .update_one(ownership(user_id, oid), build_update_doc(patch))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.update"))
        })?;
    ...
    let row = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.refetch"))
        })?
        .ok_or_else(|| ApiError::NotFound("check".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn delete_check(
    ...
    let r = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_checks.delete"))
        })?;
...
mod tests {
    ...
    fn update_doc_omits_unset() {
        let d = build_update_doc(UpdateCheckInput {
            name: Some("New".into()),
            ..Default::default()
        });
