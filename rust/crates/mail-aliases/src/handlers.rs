//! HTTP handlers for mail aliases.
...
use crate::types::MailAlias;

const COLL: &str = "mail_aliases";
const ENTITY_KIND: &str = "mail_alias";
...
fn doc_for_audit(a: &MailAlias) -> Document {
...
pub async fn list_aliases(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<MailAlias>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.find")))?;
    let mut rows: Vec<MailAlias> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.collect")))?;
...
pub async fn get_alias(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.find_one")))?
        .ok_or_else(|| ApiError::NotFound("mail_alias".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn create_alias(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_alias(
    ...
    coll.update_one(ownership(user_id, oid), update)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.update")))?;
    let after = coll
        .find_one(ownership(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.refetch")))?
        .ok_or_else(|| ApiError::NotFound("mail_alias".to_owned()))?;
...
pub async fn delete_alias(
    ...
    let res = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("mail_aliases.archive")))?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("mail_alias".to_owned()));
    }
