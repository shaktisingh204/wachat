...
use crate::dto::{
    CreateIvrInput, CreateIvrResponse, DeleteIvrResponse, ListQuery, UpdateIvrInput,
};
use crate::types::VoiceIvr;

const COLL: &str = "sabvoice_ivrs";
...
pub async fn list_ivrs(
    ...
    let cursor = coll
        ...
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.find")))?;
    let mut rows: Vec<VoiceIvr> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.collect")))?;
...
pub async fn get_ivr(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.find_one")))?
...
pub async fn create_ivr(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_ivr(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.update")))?;
    ...
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.refetch")))?
        .ok_or_else(|| ApiError::NotFound("voice_ivr".to_owned()))?;
...
pub async fn delete_ivr(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_ivrs.archive")))?;
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("voice_ivr".to_owned()));
    }
