...
pub async fn list_calls(
    ...
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.find"))
        })?;
    let mut rows: Vec<VoiceCall> = cursor
        .try_collect()
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.collect")))?;
...
pub async fn get_call(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.find_one")))?
...
pub async fn create_call(
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_call(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.update")))?;
    ...
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.refetch")))?
        .ok_or_else(|| ApiError::NotFound("voice_call".to_owned()))?;
...
pub async fn delete_call(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabcall_calls.delete")))?;
