...
pub async fn list_entries(
    user: AuthUser, State(state): State<SabcatalystFileStoreState>,
    ...
    let cur = state.mongo.collection::<Document>(FILE_STORE_COLL)
...
pub async fn create_entry(
    ...
    let dup = coll.find_one(doc! { "projectId": project, "key": &body.key }).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("file_store.dup")))?;
    if dup.is_some() { return Err(ApiError::Conflict("key already exists in project".into())); }
    let d = doc! {
        "_id": ObjectId::new(),
...
pub async fn delete_entry(
    user: AuthUser, State(state): State<SabcatalystFileStoreState>, Path(id): Path<String>,
) -> Result<StatusCode> {
    let owner = owner_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let r = state.mongo.collection::<Document>(FILE_STORE_COLL)
        .delete_one(doc! { "_id": oid, "userId": owner }).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("file_store.delete")))?;
    if r.deleted_count == 0 { return Err(ApiError::NotFound("File entry not found.".into())); }
