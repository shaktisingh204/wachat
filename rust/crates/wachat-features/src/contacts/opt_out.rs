//! Opt-out / DND list.
//!
//! Mirrors `getOptOutList`, `addToOptOut`, `removeFromOptOut`.
...
const COLL: &str = "wa_opt_outs";

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct OptOutsResp {
    pub opt_outs: Value,
}
...
pub async fn add(
    ...
    coll.update_one(
        ...
        doc! {
            "$set": {
                "reason": body.reason.unwrap_or_else(|| "manual".to_owned()),
                "optedOutAt": bson::DateTime::from_chrono(Utc::now()),
            },
...
pub async fn remove(
    _user: AuthUser,
    Path(opt_out_id): Path<String>,
    State(state): State<WachatFeaturesState>,
) -> Result<Json<OkResp>> {
    let oid = opt_oid(&opt_out_id)?;
    let coll = state.mongo.collection::<Document>(COLL);
