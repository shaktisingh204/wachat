...
pub async fn list(
    ...
    let cursor = coll.find(filter).with_options(opts).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_page_incidents.find"))
    })?;
    let mut rows: Vec<SabmonitorStatusPageIncident> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(
            anyhow::Error::new(e).context("sabmonitor_status_page_incidents.collect"),
        )
    })?;
...
pub async fn get_one(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<SabmonitorStatusPageIncident>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    mongo
        .collection::<SabmonitorStatusPageIncident>(COLL)
        .find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_page_incidents.find_one"),
            )
        })?
        .map(Json)
...
pub async fn create(
    ...
    let r = coll.insert_one(&entity).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_status_page_incidents.insert"))
    })?;
    let id = r
        .inserted_id
        .as_object_id()
...
pub async fn update(
    ...
    let r = coll
        ...
        .map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_page_incidents.update"),
            )
        })?;
    if r.matched_count == 0 {
        return Err(ApiError::NotFound("status_page_incident".into()));
    }
    coll.find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_page_incidents.refetch"),
            )
        })?
        .map(Json)
        .ok_or_else(|| ApiError::NotFound("status_page_incident".into()))
...
pub async fn delete(
    ...
    let r = coll
        ...
        .map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_page_incidents.delete"),
            )
        })?;
...
pub async fn public_list_by_status_page(
    State(mongo): State<MongoHandle>,
    Path(sp_id): Path<String>,
) -> Result<Json<ListResponse>> {
    ...
    let coll = mongo.collection::<SabmonitorStatusPageIncident>(COLL);
    let cursor = coll
        .find(doc! { "statusPageId": sp_oid })
        .with_options(opts)
        .await
        .map_err(|e| {
            ApiError::Internal(
                anyhow::Error::new(e).context("sabmonitor_status_page_incidents.public_find"),
            )
        })?;
    let rows: Vec<SabmonitorStatusPageIncident> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(
            anyhow::Error::new(e).context("sabmonitor_status_page_incidents.public_collect"),
        )
    })?;
    Ok(Json(ListResponse {
        items: rows,
