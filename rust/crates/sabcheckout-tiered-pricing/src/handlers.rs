...
use crate::types::SabcheckoutTieredPricing;

const COLL: &str = "sabcheckout_tiered_pricings";
...
fn entity_from_create(input: CreateInput, user_id: ObjectId) -> Result<SabcheckoutTieredPricing> {
    Ok(SabcheckoutTieredPricing {
        id: None,
        user_id,
...
pub struct ListResponse {
    pub items: Vec<SabcheckoutTieredPricing>,
    pub page: u32,
    pub limit: u32,
    pub has_more: bool,
}
...
pub async fn list(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<SabcheckoutTieredPricing>(COLL);
    let cursor = coll
        .find(filter)
        .with_options(opts)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("list.find")))?;
    let mut rows: Vec<SabcheckoutTieredPricing> = cursor
        .try_collect()
        .await
...
pub async fn get(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<SabcheckoutTieredPricing>> {
    let user_id = user_oid(&user)?;
    ...
    let row = coll
        ...
        .ok_or_else(|| ApiError::NotFound("sabcheckout_tiered_pricings".to_owned()))?;
    Ok(Json(row))
}
...
pub async fn update(
    ...
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("sabcheckout_tiered_pricings".to_owned()));
    }
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
