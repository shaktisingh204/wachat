//! HTTP handlers for SabCheckout SabcheckoutCoupons.
...
use crate::dto::{
    CreateSabcheckoutCouponInput, CreateSabcheckoutCouponResponse, DeleteSabcheckoutCouponResponse,
    ListQuery, UpdateSabcheckoutCouponInput,
};
use crate::types::SabcheckoutCoupon;

const COLL: &str = "sabcheckout_coupons";
...
fn entity_from_create(
    input: CreateSabcheckoutCouponInput,
    user_id: ObjectId,
) -> Result<SabcheckoutCoupon> {
    Ok(SabcheckoutCoupon::from_input(input, user_id))
}

fn build_update_doc(patch: UpdateSabcheckoutCouponInput) -> Document {
...
pub struct ListResponse {
    pub items: Vec<SabcheckoutCoupon>,
    pub page: u32,
    pub limit: u32,
    pub has_more: bool,
}
...
pub async fn list_coupons(
    ...
    let opts = FindOptions::builder()
        ...
        .limit(limit + 1)
        .build();
    let coll = mongo.collection::<SabcheckoutCoupon>(COLL);
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context(format!("{COLL}.find")))
        })?;
    let mut rows: Vec<SabcheckoutCoupon> = cursor.try_collect().await.map_err(|e| {
...
pub async fn create_coupon(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateSabcheckoutCouponInput>,
) -> Result<Json<CreateSabcheckoutCouponResponse>> {
    let user_id = user_oid(&user)?;
    ...
    Ok(Json(CreateSabcheckoutCouponResponse {
        id: new_id.to_hex(),
        entity,
    }))
}
...
pub async fn update_coupon(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
    Json(patch): Json<UpdateSabcheckoutCouponInput>,
) -> Result<Json<SabcheckoutCoupon>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabcheckoutCoupon>(COLL);
...
pub async fn delete_coupon(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<DeleteSabcheckoutCouponResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabcheckoutCoupon>(COLL);
    let result = coll
    ...
    Ok(Json(DeleteSabcheckoutCouponResponse {
