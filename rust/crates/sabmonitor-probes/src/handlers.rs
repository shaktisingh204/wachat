...
pub async fn list_probes(
    ...
    let cursor =
        coll.find(filter).with_options(opts).await.map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.find"))
        })?;
    let mut rows: Vec<SabmonitorProbe> = cursor.try_collect().await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.collect"))
    })?;
...
pub async fn get_probe(
    ...
    coll.find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.find_one"))
        })?
        .map(Json)
...
pub async fn create_probe(
    user: AuthUser,
    State(mongo): State<MongoHandle>,
    Json(input): Json<CreateProbeInput>,
) -> Result<Json<CreateProbeResponse>> {
    let user_id = user_oid(&user)?;
    ...
    let r = coll.insert_one(&entity).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.insert"))
    })?;
    let id = r
        .inserted_id
        .as_object_id()
...
pub async fn update_probe(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
    Json(patch): Json<UpdateProbeInput>,
) -> Result<Json<SabmonitorProbe>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    ...
    let r = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.update"))
        })?;
    if r.matched_count == 0 {
        return Err(ApiError::NotFound("probe".into()));
    }
    coll.find_one(ownership(user_id, oid))
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.refetch"))
        })?
        .map(Json)
        .ok_or_else(|| ApiError::NotFound("probe".into()))
}
...
pub async fn delete_probe(
    ...
    State(mongo): State<MongoHandle>,
    Path(id): Path<String>,
) -> Result<Json<DeleteProbeResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let coll = mongo.collection::<SabmonitorProbe>(COLL);
    let r = coll
        ...
        .await
        .map_err(|e| {
            ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_probes.delete"))
        })?;
