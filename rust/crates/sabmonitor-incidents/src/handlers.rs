...
#[instrument(skip_all)]
pub async fn list_incidents(user: AuthUser, State(mongo): State<MongoHandle>, Query(q): Query<ListQuery>) -> Result<Json<ListResponse>> {
    ...
    let cursor = coll.find(filter).with_options(opts).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.find")))?;
    let mut rows: Vec<SabmonitorIncident> = cursor.try_collect().await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.collect")))?;
...
#[instrument(skip_all)]
pub async fn create_incident(user: AuthUser, State(mongo): State<MongoHandle>, Json(input): Json<CreateIncidentInput>) -> Result<Json<CreateIncidentResponse>> {
    let user_id = user_oid(&user)?;
    ...
    let r = coll.insert_one(&entity).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.insert")))?;
    let id = r.inserted_id.as_object_id().ok_or_else(|| ApiError::Internal(anyhow::anyhow!("inserted_id missing")))?;
    entity.id = Some(id);
    Ok(Json(CreateIncidentResponse { id: id.to_hex(), entity }))
}

#[instrument(skip_all)]
pub async fn acknowledge_incident(user: AuthUser, State(mongo): State<MongoHandle>, Path(id): Path<String>) -> Result<Json<AckResponse>> {
    let user_id = user_oid(&user)?;
    ...
    ).await.map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.ack")))?;
    if r.matched_count == 0 { return Err(ApiError::NotFound("incident".into())); }
    let row = coll.find_one(ownership(user_id, oid)).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.refetch")))?
        .ok_or_else(|| ApiError::NotFound("incident".into()))?;
    Ok(Json(AckResponse { entity: row }))
}

#[instrument(skip_all)]
pub async fn resolve_incident(user: AuthUser, State(mongo): State<MongoHandle>, Path(id): Path<String>) -> Result<Json<AckResponse>> {
    let user_id = user_oid(&user)?;
    let oid = oid_from_str(&id)?;
    let now = BsonDateTime::from_chrono(Utc::now());
    let coll = mongo.collection::<SabmonitorIncident>(COLL);
    ...
    let before = coll.find_one(ownership(user_id, oid)).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.find_for_resolve")))?
        .ok_or_else(|| ApiError::NotFound("incident".into()))?;
    ...
    ).await.map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.resolve")))?;
    if r.matched_count == 0 { return Err(ApiError::NotFound("incident".into())); }
    let row = coll.find_one(ownership(user_id, oid)).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_incidents.refetch")))?
        .ok_or_else(|| ApiError::NotFound("incident".into()))?;
