...
// =========================================================================
// DELETE /:soId — delete_sales_order (soft)
// =========================================================================

/// `DELETE /v1/crm/sales-orders/:soId` — soft delete. Sets `archived =
/// true` and stamps `deletedAt`. The row stays in the collection so
/// downstream lineage stays accurate. Fails with 404 if the SO doesn't
...
pub async fn delete_sales_order(
    ...
    let res = coll.update_one(filter, update).await.map_err(|e| {
        ApiError::Internal(anyhow::Error::new(e).context("crm_sales_orders.soft_delete"))
    })?;
    if res.matched_count == 0 {
        return Err(ApiError::NotFound("sales order".to_owned()));
    }
