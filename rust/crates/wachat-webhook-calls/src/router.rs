//! Axum router for the call-log read, mounted at `/v1/wachat/webhook-calls`.
...
use crate::{list, state::WachatWebhookCallsState};
...
where
    ...
    WachatWebhookCallsState: FromRef<S>,
    Arc<AuthConfig>: FromRef<S>,
{
    Router::new().route("/projects/{id}/calls", get(list_calls))
}

async fn list_calls(
    user: AuthUser,
    State(s): State<WachatWebhookCallsState>,
    Path(id): Path<String>,
) -> Result<Json<list::CallsResponse>> {
