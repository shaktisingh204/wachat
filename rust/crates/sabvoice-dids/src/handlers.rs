...
pub async fn get_did(
    ...
    let row = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.find_one")))?
...
pub async fn create_did(
    ...
    let existing = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.find_dup")))?;
    if existing.is_some() {
    ...
    let inserted = coll
        .insert_one(&entity)
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.insert")))?;
    let new_id = inserted
        .inserted_id
        .as_object_id()
...
pub async fn update_did(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.update")))?;
    ...
    let after = coll
        .find_one(ownership_filter(user_id, oid))
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.refetch")))?
        .ok_or_else(|| ApiError::NotFound("voice_did".to_owned()))?;
...
pub async fn delete_did(
    ...
    let result = coll
        ...
        .await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabvoice_dids.release")))?;
    if result.matched_count == 0 {
        return Err(ApiError::NotFound("voice_did".to_owned()));
    }
