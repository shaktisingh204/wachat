...
/// Recursively reject the Mongo operators that can execute arbitrary
/// JavaScript or server-side expressions. The access clause is generated by the
/// trusted Next server, but this seam still refuses executable operators so the
/// raw-clause channel can NEVER be turned into a code-execution vector — defence
/// in depth around a security-critical filter.
fn assert_no_code_operators(value: &Bson) -> Result<()> {
    match value {
        Bson::Document(d) => {
            for (k, v) in d {
                if matches!(
                    ...
                    return Err(ApiError::BadRequest(
                        "`accessFilter` may not contain executable operators.".to_owned(),
                    ));
                }
                assert_no_code_operators(v)?;
            }
            Ok(())
        }
        Bson::Array(arr) => arr.iter().try_for_each(assert_no_code_operators),
        _ => Ok(()),
    }
}

/// `$and`-merge a server-generated read-path access-enforcement clause onto an
/// already-built query `base`. This is the Rust half of the SabCRM two-store
/// enforcement parity: the entire policy (OWD, role hierarchy, sharing rules,
/// territories) is resolved ONCE in the trusted Next server
/// (`src/lib/sabcrm/access-readpath.ts`) and arrives here as a ready Mongo
/// clause — Rust applies it mechanically, so there is no second copy of the
/// access LOGIC to keep in lockstep.
///
/// Contract:
///  - `access_filter` absent / blank / `{}` → returns `base` UNCHANGED (the
///    default-OFF passthrough: the query is byte-for-byte what it is today).
///  - present → `{ "$and": [ base, clause ] }` so it can only NARROW the result
///    set, never widen, and never collides with the base's own `$or`/`$and`.
///  - executable operators are rejected (`400`); a deny clause such as
///    `{ "_id": null }` matches no real record → fail-closed.
fn apply_access_filter(base: Document, access_filter: Option<&str>) -> Result<Document> {
    let raw = match access_filter.map(str::trim).filter(|s| !s.is_empty()) {
        Some(r) => r,
        None => return Ok(base),
    };
    let parsed: Value = serde_json::from_str(raw)
        .map_err(|e| ApiError::BadRequest(format!("invalid `accessFilter` JSON: {e}")))?;
    let clause = match bson::to_bson(&parsed)
        .map_err(|e| ApiError::BadRequest(format!("invalid `accessFilter`: {e}")))?
    {
        Bson::Document(d) => d,
        _ => {
            return Err(ApiError::BadRequest(
                "`accessFilter` must be a JSON object.".to_owned(),
            ))
        }
    };
    // An empty clause means "nothing to enforce" → passthrough, identical query.
    if clause.is_empty() {
        return Ok(base);
    }
    assert_no_code_operators(&Bson::Document(clause.clone()))?;
    Ok(doc! { "$and": [Bson::Document(base), Bson::Document(clause)] })
}
...
pub async fn list_records(
    ...
    // Read-path access enforcement (default-OFF passthrough). Wrapping the base
    // filter here covers BOTH the find() path and the relation-join aggregation
    // path below, since both reuse this `filter` as their match.
    let filter = apply_access_filter(filter, query.access_filter.as_deref())?;
...
pub async fn count_records(
    ...
    // Same access enforcement as list_records so the count matches the list.
    let filter = apply_access_filter(filter, query.access_filter.as_deref())?;
...
pub async fn get_record(
    ...
    // Read-path access enforcement: a record the viewer may not see resolves to
    // `404` (same as an out-of-scope id) so existence is never leaked.
    let filter = apply_access_filter(filter, query.access_filter.as_deref())?;
...
pub async fn group_records(
    ...
    // Read-path access enforcement (default-OFF) — the board respects the same
    // row scope as the list. Delivered in the POST body as a URL-encoded JSON
    // Mongo clause string (same shape as the list/count query param).
    let match_filter = apply_access_filter(match_filter, body.access_filter.as_deref())?;
...
pub async fn aggregate_records(
    ...
    let match_filter =
        build_list_filter(project_id, &object, None, filters_str.as_deref())?;
    // Read-path access enforcement (default-OFF) — bucket metrics must not leak
    // rollups (sums/avgs) over records the viewer may not see.
    let match_filter = apply_access_filter(match_filter, body.access_filter.as_deref())?;
    ...
    let total_filter =
        build_list_filter(project_id, &object, None, filters_str.as_deref())?;
    // Same enforcement as the bucket pass so the grand total agrees with it.
    let total_filter = apply_access_filter(total_filter, body.access_filter.as_deref())?;
...
mod access_filter_tests {
    //! Unit cover for the read-path access-enforcement seam
    //! ([`apply_access_filter`] + [`assert_no_code_operators`]). These guard the
    //! two-store enforcement parity: the default-OFF passthrough must be exact,
    //! a present clause must NARROW via `$and`, and executable operators must be
    //! refused.
    use super::*;

    fn base() -> Document {
        doc! { "projectId": "p1", "object": "deals", "deletedAt": { "$in": [Bson::Null] } }
    }

    #[test]
    fn none_is_byte_for_byte_passthrough() {
        let out = apply_access_filter(base(), None).unwrap();
        assert_eq!(out, base(), "absent clause must not alter the query");
    }

    #[test]
    fn blank_and_empty_object_are_passthrough() {
        assert_eq!(apply_access_filter(base(), Some("   ")).unwrap(), base());
        assert_eq!(apply_access_filter(base(), Some("{}")).unwrap(), base());
    }

    #[test]
    fn present_clause_is_anded_onto_base() {
        let clause = r#"{"$or":[{"userId":{"$in":["u1","u2"]}},{"data.assignedTo":{"$in":["u1","u2"]}}]}"#;
        let out = apply_access_filter(base(), Some(clause)).unwrap();
        let and = out.get_array("$and").expect("expected a $and merge");
        assert_eq!(and.len(), 2, "base + clause");
        // First element is the untouched base; second carries the owner $or.
        assert_eq!(and[0].as_document().unwrap(), &base());
        assert!(and[1].as_document().unwrap().contains_key("$or"));
    }

    #[test]
    fn deny_sentinel_is_preserved_and_matches_nothing() {
        // `{ "_id": null }` is the fail-closed sentinel; it must survive the merge
        // so a misconfiguration denies rather than leaks.
        let out = apply_access_filter(base(), Some(r#"{"_id":null}"#)).unwrap();
        let and = out.get_array("$and").unwrap();
        assert_eq!(and[1].as_document().unwrap().get("_id").unwrap(), &Bson::Null);
    }

    #[test]
    fn executable_operators_are_rejected() {
        for clause in [
            r#"{"$where":"this.x==1"}"#,
            r#"{"$expr":{"$eq":["$a","$b"]}}"#,
            r#"{"$or":[{"$function":{"body":"x","args":[],"lang":"js"}}]}"#,
            r#"{"data.x":{"$accumulator":{}}}"#,
        ] {
            assert!(
                apply_access_filter(base(), Some(clause)).is_err(),
                "must reject executable operator clause: {clause}"
            );
        }
    }

    #[test]
    fn invalid_json_and_non_object_are_rejected() {
        assert!(apply_access_filter(base(), Some("{not json")).is_err());
        assert!(apply_access_filter(base(), Some("[1,2,3]")).is_err());
        assert!(apply_access_filter(base(), Some("\"a string\"")).is_err());
    }
}
