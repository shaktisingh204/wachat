...
pub struct ListQuery {
    ...
    /// Optional URL-encoded JSON **Mongo clause** generated by the SabCRM
    /// read-path access-enforcement engine (`src/lib/sabcrm/access-readpath.ts`)
    /// in the trusted Next server. It is `$and`-merged onto the base
    /// `{ projectId, object }` scope so it can only ever NARROW the result set
    /// (never widen). Absent / empty (the DEFAULT for every project today) →
    /// the query is byte-for-byte unchanged — the default-OFF passthrough that
    /// keeps the Rust read path identical to the native-TS read path. Executable
    /// operators (`$where`/`$expr`/`$function`/`$accumulator`) are rejected with
    /// a `400`. See `apply_access_filter` in `handlers.rs`.
    #[serde(default)]
...
pub struct CountQuery {
    ...
    #[serde(default)]
    pub filters: Option<String>,
    /// Read-path access-enforcement clause — same contract as
    /// [`ListQuery::access_filter`]. `$and`-merged so the count matches the
    /// list. Absent / empty → unchanged (default-OFF passthrough).
    #[serde(default)]
    pub access_filter: Option<String>,
}
...
pub struct ScopeQuery {
    ...
    pub enrich: Option<String>,
    /// Read-path access-enforcement clause — same contract as
    /// [`ListQuery::access_filter`]. Consulted ONLY by `get_record` (a denied
    /// record then resolves to `404`, never leaking existence); the delete
    /// handlers ignore it. Absent / empty → unchanged (default-OFF passthrough).
    #[serde(default)]
    pub access_filter: Option<String>,
}
...
pub struct GroupRecordsInput {
    ...
    /// Read-path access-enforcement clause (URL-encoded JSON Mongo object) — same
    /// contract as [`ListQuery::access_filter`], delivered in the POST body
    /// rather than a query param. `$and`-merged into the `$match` so the board
    /// respects the same row scope as the list. Absent / empty → unchanged
...
pub struct AggregateInput {
    ...
    #[serde(default)]
    pub filters: Option<Value>,
    /// Read-path access-enforcement clause (URL-encoded JSON Mongo object) —
    /// same contract as [`ListQuery::access_filter`], applied to BOTH the bucket
    /// pass and the grand-total pass so neither leaks rollups over hidden
    /// records. Absent / empty → unchanged (default-OFF passthrough).
    #[serde(default)]
    pub access_filter: Option<String>,
}
