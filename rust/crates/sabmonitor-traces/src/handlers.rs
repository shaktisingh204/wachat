...
#[instrument(skip_all)]
pub async fn list_traces(user: AuthUser, State(mongo): State<MongoHandle>, Query(q): Query<ListQuery>) -> Result<Json<ListResponse>> {
    ...
    let cursor = coll.find(filter).with_options(opts).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_traces.find")))?;
    let mut rows: Vec<SabmonitorTrace> = cursor.try_collect().await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_traces.collect")))?;
...
#[instrument(skip_all)]
pub async fn get_trace(user: AuthUser, State(mongo): State<MongoHandle>, Path(trace_id): Path<String>) -> Result<Json<SabmonitorTrace>> {
    let uid = user_oid(&user)?;
    let coll = mongo.collection::<SabmonitorTrace>(COLL);
    coll.find_one(doc! { "userId": uid, "traceId": &trace_id }).await
        .map_err(|e| ApiError::Internal(anyhow::Error::new(e).context("sabmonitor_traces.find_one")))?
        .map(Json)
