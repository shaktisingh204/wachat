...
export default function SabwriterDocEditorPage() {
  ...
  const handleSendForSignature = async () => {
    ...
    if (envelopeId) {
      router.push(`/sabsign/${envelopeId}`);
    } else {
      router.push('/sabsign');
    }
  };
      ...
      <div className="flex items-center justify-between gap-3 border-b border-[var(--st-border)] px-4 py-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <Input
            ...
            className="border-0 shadow-none text-lg font-medium px-1"
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex -space-x-2" aria-label="People editing this document">
            ...
            {presence.length > 5 ? (
              <Badge tone="neutral" kind="outline">+{presence.length - 5}</Badge>
            ) : null}
          </div>
          <Button
            variant="outline"
            size="sm"
            iconLeft={History}
            onClick={() => router.push(`/sabsign/docs/${doc._id}/history`)}
          >
            History
          </Button>
          <Button variant="outline" size="sm" iconLeft={Save} onClick={handleSaveVersion}>
            Save version
          </Button>
          <Button
            variant="outline"
        ...
        <aside className="border-l border-[var(--st-border)] overflow-auto">
          ...
          <section className="p-4 border-b border-[var(--st-border)]">
            <header className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-[var(--st-text)] inline-flex items-center gap-2">
                <Sparkles className="h-4 w-4" aria-hidden="true" /> Suggestions
              </h3>
              <Badge tone="neutral" kind="outline">{suggestions.length}</Badge>
            </header>
            {suggestions.length === 0 ? (
              <EmptyState
                ...
                description="Edits proposed by collaborators will show up here."
              />
            ) : (
                    ...
                    <Card variant="outlined" padding="sm">
                      <CardBody>
                        <p className="text-xs text-[var(--st-text-secondary)] mb-2">
                          Range {s.anchor.from} to {s.anchor.to}
                        </p>
          ...
          <section className="p-4">
            <header className="flex items-center justify-between mb-3">
              ...
              <Badge tone="neutral" kind="outline">{openComments.length}</Badge>
            </header>
            ...
            {comments.length === 0 ? (
              ...
              <ul className="flex flex-col gap-2">
                {comments.map((c) => (
                  <li key={c._id}>
                    <Card variant="outlined" padding="sm" className={c.resolved ? 'opacity-60' : undefined}>
                      <CardBody>
                        <Link
                          href={`/sabsign/docs/${doc._id}/comments/${c._id}`}
                          className="block"
                        >
                        ...
                        <div className="flex items-center justify-between mt-1">
                          ...
                          {!c.resolved ? (
                            ...
                            <Badge tone="success" kind="outline">Resolved</Badge>
                          )}
                        </div>
                      </CardBody>
                    </Card>
