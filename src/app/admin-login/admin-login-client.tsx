    ...
    return (
        <ErrorBoundary fallback={
            <div className="min-h-screen flex items-center justify-center p-4 bg-zinc-950">
                <Card className="bg-zinc-900 border-zinc-800 text-zinc-100 p-6 flex flex-col items-center gap-4 text-center max-w-md">
                    <AlertCircle className="h-10 w-10 text-red-500" />
                    <div>
                        <h2 className="text-lg font-semibold text-zinc-100">Something went wrong</h2>
                        <p className="text-sm text-zinc-400">There was a problem loading the admin portal.</p>
                    </div>
    ...
    return (
        <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="pointer-events-none absolute inset-0 overflow-hidden">
                ...
                <div className="absolute -bottom-40 -right-40 h-96 w-96 rounded-full bg-amber-400/10 blur-[120px]" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-64 w-64 rounded-full bg-zinc-800/40 blur-[80px]" />
            </div>
            ...
            <div className="absolute top-6 left-6 z-10">
                <Link href="/" className="flex items-center gap-2 text-zinc-400 hover:text-amber-400 text-sm transition-colors">
            ...
            <div className="relative w-full max-w-md">
                <Card className="bg-zinc-900 border-zinc-800 text-zinc-100 shadow-2xl">
                    <ZoruCardHeader className="text-center space-y-3">
                        ...
                        {mode === 'setup' ? (
                            <>
                                <ZoruCardTitle className="text-2xl font-bold text-zinc-100 tracking-tight">First-time setup</ZoruCardTitle>
                                <ZoruCardDescription className="text-sm text-zinc-400">Create the admin account for this workspace</ZoruCardDescription>
                            </>
                        ) : (
                            <>
                                <ZoruCardTitle className="text-2xl font-bold text-zinc-100 tracking-tight">Admin Portal</ZoruCardTitle>
                                <ZoruCardDescription className="text-sm text-zinc-400">Restricted access — authorized personnel only</ZoruCardDescription>
                            </>
                        )}
                    </ZoruCardHeader>
                    ...
                    <ZoruCardContent className="space-y-6">
                        ...
                        {state.error && state.error !== 'NEEDS_SETUP' && (
                            <Alert variant="destructive" className="border-red-500/40 bg-red-500/10 text-red-300">
                                ...
                                <ZoruAlertDescription className="text-sm text-red-300">{state.error}</ZoruAlertDescription>
                            </Alert>
                        )}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col gap-3">
                                    <Button
                                        ...
                                        className="bg-zinc-900 border-zinc-700 text-zinc-200 hover:bg-zinc-800 hover:text-zinc-100"
                                        leading={
                                            <svg className="h-4 w-4 text-zinc-300 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                                        }
                                    >
                                        Corporate SSO (SAML)
                                    </Button>
    ...
    return (
        <form action={action} className="space-y-4">
            ...
            <Field label="Password">
                <div className="relative">
                    <Input
                        ...
                        placeholder="••••••••••••"
                        className="bg-zinc-900 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 pr-12"
                    />
            ...
            <Button
                ...
                disabled={isPending}
                block
                className="bg-amber-500 hover:bg-amber-400 text-zinc-950 font-semibold border-amber-500 hover:border-amber-400 shadow-amber-500/25 mt-2"
            >
                {isPending
...
function PasswordToggle({ show, onToggle }: { show: boolean; onToggle: () => void }) {
    return (
        <button
            type="button"
            onClick={onToggle}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-amber-400 transition-colors z-10"
            tabIndex={-1}
        >
