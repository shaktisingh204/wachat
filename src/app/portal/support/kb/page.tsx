...
export default async function PortalSupportKbPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-[18px] font-semibold text-zoru-ink">
      ...
      {error ? (
        <Card>
          <ZoruCardContent className="p-4 text-[13px] text-zoru-danger">{error}</ZoruCardContent>
        </Card>
      ) : visible.length === 0 ? (
        <EmptyState
