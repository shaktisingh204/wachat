...
export async function handleAdminLogin(prevState: any, formData: FormData) {
    ...
    if (!adminEmail || !adminPasswordHash || !jwtSecret) {
        ...
        console.log(`[ADMIN_LOGIN] ERROR: ${errorMessage}`);
        return { success: false, error: errorMessage };
    }

    if (email !== adminEmail) {
        console.log('[ADMIN_LOGIN] Email does not match.');
    ...
    try {
        ...
        if (!isMatch) {
            console.log('[ADMIN_LOGIN] Password does not match.');
            return { success: false, error: "Invalid credentials." };
        }
        
        console.log('[ADMIN_LOGIN] Password matches. Creating session token...');
        const token = await createAdminSessionToken();
        console.log('[ADMIN_LOGIN] Session token created.');
