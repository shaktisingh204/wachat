...
/**
 * SabCall (Cloud-PBX) server actions.
 *
 * Direct Mongo access is the primary path; the Rust crates (`sabcall-*`)
 * mirror the same on-disk shapes and are wired in when `USE_RUST_SABCALL`
 * is enabled. Every doc is scoped to the active SabCall project (the tenant)
 * resolved from the `sabcall_project` cookie — `tenantId()` returns that
 * project `_id` (the workspace id). The `userId` field on each doc holds this
 * tenant id (kept for shape-compatibility with the Rust crates).
 *
 * Real SIP / Asterisk integration is intentionally out of scope for P0 —
 * provider plumbing is stubbed (`provider: 'mock'`) and the DID-search helper
 * returns synthetic numbers. The engine phase replaces these with the live
 * Asterisk/ARI control plane.
 */
 ...
 * Every collection scopes by this (stored in the `userId` field). Throws
 * when no project is selected — the layout gate prevents pages from reaching
 * an action without a selected, set-up project.
 */
...
/** The acting user (person) id — for identity fields like voicemail listeners. */
async function currentPersonId(): Promise<string> {
...
export async function listVoiceDids(opts: { q?: string; status?: string } = {}) {
  ...
  const rows = await db
    .collection(SABCALL_COLLECTIONS.dids)
    .find(filter)
    .sort({ createdAt: -1 })
    .toArray();
...
/**
 * Mock-mode DID search. Returns up to 50 synthetic available numbers.
 * Replaced by the real Asterisk/provider number search in the engine phase.
 */
...
export async function purchaseVoiceDid(input: VoiceDidInput) {
  ...
  const now = new Date();
  // TODO: integrate real provisioning (Asterisk/provider) here when the
  // engine phase lands — currently we just record the intent.
  const doc = {
    userId,
  ...
  const result = await db.collection(SABCALL_COLLECTIONS.dids).insertOne(doc);
  revalidatePath('/sabcall/dids');
...
export async function updateVoiceDid(id: string, patch: Partial<VoiceDidInput>) {
  ...
  await db.collection(SABCALL_COLLECTIONS.dids).updateOne({ _id, userId }, { $set: set });
  revalidatePath('/sabcall/dids');
...
export async function releaseVoiceDid(id: string) {
  ...
  if (!_id) throw new Error('Invalid id.');
  const { db } = await connectToDatabase();
  // TODO: call provider release API when the engine phase is wired.
  await db
    .collection(SABCALL_COLLECTIONS.dids)
    .updateOne(
      { _id, userId },
...
export async function listVoiceIvrs(opts: { q?: string; status?: string } = {}) {
  ...
  const rows = await db
    .collection(SABCALL_COLLECTIONS.ivrs)
    .find(filter)
    .sort({ createdAt: -1 })
    .toArray();
  return { success: true as const, data: rows.map((r) => stringifyIds(r as never)) };
...
export async function getVoiceIvr(id: string) {
  ...
  const { db } = await connectToDatabase();
  const row = await db.collection(SABCALL_COLLECTIONS.ivrs).findOne({ _id, userId });
...
export async function createVoiceIvr(input: VoiceIvrInput) {
  ...
  const result = await db.collection(SABCALL_COLLECTIONS.ivrs).insertOne(doc);
  revalidatePath('/sabcall/ivr');
  return { success: true as const, id: String(result.insertedId) };
}
...
export async function updateVoiceIvr(id: string, patch: Partial<VoiceIvrInput>) {
  ...
  const { db } = await connectToDatabase();
  await db.collection(SABCALL_COLLECTIONS.ivrs).updateOne({ _id, userId }, { $set: set });
  revalidatePath('/sabcall/ivr');
  return { success: true as const };
}
...
export async function listVoiceQueues(opts: { q?: string; status?: string } = {}) {
  ...
  if (opts.q) filter.name = { $regex: opts.q, $options: 'i' };
  const rows = await db
    .collection(SABCALL_COLLECTIONS.queues)
    .find(filter)
    .sort({ createdAt: -1 })
    .toArray();
  return { success: true as const, data: rows.map((r) => stringifyIds(r as never)) };
...
export async function createVoiceQueue(input: VoiceQueueInput) {
  ...
  const result = await db.collection(SABCALL_COLLECTIONS.queues).insertOne(doc);
  revalidatePath('/sabcall/queues');
  return { success: true as const, id: String(result.insertedId) };
}
...
export async function updateVoiceQueue(id: string, patch: Partial<VoiceQueueInput>) {
  ...
  if (patch.status) set.status = patch.status;
  const { db } = await connectToDatabase();
  await db.collection(SABCALL_COLLECTIONS.queues).updateOne({ _id, userId }, { $set: set });
  revalidatePath('/sabcall/queues');
  return { success: true as const };
}
...
export async function createVoiceCallCdr(input: VoiceCallInput) {
  ...
  const result = await db.collection(SABCALL_COLLECTIONS.calls).insertOne(doc);
  revalidatePath('/sabcall/calls');
  return { success: true as const, id: String(result.insertedId) };
}
...
export async function deleteVoiceCallCdr(id: string) {
  ...
  if (!_id) throw new Error('Invalid id.');
  const { db } = await connectToDatabase();
  await db.collection(SABCALL_COLLECTIONS.calls).deleteOne({ _id, userId });
  revalidatePath('/sabcall/calls');
  return { success: true as const };
}
...
export async function listVoicemails(opts: { q?: string; status?: string } = {}) {
  ...
  const rows = await db
    .collection(SABCALL_COLLECTIONS.voicemail)
    .find(filter)
    .sort({ createdAt: -1 })
    .toArray();
  return { success: true as const, data: rows.map((r) => stringifyIds(r as never)) };
...
export async function createVoicemail(input: VoicemailInput) {
  ...
  const result = await db.collection(SABCALL_COLLECTIONS.voicemail).insertOne(doc);
  revalidatePath('/sabcall/voicemail');
  return { success: true as const, id: String(result.insertedId) };
}
...
export async function markVoicemailListened(id: string) {
  ...
  const _id = toObjectIdOrNull(id);
  if (!_id) throw new Error('Invalid id.');
  // The listener is the acting person, not the tenant (workspace) id.
  const listener = new ObjectId(await currentPersonId());
  const { db } = await connectToDatabase();
  await db.collection(SABCALL_COLLECTIONS.voicemail).updateOne(
    { _id, userId },
    {
...
export async function listAgentPresence(opts: { status?: AgentPresenceStatus } = {}) {
  ...
  const rows = await db.collection(SABCALL_COLLECTIONS.agentsPresence).find(filter).toArray();
  return { success: true as const, data: rows.map((r) => stringifyIds(r as never)) };
}
...
export async function upsertAgentPresence(input: AgentPresenceInput) {
  ...
  await db.collection(SABCALL_COLLECTIONS.agentsPresence).updateOne(
  ...
  revalidatePath('/sabcall/agent-dashboard');
  return { success: true as const };
}
...
export async function getVoiceLiveKpis() {
  ...
  const [agents, callsToday, queues] = await Promise.all([
    db.collection(SABCALL_COLLECTIONS.agentsPresence).find({ userId }).toArray(),
    db
      .collection(SABCALL_COLLECTIONS.calls)
      .countDocuments({
        userId,
    ...
    db.collection(SABCALL_COLLECTIONS.queues).find({ userId, status: 'active' }).toArray(),
  ]);
