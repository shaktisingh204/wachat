...
interface PurchaseOrder {
  ...
  createdAt?: string;
  updatedAt?: string;
  vendorId?: string; totalAmount?: number; approvedBy?: string; status?: string;
}
...
export async function listPurchaseOrders(): Promise<{ items: PurchaseOrder[], error?: string }> {
  try {
    const session = await getSession();
    if (!session?.user?._id) throw new Error("Unauthorized");
    
    const { db } = await connectToDatabase();
    const docs = await db.collection('finance_po_approvals')
...
export async function createPurchaseOrder(data: any) {
  try {
    ...
    const res = await db.collection('finance_po_approvals').insertOne(doc);
...
export async function updatePurchaseOrder(id: string, data: any) {
  try {
    ...
    await db.collection('finance_po_approvals').updateOne(
...
export async function deletePurchaseOrder(id: string) {
  try {
    ...
    const { db } = await connectToDatabase();
    await db.collection('finance_po_approvals').deleteOne({ 
      _id: new ObjectId(id), 
      userId: new ObjectId(String(session.user._id)) 
    });
