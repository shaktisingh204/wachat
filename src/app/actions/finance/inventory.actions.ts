...
export interface InventoryItem {
  ...
  createdAt?: string;
  updatedAt?: string;
  sku?: string; name?: string; totalQty?: number; status?: string;
}
...
export async function listInventoryItems(): Promise<{ items: InventoryItem[], error?: string }> {
  try {
    const session = await getSession();
    if (!session?.user?._id) throw new Error("Unauthorized");
    
    const { db } = await connectToDatabase();
    const docs = await db.collection('finance_inventory')
...
export async function createInventoryItem(data: any) {
  try {
    ...
    const res = await db.collection('finance_inventory').insertOne(doc);
...
export async function updateInventoryItem(id: string, data: any) {
  try {
    ...
    await db.collection('finance_inventory').updateOne(
...
export async function deleteInventoryItem(id: string) {
  try {
    ...
    const { db } = await connectToDatabase();
    await db.collection('finance_inventory').deleteOne({ 
      _id: new ObjectId(id), 
      userId: new ObjectId(String(session.user._id)) 
    });
