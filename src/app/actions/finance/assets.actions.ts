...
export interface Asset {
  ...
  createdAt?: string;
  updatedAt?: string;
  name?: string; purchasePrice?: number; salvageValue?: number; usefulLifeYears?: number; accumulatedDepreciation?: number;
}
...
const schema = z.object({
  ...
  accumulatedDepreciation: z.number().optional().default(0)
});

export async function listAssets(): Promise<{ items: Asset[], error?: string }> {
  try {
    const session = await getSession();
    if (!session?.user?._id) throw new Error("Unauthorized");
    
    const { db } = await connectToDatabase();
    const docs = await db.collection('finance_assets')
...
export async function createAsset(data: any) {
  try {
    ...
    const res = await db.collection('finance_assets').insertOne(doc);
...
export async function updateAsset(id: string, data: any) {
  try {
    ...
    await db.collection('finance_assets').updateOne(
...
export async function deleteAsset(id: string) {
  try {
    ...
    const { db } = await connectToDatabase();
    await db.collection('finance_assets').deleteOne({ 
      _id: new ObjectId(id), 
      userId: new ObjectId(String(session.user._id)) 
    });
