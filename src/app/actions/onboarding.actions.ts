...
export async function handleWabaOnboarding(code: string, state: any, includeCatalog: boolean): Promise<{ success: boolean; error?: string }> {
  const cookieStore = cookies();
  const stateCookie = cookieStore.get('onboarding_state')?.value;

  if (!stateCookie) {
    return { success: false, error: "Onboarding state is missing. Please try again." };
  }

  const { userId, state: storedState } = JSON.parse(stateCookie);
  ...
  try {
    ...
    // Step 4: Get WABA ID, name, and business ID
    const { data: wabaData } = await axios.get(`https://graph.facebook.com/${API_VERSION}/me/whatsapp_business_accounts`, {
      params: {
    ...
    if (!waba) {
      throw new Error("Could not find a WhatsApp Business Account associated with this login.");
    }
    const wabaId = waba.id;
    const wabaName = waba.name || `WABA ${wabaId}`;
    const businessId = waba.business?.id;
    console.log(`${LOG_PREFIX} Step 4: Fetched WABA details. ID: ${wabaId}, Name: ${wabaName}, Business ID: ${businessId}`);
