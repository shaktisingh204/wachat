...
export default function RedirectScript({ originalUrl, isIos, isAndroid }: RedirectScriptProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
            <Loader2 className="w-12 h-12 text-[var(--st-text)] animate-spin mb-6" />
            <h1 className="text-2xl font-semibold mb-2">
                Opening link{dots}
            </h1>
            <p className="text-[var(--st-text-secondary)] max-w-sm">
                If you are not redirected automatically within a few seconds, please click the button below.
            </p>
            
            <a 
                href={originalUrl}
                className="mt-8 text-[var(--st-text)] hover:underline font-medium"
            >
