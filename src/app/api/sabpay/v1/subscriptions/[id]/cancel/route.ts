...
/**
 * SabPay public API — cancel a subscription.
 *
 *   POST /api/sabpay/v1/subscriptions/:id/cancel[?at_cycle_end=1]
 *
 * When `at_cycle_end` is truthy the subscription keeps billing until the end
 * of the current cycle, then stops; otherwise it cancels immediately.
 *
...
export async function POST(
  ...
  const atCycleEndParam = new URL(req.url).searchParams.get('at_cycle_end');
  const atCycleEnd =
    atCycleEndParam === '1' ||
    atCycleEndParam === 'true' ||
    atCycleEndParam === 'yes';

  try {
    const subscription = await rustClient.sabpay.cancelSubscriptionAs(
