/**
 * CRM Public API — /v1/crm/accounts (Phase 7 foundation, hand-written).
...
const STRING_FIELDS = ["name","industry","website","phone","email","address","country","state","city","gstin","pan"] as const;

const handlers = makeCrmCollectionHandlers<CreateInput, UpdateInput>({
    entity: 'accounts',
    collection: 'crm_accounts',
    searchableFields: ["name","industry","website","phone","email"],
    webhookEvents: { created: 'account.created' },
    coerceCreate: (body) => {
