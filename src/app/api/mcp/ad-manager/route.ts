/**
 * SabNode MCP endpoint — Meta Ad Manager.
 *
 *   POST /api/mcp/ad-manager
 *
 * A stateless Streamable-HTTP MCP server exposing the workspace's Meta
 * Ad Manager as agent tools. Authentication reuses the developer
 * platform: send a SabNode API key as `Authorization: Bearer <key>`
 * (or `X-Api-Key`). The key must hold an `ads:read` or `ads:write`
 * scope — which is how the feature is plan-gated, since keys only
 * receive ad scopes on plans that include Ad Manager. Per-tool scope
 * is enforced again at call time inside the dispatcher.
 *
...
import { dispatchMcp } from '@/lib/api-platform/mcp/server';
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  // 3. Plan gate: the key must carry at least one ad scope to reach the server.
  if (!requireScope('ads:read', ctx) && !requireScope('ads:write', ctx)) {
    return ApiError.scopeMissing('ads:read').toResponse(requestId);
  }
  ...
  for (const raw of messages) {
    ...
    const res = await dispatchMcp(raw as JsonRpcRequest, ctx);
