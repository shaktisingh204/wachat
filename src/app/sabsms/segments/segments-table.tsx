...
export function SegmentsTable({
  ...
  const columns: SabsmsColumn<SegmentListRow>[] = [
    ...
    },
    {
      ...
      align: "right",
      render: (row) => (
        <span className="text-sm text-slate-700">{row.campaignsUsing}</span>
      ),
    },
    {
      id: "drips",
      ...
      align: "right",
      render: (row) => (
        <span className="text-sm text-slate-700">{row.dripsUsing}</span>
      ),
    },
    {
      id: "refreshed",
    ...
    <div className="flex items-center justify-between">
      ...
      <span
        className={`text-sm tabular-nums ${
          emphasised ? "font-semibold text-slate-900" : "text-slate-800"
        }`}
      >
        {value}
      </span>
