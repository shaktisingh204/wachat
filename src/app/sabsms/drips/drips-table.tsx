...
export function DripsTable({ rows, templateOptions }: DripsTableProps) {
  ...
  const columns: SabsmsColumn<DripRow>[] = [
    ...
    },
    {
      ...
      hideByDefault: true,
      render: (r) => (
        <span className="text-xs text-slate-600">{r.cohort ?? "—"}</span>
      ),
    },
    {
      id: "autoPause",
        ...
        return (
          <span
            ...
            className={`h-1.5 w-1.5 rounded-full ${
              isBranch ? "bg-amber-500" : "bg-slate-300"
            }`}
            aria-hidden
          />
        );
      })}
      {count > 12 && <span className="ml-0.5 text-[10px] text-slate-400">+{count - 12}</span>}
    </span>
  );
}
