...
function statusGlyph(status: string) {
  if (status === "delivered") return <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />;
                    ...
                    <div
                      ...
                      className={
                        m.direction === "outbound"
                          ...
                          : "mr-12 rounded-md bg-slate-100 p-3"
                      }
                    >
              ...
              {state.consentEvents.length === 0 ? (
                ...
                <ol className="space-y-2 text-sm">
                  {state.consentEvents.map((e) => (
                    <li
                      key={e.id}
                      className="flex items-start justify-between rounded-md border border-slate-200 bg-white p-2"
                    >
                      <div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <ZoruCardContent className="space-y-2 text-sm">
              ...
              <div>
                <span className="text-slate-500">Country:</span> {state.country}
              </div>
              {state.name && (
                <div>
                  <span className="text-slate-500">Name:</span> {state.name}
                </div>
              )}
              {state.email && (
                <div>
                  <span className="text-slate-500">Email:</span> {state.email}
                </div>
              )}
              <div>
              ...
              <div>
                ...
                <span
                  className={
                    state.riskScore >= 50
                      ? "text-rose-600 inline-flex items-center gap-1"
                      : "text-slate-700"
                  }
                >
          ...
          <Card>
            ...
            <ZoruCardContent className="space-y-2 text-sm">
              {state.drips.length === 0 ? (
                <div className="text-slate-500">Not enrolled in any drip.</div>
              ) : (
                state.drips.map((d) => (
                  <div
                    key={d.id}
          ...
          <Card>
            ...
            <ZoruCardContent className="space-y-2 text-sm">
              {state.campaigns.length === 0 ? (
                <div className="text-slate-500">No campaign memberships.</div>
              ) : (
          ...
          <Card>
            ...
            <ZoruCardContent className="space-y-2 text-sm">
              {state.crmLeadId ? (
                ...
                <div className="rounded-md border border-dashed border-slate-200 p-2 text-slate-400">
                  No linked CRM lead
                </div>
              )}
              {state.crmDealId ? (
          ...
          <ul className="space-y-3 text-sm">
            {state.consentEvents.map((e) => (
              ...
              >
                ...
                {e.source && (
                  <div className="text-xs text-slate-500">via {e.source}</div>
                )}
              </li>
            ))}
          </ul>
