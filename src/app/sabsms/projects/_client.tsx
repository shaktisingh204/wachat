...
import { CreatingOverlay } from "@/components/sabsms/motion";
import {
  createSabsmsProject,
  setActiveSabsmsProject,
  type SabsmsProjectRow,
...
export function SabsmsProjectsClient({
  projects,
  activeProjectId,
}: {
  projects: SabsmsProjectRow[];
  ...
  const goToProject = React.useCallback(
    (id: string, setupComplete: boolean) => {
      ...
      router.push(setupComplete ? "/sabsms" : "/sabsms/setup");
    },
  ...
  const select = React.useCallback(
    async (id: string, setupComplete: boolean) => {
      setSelectingId(id);
      const res = await setActiveSabsmsProject(id);
      if (!res.success) {
        toast({
  ...
  const handleCreate = React.useCallback(async () => {
    ...
    setBusy(true);
    setCreateErr(null);
    const res = await createSabsmsProject({ name: trimmed });
    if (!res.success) {
      setCreateErr(res.error);
      setBusy(false);
      return;
    }
    // Show the "creating" overlay through to navigation.
    setOpen(false);
    setCreating(true);
    const sel = await setActiveSabsmsProject(res.projectId);
    if (!sel.success) {
      toast({
  ...
  return (
    <div className="relative mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <CreatingOverlay
        ...
        subtitle="Setting up your isolated SabSMS workspace"
      />
      <PageHeader>
        <PageHeaderHeading>
          <PageTitle>SabSMS projects</PageTitle>
          <PageDescription>
            Each project is an isolated messaging workspace — its own
            campaigns, contacts, numbers, providers, and compliance. Pick one
            to continue, or create a new one. New projects must finish a short
            setup before they can send.
          </PageDescription>
        </PageHeaderHeading>
        <Button
          variant="primary"
        ...
        {projects.length === 0 ? (
          <Card className="p-10">
            <EmptyState
              ...
              title="No SabSMS projects yet"
              description="Create your first project to set up providers, senders, and compliance, then start sending."
              action={
                <Button
                  variant="primary"
                  size="sm"
          ...
          <ul className="sabsms-motion grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((p, idx) => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>New SabSMS project</DialogTitle>
            <DialogDescription>
              A project is an isolated messaging workspace. After creating it
              you&apos;ll complete a short setup (business profile, provider,
              sender, and compliance) before sending.
            </DialogDescription>
          </DialogHeader>
          ...
          <Field label="Project name" error={createErr ?? undefined}>
            <Input
              ...
              placeholder="e.g. Marketing — India"
              autoFocus
              maxLength={120}
              onKeyDown={(e) => {
                if (e.key === "Enter" && !busy) {
