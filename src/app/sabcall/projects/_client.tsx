...
import {
  createSabcallProject,
  setActiveSabcallProject,
  type SabcallProjectRow,
...
export function SabcallProjectsClient({
  projects,
  activeProjectId,
}: {
  projects: SabcallProjectRow[];
  ...
  const goToProject = React.useCallback(
    (id: string, setupComplete: boolean) => {
      ...
      router.push(setupComplete ? "/sabcall" : "/sabcall/setup");
    },
  ...
  const select = React.useCallback(
    async (id: string, setupComplete: boolean) => {
      setSelectingId(id);
      const res = await setActiveSabcallProject(id);
      if (!res.success) {
        toast({
  ...
  const handleCreate = React.useCallback(async () => {
    ...
    setBusy(true);
    setCreateErr(null);
    const res = await createSabcallProject({ name: trimmed });
    if (!res.success) {
    ...
    setOpen(false);
    const sel = await setActiveSabcallProject(res.projectId);
    if (!sel.success) {
      toast({
  ...
  return (
    <div className="relative mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <PageHeader>
        <PageHeaderHeading>
          <PageTitle>SabCall projects</PageTitle>
          <PageDescription>
            Each project is an isolated calling workspace — its own numbers,
            IVRs, queues, voicemail, and call log. Pick one to continue, or
            create a new one. New projects must finish a short setup before
            they unlock.
          </PageDescription>
        </PageHeaderHeading>
        <Button
        ...
        {projects.length === 0 ? (
          <Card className="p-10">
            <EmptyState
              ...
              title="No SabCall projects yet"
              description="Create your first project to set up your calling workspace, then provision numbers and routing."
              action={
                <Button
                  variant="primary"
                  size="sm"
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>New SabCall project</DialogTitle>
            <DialogDescription>
              A project is an isolated calling workspace. After creating it
              you&apos;ll complete a short setup (business profile) before it
              unlocks.
            </DialogDescription>
          </DialogHeader>
