...
export default function MessageTemplatesLibraryError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[Wachat message-templates-library]', error);
  }, [error]);
  ...
  const message = error.message
    ? error.message.length > 160
      ...
      : error.message
    : 'The message templates library failed to load.';
    ...
    <WaPage>
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center text-center">
        ...
        <h1 className="mt-6 text-balance text-2xl font-semibold tracking-tight text-zinc-950">
          Could not load the message library.
        </h1>
