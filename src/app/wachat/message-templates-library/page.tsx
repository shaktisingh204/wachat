...
export default function MessageTemplatesLibraryPage() {
  ...
  return (
    <div className="clay-enter flex min-h-full flex-col gap-6">
      ...
      <div>
        <h1 className="text-[30px] font-semibold tracking-[-0.015em] text-foreground leading-[1.1]">Message Templates Library</h1>
        <p className="mt-1.5 text-[13px] text-muted-foreground">Browse your project templates or the premade library. Click to copy.</p>
      </div>
      ...
      {isPending && templates.length === 0 ? (
        ...
        <ClayCard className="p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">{tab === 'project' ? 'No templates in this project yet.' : 'No premade templates available.'}</p>
        </ClayCard>
      ) : (
          ...
          {templates.map((t) => (
            <ClayCard key={t.id} padded={false} className="flex flex-col p-5">
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="text-[15px] font-semibold text-foreground">{t.name.replace(/_/g, ' ')}</h3>
                <ClayBadge tone={TONE_MAP[t.category] || 'neutral'}>{t.category}</ClayBadge>
              </div>
              <p className="flex-1 text-[12.5px] text-muted-foreground leading-relaxed line-clamp-3">{t.body}</p>
              <div className="mt-4">
