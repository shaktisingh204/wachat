...
export default function TemplateBuilderPage() {
  ...
  return (
    <div className="clay-enter flex min-h-full flex-col gap-6">
      ...
      <div>
        <h1 className="text-[30px] font-semibold tracking-[-0.015em] text-foreground leading-[1.1]">Template Builder</h1>
        <p className="mt-1.5 text-[13px] text-muted-foreground">Build WhatsApp message templates visually. Save copies JSON to clipboard.</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <div className="flex flex-col gap-4">
          <ClayCard padded={false} className="p-5">
            <h2 className="text-[15px] font-semibold text-foreground mb-3">Category</h2>
          ...
          <ClayCard padded={false} className="p-5">
            <h2 className="text-[15px] font-semibold text-foreground mb-3">Header</h2>
            <div className="flex gap-2 mb-3">
              {(['none', 'text', 'image', 'video', 'document'] as const).map((t) => (
                <button key={t} onClick={() => setHeaderType(t)}
                  className={`rounded-md px-3 py-1.5 text-[12px] font-medium capitalize transition-colors ${headerType === t ? 'bg-foreground text-white' : 'bg-background text-muted-foreground hover:bg-muted border border-border'}`}>{t}</button>
              ))}
            </div>
            ...
            {(headerType === 'image' || headerType === 'video' || headerType === 'document') && (
              <p className="text-[12px] text-muted-foreground">Upload {headerType} when submitting for approval.</p>
            )}
          </ClayCard>

          <ClayCard padded={false} className="p-5">
            <h2 className="text-[15px] font-semibold text-foreground mb-3">Body</h2>
            ...
            <div className="mt-2 flex gap-2">
              <span className="text-[12px] text-muted-foreground self-center">Variables:</span>
              {[1, 2, 3].map((n) => (
                <button key={n} onClick={() => insertVar(n)} className="rounded-md border border-border bg-background px-2 py-1 text-[11px] font-mono text-foreground hover:bg-muted">{`{{${n}}}`}</button>
              ))}
            </div>
          </ClayCard>

          <ClayCard padded={false} className="p-5">
            <h2 className="text-[15px] font-semibold text-foreground mb-3">Footer</h2>
            <input className={inputCls} placeholder="Footer text (optional)" value={footer} onChange={(e) => setFooter(e.target.value)} />
          </ClayCard>

          <ClayCard padded={false} className="p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-[15px] font-semibold text-foreground">Buttons ({buttons.length}/3)</h2>
            ...
            {buttons.map((btn, i) => (
              <div key={i} className="flex flex-wrap items-center gap-2 mb-2 rounded-lg border border-border p-3">
                <select className="rounded-md border border-border bg-background px-2 py-1.5 text-[12px] text-foreground" value={btn.type} onChange={(e) => updateButton(i, { type: e.target.value as BtnType })}>
                  <option value="quick_reply">Quick Reply</option>
                  <option value="url">URL</option>
                  <option value="phone">Phone</option>
                </select>
                <input className="flex-1 min-w-[120px] rounded-md border border-border bg-background px-2 py-1.5 text-[12px] text-foreground" placeholder="Button label" value={btn.text} onChange={(e) => updateButton(i, { text: e.target.value })} />
                {btn.type !== 'quick_reply' && (
                  <input className="flex-1 min-w-[120px] rounded-md border border-border bg-background px-2 py-1.5 text-[12px] text-foreground" placeholder={btn.type === 'url' ? 'https://...' : '+1234567890'} value={btn.value} onChange={(e) => updateButton(i, { value: e.target.value })} />
                )}
                <button onClick={() => removeButton(i)} className="text-[12px] text-destructive hover:underline">Remove</button>
              </div>
            ))}
          </ClayCard>
