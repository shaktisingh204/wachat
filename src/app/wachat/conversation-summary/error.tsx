...
export default function ConversationSummaryError({
  error,
  reset,
}: {
  ...
  useEffect(() => { console.error('[Wachat Conversation Summary Error]', error); }, [error]);
  const message = error.message
    ? error.message.length > 160 ? `${error.message.slice(0, 160)}…` : error.message
    : 'Conversation summary failed to load.';
  ...
  return (
    <WaPage>
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center text-center">
        ...
        <h1 className="mt-6 text-balance text-2xl font-semibold tracking-tight text-zinc-950">Summary failed to load.</h1>
