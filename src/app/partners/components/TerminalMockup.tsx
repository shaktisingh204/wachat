    ...
    <div className="rounded border border-white/10 bg-black overflow-hidden relative group">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-[#0a0a0a]">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleCopy} className="text-white/30 hover:text-white transition-colors" title="Copy Command">
            {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
          </button>
          {!completed && !running && (
            <button onClick={runCommand} className="text-white/30 hover:text-white transition-colors" title="Run Simulation">
