...
export type KbKpiKey = 'all' | 'published' | 'drafts' | 'mostViewed' | 'helpful';
    ...
    return (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
            ...
            <KpiCard
                label="Helpful %"
                value={
                    counts.helpfulPct != null
                        ? `${counts.helpfulPct.toFixed(0)}%`
                        : '—'
                }
                icon={<ThumbsUp className="h-4 w-4" />}
                active={active === 'helpful'}
                onClick={() => onPick(active === 'helpful' ? 'all' : 'helpful')}
                period={counts.totalVotes > 0 ? `${counts.totalVotes} votes` : 'no votes'}
            />
        </div>
    );
}
