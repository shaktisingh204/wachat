...
export function KbListClient() {
  ...
  const [visibilityFilter, setVisibilityFilter] =
    React.useState<KbVisibilityFilter>("all");
  ...
  const visibleArticles = React.useMemo(() => {
    ...
    return articles.filter((a) => {
      ...
      if (q) {
        const hay =
          `${a.title ?? ""} ${a.slug ?? ""} ${a.body ?? ""} ${a.category ?? ""} ${(
            a.tags ?? []
          ).join(" ")}`.toLowerCase();
      ...
      if (
        ...
        updated < dateRange.from.getTime()
      ) {
        return false;
      }
      if (
  ...
  const handleToggleAll = React.useCallback(
    (all: boolean) => {
      setSelected(
        all ? new Set(visibleArticles.map((a) => String(a._id))) : new Set(),
      );
    },
    [visibleArticles],
  );
  ...
  const activePreset =
    VIEW_PRESETS.find((p) => p.id === activePresetId) ?? VIEW_PRESETS[0];
