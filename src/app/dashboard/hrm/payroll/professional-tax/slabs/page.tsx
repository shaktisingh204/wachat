 ...
 *
 * State-wise PT slab definitions. The records list at the parent route
 * stamps `slabApplied` at save time from these slabs so historical rows
...
type SaveSlabState = { message?: string | null; error?: string | null };
const saveInitialState: SaveSlabState = { message: null, error: null };
                    ...
                    <ZoruCard className="p-6">
                        <div className="mb-4">
                            ...
                            <p className="mt-0.5 text-[12.5px] text-zoru-ink-muted">
                                Calculated PT based on employee salary and defined
                                state slabs.
                            </p>
                        </div>
                                ...
                                <tbody>
                                    {isLoading ? (
                                        ...
                                        report.map((item, idx) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 text-right font-mono text-zoru-ink">
                                                    ...
                                                    }).format(item.grossSalary ?? 0)}
                                                </td>
                                                <td className="px-4 py-3 text-[12px] text-zoru-ink-muted">
                                                    ₹
                                                    {item.slabMin?.toLocaleString('en-IN') ??
                                                        '—'}{' '}
                                                    – ₹
                                                    {item.slabMax?.toLocaleString('en-IN') ??
                                                        '—'}
                                                </td>
                                                <td className="px-4 py-3 text-right font-mono text-zoru-ink">
                                                    {new Intl.NumberFormat('en-IN', {
                                        ...
                                        <tr>
                                            ...
                                            >
                                                No data. Add employees with salary and
                                                state info, then define slabs.
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                    ...
                    <ZoruCard className="p-6">
                        ...
                        {isLoading ? (
                            ...
                            <div className="rounded-lg border border-dashed border-zoru-line p-6 text-center text-[12.5px] text-zoru-ink-muted">
                                No slabs configured. Click &ldquo;Add New Slab&rdquo;
                                to start.
                            </div>
                        )}
                    </ZoruCard>
