...
export default function LeaveListClient({
  initialLeaves,
  initialTypes,
  initialEmployees,
  sessionUserId,
  ...
  const availableBalance = initialAvailableBalance;
  ...
  // Initial data loaded by server component
  ...
  const runBulkApprove = React.useCallback(async () => {
    ...
    setSelected(new Set());
    // In a real app we'd refresh from server, but here we can just do a page reload or let Server Actions revalidate
  ...
  const runBulkReject = React.useCallback(async () => {
    ...
    setSelected(new Set());
    // In a real app we'd refresh from server
    window.location.reload();
  }, [selected, toast]);
