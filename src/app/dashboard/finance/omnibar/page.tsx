      ...
      if (currentQuery.includes('software expenses')) {
        aiResponse = (
          <div className="space-y-2">
            ...
            <div className="h-32 w-full bg-[var(--st-bg-muted)] flex items-center justify-center rounded-md border text-xs text-[var(--st-text-secondary)]">
    ...
    <div className="p-6 space-y-6 h-[calc(100vh-64px)] flex flex-col" ref={containerRef}>
      <div>
        <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
          <TerminalSquare className="w-8 h-8 text-[var(--st-text)]" />
          The Omnibar
        </h1>
      ...
      <Card className="flex-1 flex flex-col min-h-0 relative overflow-hidden">
        ...
        <ScrollArea className="flex-1 p-4">
          <div className="space-y-6 max-w-4xl mx-auto">
            {messages.map((msg, idx) => (
              ...
              >
                {msg.role === 'ai' && (
                  <div className="w-8 h-8 rounded-full bg-[var(--st-text)]/20 flex items-center justify-center flex-shrink-0">
                    <Bot className="w-4 h-4 text-[var(--st-text)]" />
                  </div>
                )}
                ...
                <div 
                  className={`px-4 py-3 rounded-2xl max-w-[80%] ${
                    msg.role === 'user' 
                      ? 'bg-[var(--st-text)] text-white rounded-tr-sm' 
                      : 'bg-[var(--st-bg-muted)] rounded-tl-sm shadow-sm'
                  }`}
                >
                  {msg.content}
                </div>
        ...
        <div className="p-4 bg-[var(--st-bg-secondary)] border-t">
          <form onSubmit={handleQuery} className="max-w-4xl mx-auto flex gap-2 relative">
            <Sparkles className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--st-text-secondary)]" />
            <Input 
              className="flex-1 pl-10 pr-4 h-12 text-base rounded-full bg-[var(--st-bg-muted)]/50 border-transparent focus-visible:ring-primary focus-visible:bg-[var(--st-bg-secondary)] transition-all"
