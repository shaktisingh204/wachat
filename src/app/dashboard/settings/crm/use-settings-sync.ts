...
export function useSettingsSync<T>(
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getCrmSectionTw(key);
        if (cancelled) return;
        if (res.ok) {
          // `res.data` is the section slice itself (object/array/null).
          const slice = coerceRef.current(res.data);
          setRemote(slice);
          setPhase('ready');
        } else {
  ...
  const save = React.useCallback(
    async (slice: T): Promise<SyncOutcome> => {
      try {
        const res = await updateCrmSectionTw(key, slice);
