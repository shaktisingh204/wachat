    ...
    <div className="clay-enter flex min-h-full flex-col gap-6">
      ...
      <ClayCard padded={false} className="overflow-x-auto">
        ...
        {failedMessages.length > 0 ? (
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {failedMessages.map((m) => (
                <tr key={m._id} className="border-b border-clay-border last:border-0">
                  <td className="px-5 py-3 text-[13px] text-clay-ink font-mono">{m.recipientPhone || m.contactId || '-'}</td>
                  <td className="px-5 py-3 text-[13px] text-clay-ink">{m.type || 'text'}</td>
          ...
          <div className="px-5 py-8 text-center text-sm text-clay-ink-muted">No failed messages in the last 7 days.</div>
        )}
      </ClayCard>
      <div className="h-6" />
    </div>
