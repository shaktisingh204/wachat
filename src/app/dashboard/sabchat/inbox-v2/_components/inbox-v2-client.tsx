...
export function InboxV2Client({
    ...
    return (
        <div className="grid h-[calc(100vh-12rem)] grid-cols-12 gap-3">
            {/* Inbox + filter rail */}
            <aside className="col-span-2 flex flex-col gap-2 overflow-y-auto rounded border bg-card p-2">
                <div className="text-xs font-semibold uppercase text-muted-foreground">Inboxes</div>
                {inboxes.map((ix) => (
                    <Button
                        key={ix._id}
            ...
            {/* Conversation list */}
            <section className="col-span-3 overflow-y-auto rounded border bg-card">
            ...
            {/* Timeline + composer */}
            <main className="col-span-5 flex flex-col rounded border bg-card overflow-hidden">
                {/* Tabs Bar */}
                {activeTabs.length > 0 && (
