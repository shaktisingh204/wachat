...
import { getNativeAppAPIKeys, createNativeAppAPIKey, deleteNativeAppAPIKey } from '@/app/actions/platform/native-app-apis.actions';
import type { NativeAppAPIKey } from '@/types/platform';
import { LoaderCircle, Plus, Trash2, KeyRound } from 'lucide-react';

export default function NativeAppAPIsPage() {
  const [data, setData] = useState<NativeAppAPIKey[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    const res = await getNativeAppAPIKeys();
    setData(res);
    setLoading(false);
  };
