...
import { getCustomObjects, createCustomObject, deleteCustomObject } from '@/app/actions/platform/custom-object-builder.actions';
import type { CustomObjectDefinition } from '@/types/platform';
import { LoaderCircle, Plus, Trash2, Database } from 'lucide-react';

export default function CustomObjectBuilderPage() {
  const [data, setData] = useState<CustomObjectDefinition[]>([]);
  ...
  const filteredData = data.filter(d => d.pluralName.toLowerCase().includes(query.toLowerCase()));
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input value={form.singularName} onChange={e => setForm({ ...form, singularName: e.target.value, apiIdentifier: e.target.value.toLowerCase().replace(/\s+/g, '_') })} placeholder="e.g. Property" />
              </div>
              <div className="grid gap-2">
                <Label>Plural Name</Label>
                <Input value={form.pluralName} onChange={e => setForm({ ...form, pluralName: e.target.value })} placeholder="e.g. Properties" />
              </div>
            </div>
            <div className="grid gap-2">
              <Label>API Identifier</Label>
              <Input value={form.apiIdentifier} onChange={e => setForm({ ...form, apiIdentifier: e.target.value })} className="font-mono" />
            </div>
            <div className="grid gap-2">
              <Label>Initial Fields (comma separated)</Label>
              <Input value={form.fields} onChange={e => setForm({ ...form, fields: e.target.value })} placeholder="Address, Price, Status" />
            </div>
          </div>
          <ZoruDialogFooter>
