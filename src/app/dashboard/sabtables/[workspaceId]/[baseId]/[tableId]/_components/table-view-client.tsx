  ...
  return (
    <table className="min-w-full text-sm border-separate border-spacing-0">
      <thead className="sticky top-0 bg-muted/50 z-10">
        <tr>
          <th className="w-8 px-2 py-2 border-b border-r text-left text-muted-foreground">
            #
          </th>
          {table.fields.map((f) => (
            <th
      ...
      <tbody>
        {records.map((r, idx) => (
          <tr key={r._id} className="hover:bg-muted/30 group">
            <td className="w-8 px-2 py-1 border-b border-r text-muted-foreground text-xs">
              <button
                type="button"
  ...
  return (
    <div className="p-4 grid grid-flow-col auto-cols-[280px] gap-3 overflow-x-auto">
      {Object.entries(groups).map(([k, rs]) => (
        <div key={k} className="border rounded-md bg-muted/30 flex flex-col">
          <div className="px-3 py-2 text-sm font-medium border-b">{k}</div>
          <div className="p-2 space-y-2 flex-1 min-h-[100px]">
            {rs.map((r) => (
              <div key={r._id} className="rounded-md bg-background border p-2 text-sm">
...
function GalleryView({
  ...
  return (
    <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
      {records.map((r) => (
        <div key={r._id} className="border rounded-md p-3 bg-background">
      ...
      <div className="border rounded-md divide-y">
        {fields.map((f) => (
          <div key={f.id} className="p-3">
            ...
            <div className="text-xs text-muted-foreground">{f.fieldType}</div>
          </div>
        ))}
      </div>
    </div>
