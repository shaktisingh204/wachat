    ...
    <div className="clay-enter flex min-h-full flex-col gap-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {([{ value: 'buttons', label: 'Buttons' }, { value: 'list', label: 'List Menu' }, { value: 'product', label: 'Product' }, { value: 'location_request', label: 'Location Request' }] as const).map((t) => (
                <button key={t.value} onClick={() => setMsgType(t.value)}
                  className={`rounded-md px-3 py-1.5 text-[12px] font-medium transition-colors ${msgType === t.value ? 'bg-clay-ink text-white' : 'bg-clay-bg text-clay-ink-muted border border-clay-border hover:bg-clay-bg-2'}`}>{t.label}</button>
              ))}
            </div>
          </ClayCard>
          ...
          {msgType === 'product' && (
            <ClayCard padded={false} className="p-5"><p className="text-[13px] text-clay-ink-muted">Product messages use your connected catalog. Configure products in the Catalog section.</p></ClayCard>
          )}
          {msgType === 'location_request' && (
            <ClayCard padded={false} className="p-5"><p className="text-[13px] text-clay-ink-muted">This message will prompt the user to share their location.</p></ClayCard>
          )}
