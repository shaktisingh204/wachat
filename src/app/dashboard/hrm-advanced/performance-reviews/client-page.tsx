    ...
    <div className="flex flex-col h-[calc(100vh-100px)] p-6 max-w-[1400px] mx-auto gap-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage, evaluate, and track employee performance.</p>
        </div>
      ...
      <div className="flex-1 bg-white border rounded-lg shadow-sm overflow-hidden flex flex-col">
        <div className="grid grid-cols-[40px_1fr_1fr_80px_120px_1fr_100px] gap-4 p-4 border-b bg-gray-50 font-medium text-sm text-gray-600">
            ...
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              return (
                <div
                  ...
                  className={`absolute top-0 left-0 w-full grid grid-cols-[40px_1fr_1fr_80px_120px_1fr_100px] gap-4 p-4 items-center border-b hover:bg-gray-50 transition-colors text-sm ${isSelected ? 'bg-indigo-50' : ''}`}
                  style={{
                ...
                >
                  ...
                  <div className="font-medium text-gray-900 truncate">{review.employeeId}</div>
                  <div className="text-gray-600 truncate">{review.reviewerId}</div>
                  <div>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      review.score >= 4 ? 'bg-green-100 text-green-800' : 
                      review.score >= 3 ? 'bg-blue-100 text-blue-800' : 
                      'bg-orange-100 text-orange-800'
                    }`}>
                      {review.score.toFixed(1)}
                    </span>
                  </div>
                  <div className="text-gray-500 whitespace-nowrap">{displayDate}</div>
                  <div className="text-gray-500 truncate" title={review.comments}>{review.comments}</div>
