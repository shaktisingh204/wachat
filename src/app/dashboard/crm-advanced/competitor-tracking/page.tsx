...
export default function CompetitorTrackingPage() {
  ...
  const [editingItem, setEditingItem] = React.useState<CompetitorTrackingType | null>(null);
  const [isSubmitting, setIsSubmitting] = React.useState(false);

  const loadData = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await getCompetitorTrackings();
  ...
  const handleOpenEdit = (item: CompetitorTrackingType) => {
    setEditingItem(item);
    setIsDialogOpen(true);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteCompetitorTracking(id);
      loadData();
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingItem) {
        await updateCompetitorTracking(editingItem._id, payload);
      } else {
        await createCompetitorTracking(payload);
      }
  ...
  return (
    <>
      <EntityListShell
        ...
        subtitle="Manage your competitor tracking items."
        primaryAction={
        ...
        search={{
          value: search,
          onChange: setSearch,
          placeholder: 'Search Competitor Tracking...',
        }}
        loading={loading}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[var(--st-bg-muted)]">
                  <tr>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">competitor Name</th>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">market Share</th>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">threat Level</th>
                ...
                <td className="px-4 py-2 border-t border-[var(--st-border)] text-sm text-[var(--st-text)]">{item.competitorName}</td>
                <td className="px-4 py-2 border-t border-[var(--st-border)] text-sm text-[var(--st-text)]">{item.marketShare}</td>
                <td className="px-4 py-2 border-t border-[var(--st-border)] text-sm">
                  <Badge variant="outline">{item.threatLevel}</Badge>
                </td>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit' : 'Create'} Competitor Tracking</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
            <div>
              <Label className="capitalize">competitor Name</Label>
              <Input
                name="competitorName"
                type="text"
                defaultValue={editingItem ? editingItem.competitorName : ''}
                required
              />
            </div>
            <div>
              <Label className="capitalize">market Share</Label>
              <Input
                name="marketShare"
                type="number"
                defaultValue={editingItem ? editingItem.marketShare : ''}
                required
              />
            </div>
            <div>
              <Label className="capitalize">threat Level</Label>
              <select
                name="threatLevel"
                defaultValue={editingItem ? editingItem.threatLevel : 'low'}
