...
export default function QuoteToCashPage() {
  ...
  const [editingItem, setEditingItem] = React.useState<QuoteToCashType | null>(null);
  const [isSubmitting, setIsSubmitting] = React.useState(false);

  const loadData = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await getQuoteToCashs();
  ...
  const handleOpenEdit = (item: QuoteToCashType) => {
    setEditingItem(item);
    setIsDialogOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingItem) {
        await updateQuoteToCash(editingItem._id, payload);
      } else {
        await createQuoteToCash(payload);
      }
  ...
  return (
    <>
      <EntityListShell
        ...
        subtitle="Manage your quote-to-cash pipeline items."
        primaryAction={
        ...
        search={{
          value: search,
          onChange: setSearch,
          placeholder: 'Search Quote-to-Cash Pipeline...',
        }}
        loading={loading}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[var(--st-bg-muted)]">
                  <tr>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">quote Id</th>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">amount</th>
              <th className="px-4 py-2 text-left text-sm font-medium text-[var(--st-text-secondary)] capitalize">status</th>
                ...
                <td className="px-4 py-2 border-t border-[var(--st-border)] text-sm text-[var(--st-text)]">{item.quoteId}</td>
                <td className="px-4 py-2 border-t border-[var(--st-border)] text-sm text-[var(--st-text)]">{item.amount}</td>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit' : 'Create'} Quote-to-Cash Pipeline</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
            <div>
              <Label className="capitalize">quote Id</Label>
              <Input
                name="quoteId"
                type="text"
                defaultValue={editingItem ? editingItem.quoteId : ''}
                required
              />
            </div>
            <div>
            ...
            <div>
              ...
              <select
                name="status"
                defaultValue={editingItem ? editingItem.status : 'draft'}
