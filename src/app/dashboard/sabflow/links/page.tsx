...
export default async function LinksPage() {
  ...
  if (!session?.user) {
    return (
      <div className="p-8 text-[13px] text-[var(--st-text-secondary)]">
            ...
            return (
              ...
              >
                <Link
                  ...
                  className="font-medium text-[var(--gray-12)] hover:text-[var(--st-text)] truncate min-w-0 max-w-[45%] sm:max-w-none"
                >
                  {fromNode?.name ?? e.from}
                </Link>
                <LuArrowRight className="h-3 w-3 shrink-0 text-[var(--gray-9)]" />
                {e.isDangling ? (
                  <span className="inline-flex items-center gap-1 text-[var(--st-text)] dark:text-[var(--st-text-secondary)] min-w-0 truncate max-w-[45%] sm:max-w-none">
