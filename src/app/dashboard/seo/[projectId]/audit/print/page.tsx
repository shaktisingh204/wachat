...
export default async function PrintAuditPage({ 
    ...
    if (!audit) {
        return (
            <div className="flex items-center justify-center h-screen bg-zoru-surface-2 text-zoru-ink">
        ...
        <div className="p-8 bg-white text-black max-w-[210mm] mx-auto min-h-screen">
            ...
            <header className="border-b pb-6 mb-8 flex justify-between items-start print-break-inside-avoid">
                <div>
                    ...
                    <p className="text-zoru-ink font-medium">Project: {project?.domain || projectId}</p>
                    <p className="text-sm text-zoru-ink-muted mt-1">Date: {fmtDate(audit.createdAt || audit.startedAt || Date.now())}</p>
                </div>
                {logoUrl && (
            ...
            <section className="mb-10 print-break-inside-avoid">
                ...
                <div className="grid grid-cols-3 gap-6">
                    <div className="p-6 bg-zoru-surface-2 border rounded-lg text-center shadow-sm">
                        <div className="text-4xl font-bold" style={{ color: 'var(--brand-color)' }}>{audit.totalScore || 85}/100</div>
                        <div className="text-sm text-zoru-ink mt-2 font-medium">Health Score</div>
                    </div>
                    <div className="p-6 bg-zoru-surface-2 border rounded-lg text-center shadow-sm">
                        <div className="text-4xl font-bold text-zoru-ink">{audit.summary?.totalPages || audit.pages?.length || 0}</div>
                        <div className="text-sm text-zoru-ink mt-2 font-medium">Pages Crawled</div>
                    </div>
                    <div className="p-6 bg-zoru-surface-2 border rounded-lg text-center shadow-sm">
                        <div className="text-4xl font-bold text-zoru-ink">{audit.summary?.criticalIssues || 0}</div>
                        <div className="text-sm text-zoru-ink mt-2 font-medium">Critical Issues</div>
                    </div>
                </div>
            </section>

            <section className="print-break-inside-avoid">
                <h2 className="text-xl font-bold mb-5 border-l-4 border-zoru-line pl-3">Top Issues</h2>
                <div className="space-y-3">
                    {(!audit.pages || audit.pages.length === 0) ? (
                        <p className="text-zoru-ink italic p-4 bg-zoru-surface-2 rounded border">No pages found to show issues.</p>
                    ) : (() => {
                        ...
                        if (topIssues.length === 0) {
                            return <p className="text-zoru-ink italic p-4 bg-zoru-surface-2 rounded border">No major issues found. Great job!</p>;
                        }
                        
                        return topIssues.map((issue, i) => (
                            <div key={i} className="p-4 border rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center bg-zoru-surface-2 print-break-inside-avoid gap-2 shadow-sm">
                                <div className="max-w-[80%] break-all">
                                    <span className="font-medium text-zoru-ink block mb-1">{issue.message}</span>
                                    <a href={issue.url} target="_blank" rel="noreferrer" className="text-xs text-zoru-ink hover:underline">{issue.url}</a>
                                </div>
                                <span className="text-xs font-bold px-3 py-1 bg-zoru-surface-2 text-zoru-ink rounded-full uppercase tracking-wider shrink-0">{issue.severity || 'High'}</span>
                            </div>
                        ));
                    })()}
                </div>
            ...
            <footer className="mt-16 pt-6 border-t text-center text-sm text-zoru-ink font-medium print-break-inside-avoid">
