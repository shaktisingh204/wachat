        ...
        <div className="p-8 bg-white text-black max-w-[210mm] mx-auto min-h-screen">
            ...
            <header className="border-b pb-6 mb-8 flex justify-between items-start print-break-inside-avoid">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Project: {project?.domain || projectId}</p>
                    <p className="text-sm text-gray-400 mt-1">Date: {fmtDate(audit.createdAt || audit.startedAt || Date.now())}</p>
                </div>
                {logoUrl && (
            ...
            <section className="mb-10 print-break-inside-avoid">
                ...
                <div className="grid grid-cols-3 gap-6">
                    <div className="p-6 bg-gray-50 border rounded-lg text-center shadow-sm">
                    ...
                    <div className="p-6 bg-gray-50 border rounded-lg text-center shadow-sm">
                        <div className="text-4xl font-bold text-gray-800">{audit.summary?.totalPages || audit.pages?.length || 0}</div>
                        <div className="text-sm text-gray-600 mt-2 font-medium">Pages Crawled</div>
                    </div>
                    <div className="p-6 bg-gray-50 border rounded-lg text-center shadow-sm">
                        <div className="text-4xl font-bold text-red-600">{audit.summary?.criticalIssues || 0}</div>
            ...
            <section className="print-break-inside-avoid">
                ...
                <div className="space-y-3">
                    {(!audit.pages || audit.pages.length === 0) ? (
                        <p className="text-gray-600 italic p-4 bg-gray-50 rounded border">No pages found to show issues.</p>
                    ) : (() => {
                        ...
                        if (topIssues.length === 0) {
                            return <p className="text-gray-600 italic p-4 bg-gray-50 rounded border">No major issues found. Great job!</p>;
                        }
                        
                        return topIssues.map((issue, i) => (
                            <div key={i} className="p-4 border rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 print-break-inside-avoid gap-2 shadow-sm">
                                <div className="max-w-[80%] break-all">
                                    <span className="font-medium text-gray-800 block mb-1">{issue.message}</span>
                                    <a href={issue.url} target="_blank" rel="noreferrer" className="text-xs text-blue-500 hover:underline">{issue.url}</a>
                                </div>
                                <span className="text-xs font-bold px-3 py-1 bg-red-100 text-red-700 rounded-full uppercase tracking-wider shrink-0">{issue.severity || 'High'}</span>
                            </div>
                        ));
                    })()}
                </div>
