...
import { Card, CardBody, Textarea, cn, Progress, Badge } from '@/components/sabcrm/20ui';
            ...
            <CardBody className="p-5">
              ...
              <div className="mb-6">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs text-[var(--st-text-secondary)]">0-100 (Higher is easier)</span>
            ...
            <CardBody className="p-5 h-full">
              ...
              {keywords.length > 0 ? (
                <div className="space-y-6">
                  <div>
                    <h4 className="text-sm font-medium text-[var(--st-text-secondary)] mb-3">Top Words</h4>
                    <div className="space-y-2.5">
                      {keywords.slice(0, 5).map((kw, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <span className="text-sm font-medium truncate max-w-[120px]" title={kw.word}>{kw.word}</span>
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-[var(--st-text-secondary)] w-4 text-right">{kw.count}</span>
                  ...
                  {bigrams.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium text-[var(--st-text-secondary)] mb-3">Top 2-Word Phrases</h4>
                      <div className="space-y-2.5">
                        {bigrams.slice(0, 5).map((bg, i) => (
                          <div key={i} className="flex items-center justify-between">
                            <span className="text-sm font-medium truncate max-w-[120px]" title={bg.word}>{bg.word}</span>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-[var(--st-text-secondary)] w-4 text-right">{bg.count}</span>
                ...
                <div className="text-sm text-[var(--st-text-secondary)] text-center py-8">
