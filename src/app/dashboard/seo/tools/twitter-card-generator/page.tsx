...
import { Button, Input, Label, Textarea, Card } from '@/components/zoruui';
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <Card className="p-4 space-y-4">
            ...
            <div className="border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-black w-full max-w-sm mx-auto shadow-sm">
          ...
          <Card className="p-4 space-y-4">
            ...
            <div className="relative">
              <Textarea 
                readOnly 
                value={out} 
                className="min-h-[250px] font-mono text-xs bg-muted/50 resize-none" 
              />
            </div>
          </Card>
        </div>
  ...
  if (state.card === 'summary') {
    return (
      <div className="flex flex-row p-0">
        <div className="w-[125px] h-[125px] shrink-0 bg-muted border-r border-zinc-200 dark:border-zinc-800 overflow-hidden relative">
          {state.image ? (
        ...
        <div className="p-3 flex flex-col justify-center flex-1 min-w-0">
          ...
          <div className="text-xs text-zinc-500 dark:text-zinc-400 line-clamp-2 leading-snug mb-1">{state.description || 'Page description will appear here...'}</div>
          <div className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-1 mt-auto">
            <span className="w-3 h-3 rounded-full bg-zinc-200 dark:bg-zinc-700 flex-shrink-0"></span>
            {domain}
          </div>
        </div>
      </div>
  ...
  if (state.card === 'summary_large_image' || state.card === 'player') {
    return (
      <div className="flex flex-col p-0">
        <div className="w-full aspect-[1.91/1] bg-muted border-b border-zinc-200 dark:border-zinc-800 overflow-hidden relative">
          {state.image ? (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={state.image} alt={state.imageAlt || ''} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-muted-foreground text-xs">Hero Image</div>
          )}
          {state.card === 'player' && (
    ...
    return (
      <div className="flex flex-col p-0">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-3">
          <div className="w-16 h-16 bg-muted rounded-xl flex-shrink-0 overflow-hidden">
            {state.image ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-muted-foreground text-[10px]">App Icon</div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-zinc-500 dark:text-zinc-400 line-clamp-2 leading-snug">
        ...
        <div className="p-3 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900">
          <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Get the app</div>
