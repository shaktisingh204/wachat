    ...
    <ToolShell title="Nginx Redirect Generator" description="Generate optimized nginx rewrite or return rules.">
      <div className="space-y-4">
        <div className="space-y-3">
          {rows.map((r) => (
            <div key={r.id} className="flex flex-col xl:flex-row gap-2 items-start xl:items-center bg-muted/30 p-3 xl:p-0 xl:bg-transparent rounded-lg">
              <select 
                className="border rounded-md h-9 px-2 bg-background text-sm min-w-[110px] w-full xl:w-auto" 
                value={r.matchType} 
      ...
      <div className="mt-8 space-y-2">
        ...
        <Textarea 
          readOnly 
          value={out} 
          className="min-h-[260px] font-mono text-sm bg-muted/50 p-4" 
        />
      </div>
    </ToolShell>
  );
