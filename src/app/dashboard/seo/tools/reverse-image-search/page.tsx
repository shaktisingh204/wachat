  ...
  return (
    <ToolShell title="Reverse Image Search" description="Search for an image across multiple reverse-image engines.">
      <div className="flex flex-col gap-6">
        ...
        {preview && (
          <div className="flex flex-col gap-2">
            <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Image Preview</h3>
            <div className="rounded-md border border-neutral-200 dark:border-neutral-800 p-2 max-w-sm">
              <img 
                src={preview} 
                alt="Search preview" 
        ...
        {links.length > 0 && (
          <Card>
            <ZoruCardContent className="p-4 space-y-3">
              <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 border-b pb-2 mb-2">Search Engines</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {links.map((l) => (
                  <a 
                    key={l.name} 
                    className="flex items-center gap-2 p-3 rounded-md border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-colors" 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    href={l.href}
                  >
                    <span className="text-lg">🔗</span> 
                    <span className="text-sm font-medium text-blue-600 dark:text-blue-400">Search on {l.name}</span>
                  </a>
                ))}
              </div>
            </ZoruCardContent>
