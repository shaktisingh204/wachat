    ...
    <ToolShell title="On-Page SEO Audit (JS Rendered)" description="In-depth audit of on-page SEO factors using a headless browser to execute client-side JavaScript. Ideal for SPAs and React/Vue sites.">
      ...
      {loading && (
        <Card className="mt-4 border-dashed">
          <ZoruCardContent className="p-8 text-center text-muted-foreground flex flex-col items-center gap-4">
              ...
              {checks.map((c, i) => (
                <div key={i} className="flex items-center justify-between text-sm border-b last:border-0 py-2">
                  <div className="flex items-center gap-3">
                    {c.pass ? <Check className="h-5 w-5 text-green-600 flex-shrink-0" /> : <X className="h-5 w-5 text-red-600 flex-shrink-0" />}
                    <span className={c.pass ? 'text-foreground' : 'text-foreground font-medium'}>{c.label}</span>
                  </div>
                  {c.note && <span className="text-xs text-muted-foreground whitespace-nowrap ml-4">{c.note}</span>}
                </div>
              ))}
            </div>
          </ZoruCardContent>
