              ...
              <ZoruCardContent className="p-4 space-y-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-xs font-medium">
                    ...
                    <span className={widthStatus === 'ok' ? 'text-green-600' : widthStatus === 'too-long' ? 'text-red-600' : 'text-amber-600'}>
                  ...
                  <div className="h-2 bg-muted rounded overflow-hidden">
                    <div 
                      className={cn(
                        ...
                        widthStatus === 'ok' ? 'bg-green-500' : widthStatus === 'too-long' ? 'bg-red-500' : 'bg-amber-500'
                      )} 
                      style={{ width: `${Math.min(100, (pixelWidth / DESKTOP_MAX) * 100)}%` }} 
                    />
                  </div>
                  <p className="text-[11px] text-muted-foreground mt-1">Google typically truncates titles over ~600 pixels on desktop.</p>
                </div>

                <div className="pt-2 space-y-3 border-t">
                  <div className="flex items-start gap-2 text-sm">
                    {stuffing ? <LuAlertTriangle className="text-red-500 mt-0.5" /> : <LuCheckCircle2 className="text-green-500 mt-0.5" />}
                    <div>
                      <span className="font-medium">Keyword Stuffing: </span>
                      {stuffing ? (
                        <span className="text-red-600">Possible stuffing detected. Repeated words: {stuffedWords.join(', ')}.</span>
                      ) : (
                        <span className="text-green-600">No stuffing detected.</span>
                      )}
                    </div>
                  </div>
                  
                  <div className="flex items-start gap-2 text-sm">
                    {titleMatchesH1 || titleContainsH1 ? <LuCheckCircle2 className="text-green-500 mt-0.5" /> : <LuInfo className="text-blue-500 mt-0.5" />}
                    <div>
                      <span className="font-medium">H1 Match: </span>
                      <span className={titleMatchesH1 || titleContainsH1 ? 'text-green-600' : 'text-blue-600'}>{h1StatusText}</span>
                      {h1 && <div className="text-xs text-muted-foreground mt-1">H1: "{h1}"</div>}
                    </div>
                  </div>
                </div>
              </ZoruCardContent>
                ...
                {aiSuggestions.length > 0 ? (
                  <ul className="space-y-2">
                    {aiSuggestions.map((sug, idx) => (
                      <li key={idx} className="p-3 bg-muted/50 rounded-md text-sm flex justify-between items-center group">
