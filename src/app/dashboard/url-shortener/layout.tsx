...
import { Link2, FileText, Settings, Webhook } from 'lucide-react';

const URL_NAV = [
    { href: '/dashboard/url-shortener',          label: 'Links',       icon: Link2 },
    { href: '/dashboard/url-shortener/bio',       label: 'Link in Bio', icon: FileText },
    { href: '/dashboard/url-shortener/settings',  label: 'Settings',    icon: Settings },
];

const URL_SUBNAV: Record<string, { href: string; label: string; icon: React.ElementType }[]> = {
    '/dashboard/url-shortener/settings': [
        { href: '/dashboard/url-shortener/settings/webhooks', label: 'Webhooks', icon: Webhook },
    ],
};
...
export default function UrlShortenerLayout({ children }: { children: React.ReactNode }) {
    ...
    const activeSubnav = Object.entries(URL_SUBNAV).find(([key]) => pathname.startsWith(key))?.[1];
        ...
        <div className="relative h-full flex flex-col">
            ...
            <FeatureLock isAllowed={isAllowed}>
                <div className="flex items-center gap-1 border-b border-zoru-border px-4 pt-1 pb-0 flex-shrink-0">
                    {URL_NAV.map(({ href, label, icon: Icon }) => {
                        const isActive = pathname === href || (href !== '/dashboard/url-shortener' && pathname.startsWith(href));
                        return (
                            <Link
                                key={href}
                                href={href}
