        ...
        <Card className="flex flex-col">
            ...
            >
                <table className="w-full text-sm relative border-collapse">
                    <thead className="text-left text-muted-foreground sticky top-0 bg-zoru-bg z-10 shadow-[0_1px_0_var(--zoru-line)]">
                        <tr>
                            {columns.map((c) => (
                    ...
                    <tbody>
                        ...
                        {virtualRows.map((row, idx) => (
                            <tr key={startIndex + idx} style={{ height: rowHeight }} className="border-t border-border/50 hover:bg-muted/50">
                                {row.map((cell, ci) => (
