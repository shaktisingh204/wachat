...
export default function PettyCashEditError({
    error,
    reset,
}: {
    ...
    useEffect(() => {
        console.error('Petty Cash Edit Error:', error);
    }, [error]);
    ...
    return (
        <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-8 text-center animate-in fade-in-50">
            ...
            <div className="space-y-1">
                <h3 className="text-lg font-semibold tracking-tight">Edit Error</h3>
                <p className="text-sm text-[var(--st-text-secondary)] max-w-[400px]">
                    {error.message || 'We encountered a problem loading the petty cash edit form.'}
                </p>
            </div>
            <Button
                variant="outline"
