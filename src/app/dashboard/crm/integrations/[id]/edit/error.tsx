...
export default function EditIntegrationError({
  error,
  reset,
}: {
  ...
  React.useEffect(() => {
    console.error('Edit Integration Route Error:', error);
  }, [error]);
      ...
      <Card className="flex max-w-md flex-col items-center justify-center space-y-4 p-8 text-center shadow-sm">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[13px] text-[var(--st-text-secondary)]">
            {error.message || 'An unexpected error occurred while loading the integration form.'}
          </p>
        </div>
