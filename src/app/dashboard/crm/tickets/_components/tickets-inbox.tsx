...
export function TicketsInbox({ tickets }: TicketsInboxProps) {
    const [selectedTicketId, setSelectedTicketId] = React.useState<string | null>(tickets.length > 0 ? String(tickets[0]._id) : null);
                                ...
                                <li key={id}>
                                    <button
                                        ...
                                        className={[
                                            ...
                                            isActive ? 'bg-zoru-surface shadow-sm' : 'hover:bg-zoru-surface/50'
                                        ].join(' ')}
                                    >
                                        ...
                                        <div className="flex items-center justify-between text-[11px] text-zoru-ink-muted">
                                            <span className="truncate">
                                                {t.requesterId ? `Requester: ${t.requesterId.slice(-6)}` : 'No requester'}
                                            </span>
                                            <span>
