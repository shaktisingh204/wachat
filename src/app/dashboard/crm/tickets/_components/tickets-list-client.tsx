...
import { TicketsKpiStrip, computeTicketKpis, type TicketsKpiKey } from './tickets-kpi-strip';
import { TicketsHeaderTools, type TicketsViewMode } from './tickets-header-tools';
...
export function TicketsListClient({
    ...
    const [hasMore, setHasMore] = React.useState(initialTickets.length === TICKETS_PER_PAGE);
    const [isPending, startTransition] = React.useTransition();

    // Search + filters.
    const [search, setSearch] = React.useState('');
    const [kpiKey, setKpiKey] = React.useState<TicketsKpiKey>('all');
    const [statusFilter, setStatusFilter] = React.useState<TicketStatusFilter>('all');
    ...
    const visibleTickets = React.useMemo(() => {
        ...
        return tickets.filter((t) => {
            if (priorityFilter && String(t.priority ?? '').toLowerCase() !== priorityFilter) {
                return false;
            }
            if (channelFilter && String(t.channel ?? '').toLowerCase() !== channelFilter) {
                return false;
            }
            if (categoryFilter && t.category !== categoryFilter) return false;
            if (requesterKindFilter !== 'all' && requesterKindOf(t) !== requesterKindFilter) {
                return false;
            }
            if (dateRange?.from) {
                const created = t.createdAt
                    ...
                    : NaN;
                if (Number.isFinite(created) && created < dateRange.from.getTime()) return false;
            }
            if (dateRange?.to) {
                const created = t.createdAt
                    ? new Date(t.createdAt).getTime()
                    ...
                    ? new Date(t.audit.createdAt).getTime()
                    : NaN;
                if (Number.isFinite(created) && created > dateRange.to.getTime()) return false;
            }
            ...
            if (clientPredicate && !clientPredicate(t as unknown as Record<string, unknown>)) {
                return false;
            }
            return true;
        });
    ...
    const handleToggleAll = React.useCallback(
        (all: boolean) => {
            setSelected(
                all
                    ? new Set(visibleTickets.map((t) => String(t._id)))
                    : new Set(),
            );
        },
        [visibleTickets],
    ...
    return (
        <>
            <EntityListShell
                ...
                empty={
                    !isPending && visibleTickets.length === 0 && initialError === undefined ? (
