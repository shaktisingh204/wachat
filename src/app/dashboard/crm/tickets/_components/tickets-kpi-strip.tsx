...
export function computeTicketKpis(tickets: CrmTicketDoc[]): TicketsKpiCounts {
    ...
    for (const t of tickets) {
        ...
        const updated = t.updatedAt
            ...
            : NaN;
        if (status === 'resolved' && Number.isFinite(updated) && updated >= weekAgo) {
            resolvedThisWeek += 1;
        }
...
export function TicketsKpiStrip({ counts, active, onPick }: TicketsKpiStripProps) {
    return (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
            ...
            <KpiCard
                ...
                onClick={() => onPick(active === 'resolvedWeek' ? 'all' : 'resolvedWeek')}
            />
            <KpiCard
                label="CSAT avg"
                value={
                    counts.csatAvg != null
                        ? `${counts.csatAvg.toFixed(2)} / 5`
                        : '—'
                }
