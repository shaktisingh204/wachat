...
export function QuotationFloatingBar({ quotationId }: { quotationId: string }) {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 rounded-full border border-zoru-line bg-zoru-surface/95 px-5 py-3 shadow-[var(--zoru-shadow-lg)] backdrop-blur-sm z-50 animate-in slide-in-from-bottom-8 fade-in duration-300">
      <Button size="sm" variant="outline" asChild className="rounded-full shadow-sm">
        <Link href={`/dashboard/crm/sales/invoices/new?fromKind=quotation&fromId=${quotationId}`}>
          <Receipt className="mr-2 h-4 w-4 text-zoru-primary" />
          Convert to Invoice
        </Link>
      </Button>
