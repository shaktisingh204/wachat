...
export function InvoicePrintView({ invoice, customerLabel }: InvoicePrintViewProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6 p-6 print:p-0">
      <header className="flex items-start justify-between border-b border-zoru-line pb-4">
        <div>
        ...
        <div className="text-right text-[13px]">
          ...
          <div className="text-xl font-semibold tabular-nums text-zoru-ink">
      ...
      <section>
        ...
        <p className="text-[13px] text-zoru-ink">{customerLabel ?? invoice.clientId}</p>
      ...
      <section>
        ...
        <table className="w-full text-[12.5px]">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-3 text-center text-zoru-ink-muted">
                  No line items.
                </td>
              </tr>
            ) : (
              items.map((li, i) => (
                <tr key={i} className="border-b border-zoru-line/60">
                  ...
                  <td className="py-1.5 text-zoru-ink-muted">{li.description ?? '—'}</td>
      ...
      <section className="flex justify-end">
        <dl className="w-full max-w-sm space-y-1.5 text-[13px]">
          ...
          <div className="flex justify-between border-t border-zoru-line pt-2 font-semibold">
            <span>Total</span>
        ...
        <section>
          ...
          <div className="grid gap-2 text-[12.5px] md:grid-cols-2">
            {invoice.bankDetails?.bankName ? (
              <p>
                <span className="text-zoru-ink-muted">Bank:</span>{' '}
                {invoice.bankDetails.bankName}
              </p>
            ) : null}
            {invoice.bankDetails?.accountNo ? (
              <p>
                <span className="text-zoru-ink-muted">A/c:</span>{' '}
                {invoice.bankDetails.accountNo}
              </p>
            ) : null}
            {invoice.bankDetails?.ifsc ? (
              <p>
                <span className="text-zoru-ink-muted">IFSC:</span>{' '}
                {invoice.bankDetails.ifsc}
              </p>
            ) : null}
            {invoice.upiId ? (
              <p>
                <span className="text-zoru-ink-muted">UPI:</span> {invoice.upiId}
              </p>
            ) : null}
          </div>
        </section>
      ...
      <footer className="mt-12 flex justify-end">
        <div className="text-right text-[12.5px]">
          <div className="border-t border-zoru-line pt-2">Authorized signature</div>
        </div>
      </footer>
    </div>
  );
