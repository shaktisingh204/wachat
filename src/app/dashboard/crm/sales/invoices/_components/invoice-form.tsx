...
/**
 * <InvoiceForm> — single source of truth for both Create and Edit flows.
 *
 * Server-action driven via `saveInvoiceAction`. The form encodes every
...
/**
 * Row in client state. Mirrors `CrmInvoiceLineItem` but adds a stable
 * key so React can keyless-track rows during add/remove.
 */
interface LineItemRow extends CrmInvoiceLineItem {
  _key: string;
}
...
export function InvoiceForm({ initial, customFields }: InvoiceFormProps) {
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <ZoruLabel htmlFor="date">
              Issue date <span className="text-zoru-danger-ink">*</span>
            </ZoruLabel>
            <ZoruInput
              id="date"
              name="date"
