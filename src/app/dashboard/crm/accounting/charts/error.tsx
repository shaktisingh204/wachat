...
export default function ChartsError({ error, resetErrorBoundary }: FallbackProps) {
    return (
        <div className="flex h-[400px] w-full flex-col items-center justify-center space-y-4 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-zoru-ink/10">
            ...
            <div className="space-y-2">
                ...
                {error?.message && (
                    <p className="text-xs text-zoru-ink-muted bg-zoru-surface-2 p-2 rounded-md max-w-md mx-auto truncate">
                        {error.message}
                    </p>
                )}
            </div>
