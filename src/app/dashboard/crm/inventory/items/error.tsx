...
export default function ItemsError({
    ...
    useEffect(() => {
        console.error('Items Module Error:', error);
    }, [error]);
    ...
    return (
        <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-8 text-center animate-in fade-in-50">
            ...
            <div className="space-y-1">
                <h3 className="text-lg font-semibold tracking-tight">Items Module Error</h3>
                <p className="text-sm text-muted-foreground max-w-[400px]">
                    {error.message || 'We encountered a problem loading the items list.'}
                </p>
            </div>
            <Button
                variant="outline"
