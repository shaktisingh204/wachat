...
function ItemsErrorFallback({ error, resetErrorBoundary }: { error: Error; resetErrorBoundary: () => void }) {
  return (
    <div className="flex h-[50vh] flex-col items-center justify-center gap-4 text-center p-6 border rounded-lg border-[var(--st-danger)]/20 bg-[var(--st-danger)]/5">
      ...
      <h2 className="text-lg font-bold text-[var(--st-danger)]">Something went wrong</h2>
      ...
      <div className="mt-2 text-xs font-mono bg-[var(--st-bg-secondary)] border border-[var(--st-border)] p-2 rounded max-w-lg overflow-auto text-[var(--st-text)]">
        {error.message}
      </div>
