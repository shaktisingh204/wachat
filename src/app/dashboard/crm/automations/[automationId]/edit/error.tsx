...
export default function AutomationEditError({
    error,
    reset,
}: {
    ...
    useEffect(() => {
        console.error('Automation Edit Error:', error);
    }, [error]);
    ...
    return (
        <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-8 text-center animate-in fade-in-50">
            ...
            <div className="space-y-1">
                <h3 className="text-lg font-semibold tracking-tight">Failed to load Automation Editor</h3>
                <p className="text-sm text-zoru-ink-muted max-w-[400px]">
                    {error.message || 'We encountered a problem loading this automation for editing.'}
                </p>
            </div>
            <Button
                variant="outline"
