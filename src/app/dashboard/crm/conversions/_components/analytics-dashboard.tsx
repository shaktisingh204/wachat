...
import { Card, Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@/components/sabcrm/20ui';
    ...
    return (
        <div className="space-y-6 mt-6">
            <div className="flex items-center justify-between">
                <h2 className="text-lg font-medium text-[var(--st-text)]">Conversion Analytics</h2>
            ...
            {!data ? (
                <Card variant="default" className="flex items-center justify-center p-12 min-h-[300px]">
                    ...
                    <span className="ml-3 text-sm text-[var(--st-text-secondary)]">Aggregating complex data...</span>
                </Card>
            ) : (
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-opacity duration-300 relative ${isFetching ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <Card variant="default" className="p-5 flex flex-col">
                        <h3 className="text-md font-medium text-[var(--st-text)] mb-4">Sales Funnel</h3>
                    ...
                    <Card variant="default" className="p-5 flex flex-col">
                        <h3 className="text-md font-medium text-[var(--st-text)] mb-4">Conversion by Source</h3>
                    ...
                    <Card variant="default" className="p-5 lg:col-span-2">
                        <h3 className="text-md font-medium text-[var(--st-text)] mb-4">A/B Testing Insights</h3>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm text-[var(--st-text-secondary)]">
                                <thead className="border-b border-[var(--st-border)] text-xs uppercase bg-[var(--st-bg-muted)] text-[var(--st-text)]">
                                    <tr>
                                ...
                                <tbody>
                                    {data.abTesting.map((row: any, i: number) => (
                                        <tr key={i} className="border-b border-[var(--st-border)] hover:bg-[var(--st-bg-muted)] transition">
                                            <td className="px-4 py-3 font-medium text-[var(--st-text)]">{row.variant}</td>
                                            <td className="px-4 py-3">{row.conversionRate}%</td>
                                            <td className="px-4 py-3">
                                                {row.lift > 0 ? (
                                                    <span className="text-[var(--st-text)] font-medium">+{row.lift}%</span>
                                                ) : (
                                                    <span className="text-[var(--st-text-secondary)]">Baseline</span>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
