...
export function PreviewRunnerClient({ forms, pages }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="inline-flex border rounded-md p-1 bg-muted/30">
        <button
          type="button"
          onClick={() => setTab('forms')}
          className={`px-3 py-1.5 text-sm rounded ${
            tab === 'forms' ? 'bg-background shadow-sm' : 'text-muted-foreground'
          }`}
        >
          Forms ({forms.length})
        </button>
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded ${
            tab === 'pages' ? 'bg-background shadow-sm' : 'text-muted-foreground'
          }`}
        >
          Pages ({pages.length})
        </button>
...
function FormRuntime({ form }: { form: SabcreatorFormDoc }) {
  ...
  return (
    <Card className="p-4">
      ...
      {fields.length === 0 ? (
        <div className="text-sm text-muted-foreground">No fields defined.</div>
      ) : (
