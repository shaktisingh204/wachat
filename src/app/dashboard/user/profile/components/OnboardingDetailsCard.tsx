            ...
            <ZoruCardContent className="space-y-5">
                ...
                {(ob.startedAt || ob.completedAt) && (
                    <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm pt-2 border-t">
                        {ob.startedAt && (
                            <><span className="text-muted-foreground">Started</span><span>{formatSafeDate(ob.startedAt)}</span></>
                        )}
                        {ob.completedAt && (
                            <><span className="text-muted-foreground">Completed</span><span>{formatSafeDate(ob.completedAt)}</span></>
                        )}
                    </div>
                )}
            </ZoruCardContent>
