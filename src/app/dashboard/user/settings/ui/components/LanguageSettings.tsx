    ...
    return (
        <div className="space-y-4 pt-4 border-t border-border">
            <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Select your preferred language for the dashboard interface.</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-[200px] overflow-y-auto p-1">
                {filteredAndSortedLanguages.length > 0 ? (
                    filteredAndSortedLanguages.map(lang => (
                        <div
                            key={lang.code}
                            className={`p-3 rounded-md border cursor-pointer transition-colors ${selectedLanguage === lang.code ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted'}`}
