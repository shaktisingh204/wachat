    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {/* Sheet tabs */}
          <div className="flex items-center gap-1 border-t bg-muted/40 px-2 py-1">
            {props.sheets.map((s) => (
              <Link
                key={s._id}
            ...
            <button
              type="button"
              onClick={onAddSheet}
              className="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-background/60"
            >
              + Add sheet
            </button>
          </div>
...
function Toolbar({
  ...
  return (
    <div className="flex items-center gap-2 border-b bg-background px-3 py-2">
      <Link href="/dashboard/sabsheet" className="text-sm text-muted-foreground hover:underline">
        SabSheet
      </Link>
...
function FormulaBar({
  ...
  return (
    <div className="flex items-center gap-2 border-b bg-background px-3 py-1">
      <div className="w-16 rounded border bg-muted/40 px-2 py-1 text-center font-mono text-xs">
        {addr}
      </div>
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      <table className="w-full border-collapse text-sm">
        <thead className="sticky top-0 z-10 bg-muted/60">
          <tr>
            <th className="w-10 border bg-muted/80 px-1 text-xs text-muted-foreground" />
            {Array.from({ length: VIEW_COLS }).map((_, c) => (
              <th
                key={c}
                className="min-w-[88px] border bg-muted/80 px-2 py-1 text-xs font-medium text-muted-foreground"
              >
                {colLabel(c)}
              </th>
            ))}
        ...
        <tbody>
          {Array.from({ length: VIEW_ROWS }).map((_, row) => (
            <tr key={row}>
              <th
                scope="row"
                className="border bg-muted/60 px-1 text-center text-xs text-muted-foreground"
              >
                {row + 1}
              </th>
      ...
      <Tabs defaultValue="comments" className="flex h-full flex-col">
        ...
        <ZoruTabsContent value="comments" className="min-h-0 flex-1 space-y-3 overflow-auto p-3">
          ...
          <ul className="space-y-2">
            {comments.map((c) => (
              <li key={c._id} className="rounded-md border bg-background p-2 text-xs">
        ...
        <ZoruTabsContent value="pivots" className="min-h-0 flex-1 space-y-2 overflow-auto p-3">
          ...
          <ul className="space-y-1">
            {pivots.length === 0 ? (
              <li className="rounded-md border bg-background p-3 text-xs text-muted-foreground">
              ...
              pivots.map((p) => (
                ...
                >
                  ...
                  <div className="text-[10px] text-muted-foreground">{p.sourceRange}</div>
                </li>
              ))
            )}
          </ul>
