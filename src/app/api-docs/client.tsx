    ...
    return (
        <MarketingShell session={session}>
            <PageHero
                ...
                title={<>A clean REST API for <span className="bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 bg-clip-text text-transparent">every module.</span></>}
                subtitle="Signed, idempotent, versioned. Hit /v1 from anywhere — your CRM, your dashboards, your bots."
                extra={
                    <Link href={session?.user ? '/dashboard/api-keys' : '/login?signup=1'} className="inline-flex items-center gap-2 rounded-full bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-zinc-800">
            ...
            <SectionWrap>
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-500">Hello, SabNode</p>
                ...
                <m.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                    className="mt-10 overflow-hidden rounded-3xl border border-zinc-200">
                    <div className="flex items-center gap-1 border-b border-zinc-200 bg-white px-3 py-2">
                        {LANGS.map((l) => (
                            <button key={l} onClick={() => setLang(l)} className="relative rounded-full px-3 py-1 text-[12px] font-semibold transition">
                                {lang === l && <m.span layoutId="lang-tab" className="absolute inset-0 rounded-full bg-zinc-900" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
                                <span className={`relative z-10 ${lang === l ? 'text-white' : 'text-zinc-700'}`}>{l}</span>
                            </button>
                        ))}
                        <span className="ml-auto rounded-md bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700">200 OK</span>
                    </div>
                    <pre className="overflow-x-auto bg-zinc-950 px-6 py-5 font-mono text-[12.5px] leading-relaxed text-zinc-100">{SAMPLES[lang]}</pre>
                </m.div>
            </SectionWrap>
                    ...
                    {FEATURES.map((f, i) => {
                        ...
                        return (
                            <m.div key={f.t} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                                ...
                                <p className="mt-3 text-base font-semibold text-zinc-950">{f.t}</p>
            ...
            {/* Endpoint list */}
            <SectionWrap>
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-500">Common endpoints</p>
                <h2 className="mt-3 max-w-3xl text-balance text-4xl font-semibold tracking-tight text-zinc-950 md:text-5xl">
                ...
                <div className="mt-10 overflow-hidden rounded-2xl border border-zinc-200 bg-white">
                    {ENDPOINTS.map((e, i) => (
                        <m.div key={e.path} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                            ...
                            className="grid grid-cols-12 items-center gap-3 border-b border-zinc-100 px-5 py-3 last:border-0 hover:bg-zinc-50">
                            ...
                            <span className="col-span-5 font-mono text-[12.5px] text-zinc-900">{e.path}</span>
                            <span className="col-span-5 text-[13px] text-zinc-600">{e.desc}</span>
                        </m.div>
                    ))}
                </div>
                <p className="mt-6 text-sm text-zinc-500">All endpoints versioned under /v1 — breaking changes ship as /v2 with at least 6 months overlap.</p>
            </SectionWrap>
