...
const toneMap: Record<string, { bg: string; text: string; icon: typeof Sparkles }> = {
    amber: { bg: 'bg-zoru-surface-2', text: 'text-zoru-ink', icon: Sparkles },
    sky: { bg: 'bg-zoru-surface-2', text: 'text-zoru-ink', icon: Zap },
    emerald: { bg: 'bg-zoru-surface-2', text: 'text-zoru-ink', icon: Shield },
    rose: { bg: 'bg-zoru-surface-2', text: 'text-zoru-ink', icon: Bug },
};
    ...
    return (
        <MarketingShell session={session}>
            <PageHero
                ...
                title={<>What we shipped, <span className="bg-gradient-to-r from-zoru-ink via-zoru-ink to-zoru-ink bg-clip-text text-transparent">in plain English.</span></>}
            ...
            <SectionWrap>
                <div className="relative">
                    <div aria-hidden className="absolute left-3 top-3 bottom-3 w-px bg-zoru-surface-2 md:left-[140px]" />
                            ...
                            return (
                                ...
                                >
                                    <div className="pl-8 md:pl-0 md:pr-8 md:text-right">
                                        <p className="text-sm font-semibold text-zoru-ink">{r.date}</p>
                                        <p className="mt-0.5 font-mono text-[12px] text-zoru-ink">{r.v}</p>
                                    </div>
                                    <div className="absolute left-0 top-1 md:left-[133px]">
                                        <div className={`grid h-7 w-7 place-items-center rounded-full border-4 border-zoru-line ${tone.bg}`}>
                                            <ToneIcon className={`h-3.5 w-3.5 ${tone.text}`} />
                                        </div>
                                    </div>
                                    <div className="ml-8 rounded-2xl border border-zoru-line bg-white p-6 md:ml-0">
                                        <div className="flex items-center gap-2">
                                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${tone.bg} ${tone.text}`}>
                                                {r.tag}
                                            </span>
                                        </div>
                                        <h3 className="mt-3 text-2xl font-semibold tracking-tight text-zoru-ink">{r.title}</h3>
                                        <ul className="mt-4 space-y-2">
                                            {r.items.map((it) => (
                                                <li key={it} className="flex items-start gap-2 text-[14px] text-zoru-ink">
                                                    <span className="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-zoru-surface-2" />
                                                    {it}
                                                </li>
                                            ))}
                                        </ul>
