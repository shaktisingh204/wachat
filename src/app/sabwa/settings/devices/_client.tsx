...
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, Badge, Button, Card, CardBody, CardDescription, CardFooter, CardHeader, CardTitle, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Input, Label, Table, TBody, Td, Th, THead, Tr, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger, cn, useToast } from '@/components/sabcrm/20ui/compat';
...
export function DeviceSettingsClient() {
  ...
  const [renameTarget, setRenameTarget] = React.useState<WithId<SabwaSession> | null>(null);
  const [renameValue, setRenameValue] = React.useState('');
  const [renamePending, startRenameTransition] = React.useTransition();

  const [logoutTarget, setLogoutTarget] = React.useState<WithId<SabwaSession> | null>(null);
  ...
  const handleRename = () => {
    ...
    startRenameTransition(async () => {
      try {
        const result = await renameSession(renameTarget._id.toString(), newLabel);
        if (!result.ok) throw new Error(result.error);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <Card>
          <CardBody className="flex h-40 items-center justify-center text-[var(--st-text-secondary)]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {sessions.map((s) => (
            <Card key={s._id.toString()} className="flex flex-col p-0">
              ...
              <CardFooter className="flex justify-between gap-2 border-t border-[var(--st-border)] bg-[var(--st-bg-secondary)]/40 py-3 px-5 sm:px-6">
                <Button
                  type="button"
                  size="sm"
                  variant="ghost"
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose an internal label — it is never shown to your WhatsApp contacts.
            </DialogDescription>
          </DialogHeader>
