...
const TEMPLATES: Tmpl[] = [
    { title: 'Abandoned cart → WA + 10% off', cat: 'Campaigns', blurb: 'Trigger 1h after cart abandon, send template, expire code in 24h.', minutes: 6, accent: 'from-amber-400 to-orange-500', badge: 'Popular' },
    ...
    { title: 'Quote → e-sign → invoice', cat: 'Sales', blurb: 'Approve deal → generate quote → SabSign → auto-invoice on accept.', minutes: 12, accent: 'from-indigo-400 to-violet-500' },
    ...
    { title: 'Payment captured → ship', cat: 'Automation', blurb: 'Razorpay webhook → mark deal won → request courier label.', minutes: 6, accent: 'from-emerald-400 to-teal-500', badge: 'Popular' },
                    ...
                    {CATEGORIES.map((c) => {
                        ...
                        return (
                            <button key={c} onClick={() => setTab(c)} className="relative rounded-full px-3.5 py-1.5 text-[12px] font-medium transition">
                                {tab === c && <m.span layoutId="tmpl-tab" className="absolute inset-0 rounded-full bg-zinc-900" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
                                <span className={`relative z-10 ${tab === c ? 'text-white' : 'text-zinc-600 hover:text-zinc-900'}`}>
                                    {c} <span className={`ml-1.5 text-[10px] ${tab === c ? 'text-white/60' : 'text-zinc-400'}`}>{count}</span>
                                </span>
                            </button>
                        );
                    })}
                ...
                <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {visible.map((t, i) => (
                        <m.div
                            ...
                            className="group rounded-2xl border border-zinc-200 bg-white p-5 transition hover:-translate-y-1 hover:border-zinc-900 hover:shadow-[0_24px_60px_-30px_rgba(0,0,0,0.18)]"
                        >
                            ...
                            <h3 className="mt-4 text-lg font-semibold tracking-tight text-zinc-950">{t.title}</h3>
                            <p className="mt-1 text-[13px] leading-relaxed text-zinc-600">{t.blurb}</p>
                            <div className="mt-4 flex items-center justify-between">
                                <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-[11px] font-semibold text-zinc-700">{t.cat}</span>
                                <span className="text-[11px] text-zinc-500">~{t.minutes} min to clone</span>
                            </div>
                            <button className="mt-4 inline-flex w-full items-center justify-center gap-1.5 rounded-full bg-zinc-900 px-3 py-1.5 text-[12px] font-semibold text-white transition hover:bg-zinc-800">
