...
export default function SabChatWidgetPage() {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[9999] flex flex-col items-end justify-end p-4 sm:p-6 font-sans">
      {/* Widget Container */}
      <div 
        className={`pointer-events-auto flex flex-col overflow-hidden shadow-2xl transition-all duration-300 ease-in-out transform origin-bottom-right rounded-2xl border border-zoru-line bg-zoru-surface
          ${isOpen ? 'scale-100 opacity-100 mb-4 h-[600px] w-full max-w-[400px]' : 'scale-0 opacity-0 h-0 w-0'}`}
      >
        {/* Header */}
        <div className="bg-zoru-brand flex items-center justify-between px-4 py-3 text-white">
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-zoru-surface-2">
          
          {/* 1. PRE-CHAT FORM */}
          {view === 'pre-chat' && (
            <div className="flex-1 overflow-y-auto p-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="mb-6 space-y-2">
                <h4 className="font-semibold text-zoru-ink text-lg">Welcome to SabChat! 👋</h4>
                <p className="text-sm text-zoru-ink-muted">Please introduce yourself before we start the conversation.</p>
              </div>
              ...
              <div className="mt-8 border-t border-zoru-line pt-6">
                <p className="text-xs text-center text-zoru-ink-muted mb-4">Or try looking for an answer yourself</p>
          ...
          {view === 'chat' && (
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-left-2 duration-300">
              {/* Action Bar */}
              <div className="flex items-center justify-around border-b border-zoru-line bg-zoru-surface py-2 px-1">
                <Button variant="ghost" size="sm" onClick={() => handleStartCall('voice-call')} className="text-xs flex-col h-auto py-2 gap-1 text-zoru-ink-muted hover:text-zoru-brand">
                  <PhoneCall className="w-4 h-4" />
                  Voice
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleStartCall('video-call')} className="text-xs flex-col h-auto py-2 gap-1 text-zoru-ink-muted hover:text-zoru-brand">
                  <Video className="w-4 h-4" />
                  Video
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleStartCall('co-browse')} className="text-xs flex-col h-auto py-2 gap-1 text-zoru-ink-muted hover:text-zoru-brand">
                  <MonitorUp className="w-4 h-4" />
                  Co-browse
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setView('help')} className="text-xs flex-col h-auto py-2 gap-1 text-zoru-ink-muted hover:text-zoru-brand">
              ...
              <ScrollArea className="flex-1 p-4 flex flex-col gap-4">
                <div className="space-y-4">
                  <div className="text-center text-xs text-zoru-ink-muted my-2">Today</div>
                  {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm shadow-sm
                        ${msg.sender === 'user' 
                          ? 'bg-zoru-brand text-white rounded-tr-sm' 
                          : 'bg-zoru-surface border border-zoru-line text-zoru-ink rounded-tl-sm'}`}
                      >
                        {msg.text}
                        <div className={`text-[10px] mt-1 text-right ${msg.sender === 'user' ? 'text-white/70' : 'text-zoru-ink-muted'}`}>
                          {msg.time}
                        </div>
                      </div>
                    </div>
              ...
              {/* Input Area */}
              <div className="p-3 bg-zoru-surface border-t border-zoru-line">
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 relative">
                  <Input 
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    placeholder="Type a message..." 
                    className="pr-10 rounded-full bg-zoru-surface-2 border-zoru-line"
                  />
                  <Button 
                    type="submit" 
                    size="icon" 
          ...
          {view === 'help' && (
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-2 duration-300">
              <div className="p-4 border-b border-zoru-line bg-zoru-surface">
                <div className="relative">
                  <Search className="absolute left-3 top-2.5 h-4 w-4 text-zoru-ink-muted" />
                  <Input 
                    placeholder="Search articles..." 
                    className="pl-9 bg-zoru-surface-2"
                    value={searchQuery}
                    onChange={(e) => {
              ...
              <ScrollArea className="flex-1 p-2">
                {isSearching ? (
                  <div className="flex items-center justify-center h-32">
                    <Loader2 className="w-6 h-6 animate-spin text-zoru-brand" />
                  </div>
                ) : (
                  <div className="space-y-2">
                    {[
                      ...
                      <button key={i} className="w-full text-left p-3 hover:bg-zoru-surface rounded-xl transition-colors border border-transparent hover:border-zoru-line group">
                        <Badge variant="secondary" className="mb-1">{article.category}</Badge>
                        <h4 className="text-sm font-medium text-zoru-ink group-hover:text-zoru-brand transition-colors">{article.title}</h4>
                      </button>
                    ))}
                    {searchQuery && !['How to setup your account', 'Billing and Invoicing FAQs', 'Integrating with WhatsApp', 'Managing team permissions'].some(a => a.toLowerCase().includes(searchQuery.toLowerCase())) && (
                      <div className="text-center py-8 text-zoru-ink-muted text-sm">
              ...
              <div className="p-4 bg-zoru-surface border-t border-zoru-line text-center">
                <p className="text-xs text-zoru-ink-muted mb-2">Still need help?</p>
      ...
      <button
        ...
        className="pointer-events-auto mt-4 flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-zoru-brand text-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-transform hover:scale-105 active:scale-95 z-50"
