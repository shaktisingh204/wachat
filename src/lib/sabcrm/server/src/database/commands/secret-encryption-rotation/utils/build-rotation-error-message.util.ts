...
export const buildRotationErrorMessage = (
  ...
  if (
    ...
    return (
      `[${siteName}] row ${rowId}: ${error.message} ` +
      "The row is encrypted with a key that is neither ENCRYPTION_KEY nor FALLBACK_ENCRYPTION_KEY — " +
      "set FALLBACK_ENCRYPTION_KEY to the key that produced this envelope (e.g. after a partial earlier rotation)."
    );
  }
