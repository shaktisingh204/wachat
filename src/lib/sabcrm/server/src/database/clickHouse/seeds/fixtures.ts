import { formatDateTimeForClickHouse } from '@/lib/sabcrm/server/src/database/clickHouse/clickHouse.util';

// PORT-NOTE: Workspace/object event constants and seed workspace IDs are
// referenced here. Since those modules are not yet ported, we inline the
// string literal event names and workspace IDs to keep this file self-contained.

const OBJECT_RECORD_CREATED_EVENT = 'objectRecord.created';
const OBJECT_RECORD_DELETED_EVENT = 'objectRecord.deleted';
const OBJECT_RECORD_UPDATED_EVENT = 'objectRecord.updated';
const CUSTOM_DOMAIN_ACTIVATED_EVENT = 'customDomain.activated';
const CUSTOM_DOMAIN_DEACTIVATED_EVENT = 'customDomain.deactivated';

// Apple seed workspace
const SEED_APPLE_WORKSPACE_ID = '20202020-1c25-4d02-bf25-6aeccf7ea419';

// User-workspace seed IDs (Twenty dev-seeder constants)
const USER_WORKSPACE_DATA_SEED_IDS = {
  TIM: '20202020-0687-4c41-b707-ed1bfca972a7',
  JANE: '20202020-3957-45c9-be39-337dc4d9100a',
  JONY: '20202020-7169-42cf-bc47-1cfef15957b8',
  PHIL: '20202020-7169-42cf-bc47-1cfef15957b9',
} as const;

export type GenericTrackEvent = {
  type: 'track';
  event: string;
  timestamp: string;
  version: string;
...
export const workspaceEventFixtures: GenericTrackEvent[] = [
  {
    type: 'track',
...
export const objectEventFixtures: ObjectEventFixture[] = [
  {
    type: 'track',
...
const buildUsageEventFixtures = (): UsageEventFixture[] => {
  ...
  // Pseudo-random using a fixed seed for reproducibility across runs.
  let rngState = 42;
    ...
    for (let userIdx = 0; userIdx < users.length; userIdx++) {
      ...
      for (const op of operations) {
        ...
        const eventsCount = Math.max(
          1,
          Math.round(
            (1 + nextRandom() * 4) * dayMultiplier * recencyMultiplier * userWeight,
          ),
        );

        for (let eventIdx = 0; eventIdx < eventsCount; eventIdx++) {
          const hour = Math.floor(9 + nextRandom() * 9);
...
export const usageEventFixtures: UsageEventFixture[] = buildUsageEventFixtures();
