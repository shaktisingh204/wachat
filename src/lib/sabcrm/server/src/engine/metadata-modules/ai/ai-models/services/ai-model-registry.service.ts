...
// PORT-NOTE: NestJS @Injectable / Logger removed; plain class with constructor
// injection. Logger replaced with console.warn. All business logic preserved.
// This replaces the earlier stub from a prior batch.

import { type LanguageModel } from 'ai';
import { type AiSdkPackage } from '@/lib/sabcrm/shared/src/ai/constants/ai-sdk-packages.const';

import { AUTO_SELECT_FAST_MODEL_ID } from '@/lib/sabcrm/shared/src/constants/AutoSelectFastModelId';
import { AUTO_SELECT_SMART_MODEL_ID } from '@/lib/sabcrm/shared/src/constants/AutoSelectSmartModelId';
import { isAutoSelectModelId } from '@/lib/sabcrm/shared/src/utils/isAutoSelectModelId';

import { ConfigVariablesGroup } from '@/lib/sabcrm/server/src/engine/core-modules/twenty-config/enums/config-variables-group.enum';
import { type ConfigGroupHashService } from '@/lib/sabcrm/server/src/engine/core-modules/twenty-config/services/config-group-hash.service';
...
import { type AiModelPreferencesService } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/services/ai-model-preferences.service';
import { type ProviderConfigService } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/services/provider-config.service';
...
import { DEFAULT_CONTEXT_WINDOW_TOKENS } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/types/default-context-window-tokens.const';
import { DEFAULT_MAX_OUTPUT_TOKENS } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/types/default-max-output-tokens.const';
import { buildCompositeModelId } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/utils/composite-model-id.util';
import { inferModelFamily } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/utils/infer-model-family.util';
import { isProviderConfigured } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/utils/is-provider-configured.util';
import {
  isModelAllowedByWorkspace,
  type WorkspaceModelAvailabilitySettings,
} from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/utils/is-model-allowed.util';
...
export class AiModelRegistryService {
  ...
  private registerModelsFromProviders(providers: AiProvidersConfig): void {
    for (const [providerKey, config] of Object.entries(providers)) {
      if (!config.npm) {
        console.warn(
          `AiModelRegistryService: Skipping provider "${providerKey}": missing npm field`,
        );
        continue;
      }
