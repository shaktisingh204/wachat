...
// PORT-NOTE: NestJS @Injectable / Logger removed; plain class. Logger replaced
// with console.warn. inferAiSdkPackage imported from ported shared utility.

import { inferAiSdkPackage } from '@/lib/sabcrm/shared/src/ai/utils/infer-ai-sdk-package.util';

import { MODELS_DEV_API_URL } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/constants/models-dev.const';
import { type ModelsDevData } from '@/lib/sabcrm/server/src/engine/metadata-modules/ai/ai-models/types/models-dev-data.type';
...
export class ModelsDevCatalogService {
  ...
  private async getCachedData(): Promise<ModelsDevData | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `ModelsDevCatalogService: models.dev API returned ${response.status}, using cached data`,
        );
      ...
      console.warn(
        `ModelsDevCatalogService: Failed to fetch models.dev: ${error instanceof Error ? error.message : String(error)}`,
      );
