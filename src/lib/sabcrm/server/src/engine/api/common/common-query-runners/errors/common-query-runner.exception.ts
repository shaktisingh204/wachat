...
// PORT-NOTE: Ported from Twenty's CommonQueryRunnerException (NestJS custom exception).
// Preserves all exception codes and structure; no NestJS dependency in this port.
// The MessageDescriptor type from @lingui/core is replaced with a plain string or unknown.
...
export class CommonQueryRunnerException extends Error {
  code: CommonQueryRunnerExceptionCode;
  meta?: { userFriendlyMessage?: unknown };
  ...
  constructor(
    ...
    super(message);
    this.name = 'CommonQueryRunnerException';
    this.code = code;
    this.meta = meta;
  }
}
