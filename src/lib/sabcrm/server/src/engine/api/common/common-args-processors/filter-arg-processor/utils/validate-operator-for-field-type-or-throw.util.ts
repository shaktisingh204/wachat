...
import { getOperatorsForFieldType } from 'src/lib/sabcrm/server/src/engine/api/common/common-args-processors/filter-arg-processor/utils/get-operators-for-field-type.util';
import {
...
export const validateOperatorForFieldTypeOrThrow = (
  ...
  if (!allowedOperators.includes(operator)) {
    ...
    throw new CommonQueryRunnerException(
      ...
      {
        userFriendlyMessage: `Invalid filter : Operator "${operator}" is not valid for this "${fieldName}" ${fieldType} field`,
      },
    );
  }
};
