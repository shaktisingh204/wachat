...
import { type ToolRetrievalOptions } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/interfaces/tool-retrieval-options.type";

import { compactToolOutput } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/output-transforms/compact-tool-output.util";
import { type ToolExecutorService } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/services/tool-executor.service";
import { type LearnToolsAspect } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/tools/learn-tools.tool";
import { type ToolContext } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/types/tool-context.type";
...
import { wrapWithErrorHandler } from "@/lib/sabcrm/server/src/engine/core-modules/tool-provider/utils/tool-error.util";
...
} from "@/lib/sabcrm/server/src/engine/core-modules/tool/utils/wrap-tool-for-execution.util";
import { type RolePermissionConfig } from "@/lib/sabcrm/server/src/engine/twenty-orm/types/role-permission-config";

// PORT-NOTE: NestJS @Injectable() / DI removed; construct via createToolRegistryService().
...
export class ToolRegistryService {
  ...
  async resolveAndExecute(
    ...
    try {
      ...
      console.error(`[ToolRegistryService] Error executing tool "${toolName}": ${errorMessage}`);
  ...
  async getToolsByCategories(
    ...
    console.log(
      `[ToolRegistryService] Generated ${Object.keys(toolSet).length} tools for categories: [${categories?.join(", ") ?? "all"}]`,
    );
...
export function createToolRegistryService(
  ...
  return new ToolRegistryService(providers, toolExecutorService);
}
