// PORT-NOTE: The original imported from @sniptt/guards (isNumber, isObject, isString) and
// microdiff (Difference). Both are unavailable in SabNode's node_modules, so:
//   - Difference type is inlined below (matches microdiff's public API).
//   - Guard helpers are replaced with plain TS typeof checks.
// isDefined is sourced from the local SabNode shared util.

import { isDefined } from '@/lib/sabcrm/shared/src/utils/validation/isDefined';

// Inlined from microdiff — covers CREATE, CHANGE, and REMOVE difference shapes.
type DifferenceCreate = { type: 'CREATE'; path: (string | number)[]; value: unknown };
type DifferenceChange = { type: 'CHANGE'; path: (string | number)[]; value: unknown; oldValue: unknown };
type DifferenceRemove = { type: 'REMOVE'; path: (string | number)[]; oldValue: unknown };
export type Difference = DifferenceCreate | DifferenceChange | DifferenceRemove;
...
export const applyDiff = <T>(obj: T, diffs: Difference[]): T => {
  ...
  const mutableObj = deepClone(obj as unknown as MutableData) as T;
  ...
  for (const diff of diffs) {
    ...
    try {
      applyDiffToPath(mutableObj as unknown as MutableData, diff, arrayDeletionQueue);
    } catch (error) {
      throw new Error(
...
const applyDiffToPath = (
  ...
  switch (type) {
    ...
    default:
      throw new Error(`Unsupported diff type: ${(diff as Difference).type}`);
  }
};
...
const navigateToParent = (
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (typeof pathElement === 'number' && !Array.isArray(current)) {
      throw new Error(
        `Expected array at path element ${i}, got ${typeof current}`,
      );
    }

    if (typeof pathElement === 'string' && Array.isArray(current)) {
...
const setValueAtPath = (
  ...
  if (Array.isArray(container)) {
    ...
    try {
      (container as ObjectType)[pathElement as string] = value;
    } catch (error) {
      throw new Error(
...
const handleObjectRemoval = (
  ...
  delete parentObject[key as string];
};
...
const filterRemoveSymbols = (array: ArrayType): void => {
  ...
  for (let i = indicesToRemove.length - 1; i >= 0; i--) {
    array.splice(indicesToRemove[i] as number, 1);
  }
};
