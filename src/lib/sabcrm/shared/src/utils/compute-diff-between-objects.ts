...
import { fastDeepEqual } from '@/lib/sabcrm/shared/src/utils/json/fast-deep-equal';
...
const extractProperties = <T extends { id: string }>(
  ...
  return properties.reduce<Partial<T>>((acc, property) => {
    return {
      ...acc,
...
/**
 * Computes a diff between two lists of objects with `id` fields.
 * Returns categorized lists: items to create, update, restore+update, and ids to remove.
 */
