// PORT-NOTE: This is a Vite plugin for Linaria/WYW CSS pre-build profiling.
// SabNode uses Next.js (not Vite), so this plugin is not invoked at runtime.
// It is preserved here for completeness in case tooling re-uses twenty-shared utilities.
  ...
  return {
    ...
    configResolved(config: { command: string; root: string }) {
      ...
      if (typeof wywPlugin.configResolved === 'function') {
        (wywPlugin.configResolved as (config: { command: string; root: string }) => void).call(this, config);
      }
    },
    async buildStart() {
      ...
      const warmupWarning = warmupMs > warmupThresholdMs ? ' slow' : '';
      console.log(
