/**
 * Forge block: LM Chat Groq
 *
 * Source: n8n-master/packages/@n8n/nodes-langchain/nodes/llms/LmChatGroq/LmChatGroq.node.ts
 *
...
async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  ...
  if (!prompt) throw new Error('Groq: prompt is required');
  ...
  return {
    ...
    logs: [`Groq chat → ${model}`],
  };
}

const block: ForgeBlock = {
  id: 'forge_lm_chat_groq',
  name: 'LM Chat Groq',
  description: 'Send a chat completion request to Groq.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      fields: [
        ...
        { id: 'model', label: 'Model', type: 'text', placeholder: 'llama-3.1-70b-versatile' },
