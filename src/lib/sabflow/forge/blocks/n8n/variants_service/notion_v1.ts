/**
 * Forge block: Notion V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Notion/v1/NotionV1.node.ts
 *
 * Compat-shim — preserves the `forge_notion_v1` id so flow files that pinned
 * the Notion v1 node keep deserialising. Delegates to the modern
 * `forge_notion` block.
 */
  ...
  actions: [
    {
      ...
      fields: [
        {
          ...
          type: 'text',
          required: true,
          helperText: 'Pick action id from forge_notion — e.g. create_page.',
        },
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`Notion v1: unknown action "${targetId}"`);
