/**
 * Forge block: Discord V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Discord/v1/DiscordV1.node.ts
 *
 * Compat-shim — preserves the `forge_discord_v1` id so flow files that pinned
 * the Discord v1 node keep deserialising. Delegates to the modern
 * `forge_discord` block.
 */
  ...
  actions: [
    {
      ...
      fields: [
        {
          ...
          type: 'text',
          required: true,
          helperText: 'Pick action id from forge_discord — e.g. send_message.',
        },
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`Discord v1: unknown action "${targetId}"`);
