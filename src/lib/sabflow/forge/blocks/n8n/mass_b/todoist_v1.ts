/**
 * Forge block: Todoist V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Todoist/v1/TodoistV1.node.ts
 *
 * Compat-shim — preserves the `forge_todoist_v1` id so flow files that pinned
 * the Todoist v1 node keep deserialising. Delegates to the modern
 * `forge_todoist` block.
 */
...
const block: ForgeBlock = {
  id: 'forge_todoist_v1',
  name: 'Todoist (v1)',
  description: 'Legacy n8n Todoist v1 shape. Delegates to forge_todoist.',
  ...
  actions: [
    {
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`Todoist v1: unknown action "${targetId}"`);
