/**
 * Forge block: Syncro MSP V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/SyncroMsp/v1/SyncroMspV1.node.ts
 *
 * Compat-shim — preserves the `forge_syncromsp_v1` id so flow files that pinned
 * the Syncro MSP v1 node keep deserialising. Delegates to the modern
 * `forge_syncro_msp` block.
 */
  ...
  actions: [
    {
      id: 'legacy_action',
      label: 'Legacy action',
      description: 'Forwards to modern Syncro MSP block. Action ids preserved.',
      fields: [
        {
          id: 'targetActionId',
          label: 'Action',
          type: 'text',
          required: true,
          helperText: 'Pick action id from forge_syncro_msp.',
        },
        { id: 'inputs', label: 'Inputs (JSON)', type: 'json' },
      ],
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        const modern = getForgeBlock('forge_syncro_msp');
        if (!modern) throw new Error('forge_syncro_msp not registered');
        const targetId = String(ctx.options.targetActionId ?? '');
        const target = modern.actions?.find((a) => a.id === targetId);
        if (!target) throw new Error(`Syncro MSP v1: unknown action "${targetId}"`);
