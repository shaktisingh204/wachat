/**
 * Forge block: Pipedrive V2 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Pipedrive/v2/PipedriveV2.node.ts
 *
 * Compat-shim — preserves the `forge_pipedrive_v2` id so flow files that pinned
 * the Pipedrive v2 node keep deserialising. Delegates to the modern
...
const block: ForgeBlock = {
  id: 'forge_pipedrive_v2',
  name: 'Pipedrive (v2)',
  description: 'Legacy n8n Pipedrive v2 shape. Delegates to forge_pipedrive.',
  ...
  actions: [
    {
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`Pipedrive v2: unknown action "${targetId}"`);
