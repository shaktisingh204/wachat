/**
 * Forge block: Airtable V2 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Airtable/v2/AirtableV2.node.ts
 *
 * Compat-shim — preserves the `forge_airtable_v2` id so flow files that pinned
 * the Airtable v2 node keep deserialising. Delegates to the modern
  ...
  actions: [
    {
      ...
      fields: [
        {
          ...
          type: 'text',
          required: true,
          helperText: 'Pick action id from forge_airtable.',
        },
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`Airtable v2: unknown action "${targetId}"`);
