/**
 * Forge block: HubSpot V2 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/Hubspot/V2/HubspotV2.node.ts
 *
 * Compat-shim — preserves the `forge_hubspot_v2` id so flow files that pinned
 * the HubSpot v2 node keep deserialising. Delegates to the modern
 * `forge_hubspot` block.
 */
...
const block: ForgeBlock = {
  id: 'forge_hubspot_v2',
  name: 'HubSpot (v2)',
  description: 'Legacy n8n HubSpot v2 shape. Delegates to forge_hubspot.',
  iconName: 'LuOrbit',
  ...
  actions: [
    {
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`HubSpot v2: unknown action "${targetId}"`);
