/**
 * Forge block: MailerLite V2 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/MailerLite/v2/MailerLiteV2.node.ts
 *
 * Compat-shim — preserves the `forge_mailerlite_v2` id so flow files that pinned
 * the MailerLite v2 node keep deserialising. Delegates to the modern
 * `forge_mailerlite` block.
 */
  ...
  actions: [
    {
      id: 'legacy_action',
      label: 'Legacy action',
      description: 'Forwards to modern MailerLite block. Action ids preserved.',
      fields: [
        {
          id: 'targetActionId',
          label: 'Action',
          type: 'text',
          required: true,
          helperText: 'Pick action id from forge_mailerlite.',
        },
        { id: 'inputs', label: 'Inputs (JSON)', type: 'json' },
      ],
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        const modern = getForgeBlock('forge_mailerlite');
        if (!modern) throw new Error('forge_mailerlite not registered');
        const targetId = String(ctx.options.targetActionId ?? '');
        const target = modern.actions?.find((a) => a.id === targetId);
        if (!target) throw new Error(`MailerLite v2: unknown action "${targetId}"`);
