/**
 * Forge block: SeaTable V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/SeaTable/v1/SeaTableV1.node.ts
 *
 * Compat-shim — preserves the `forge_seatable_v1` id so flow files that pinned
 * the SeaTable v1 node keep deserialising. Delegates to the modern
  ...
  actions: [
    {
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target) throw new Error(`SeaTable v1: unknown action "${targetId}"`);
