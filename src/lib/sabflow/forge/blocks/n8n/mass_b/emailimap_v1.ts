/**
 * Forge block: Email Read IMAP V1 (legacy n8n shape)
 *
 * Source: n8n-master/packages/nodes-base/nodes/EmailReadImap/v1/EmailReadImapV1.node.ts
 *
 * Compat-shim — preserves the `forge_emailimap_v1` id so flow files that pinned
 * the Email Read IMAP v1 node keep deserialising. Delegates to the modern
...
const block: ForgeBlock = {
  id: 'forge_emailimap_v1',
  name: 'Email Read IMAP (v1)',
  description:
    'Legacy n8n Email Read IMAP v1 shape. Delegates to forge_email_read_imap.',
  iconName: 'LuMail',
  ...
  actions: [
    {
      ...
      run: async (ctx: ForgeActionContext): Promise<ForgeActionResult> => {
        ...
        if (!target)
          throw new Error(`Email Read IMAP v1: unknown action "${targetId}"`);
