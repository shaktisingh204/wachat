...
async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  ...
  const text = body?.message?.content?.map((c) => c.text ?? '').join('') ?? '';
  return {
...
const block: ForgeBlock = {
  id: 'forge_lm_chat_cohere',
  name: 'LM Chat Cohere',
  description: 'Chat completion via Cohere v2 chat API.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against Cohere /v2/chat.',
      fields: [
