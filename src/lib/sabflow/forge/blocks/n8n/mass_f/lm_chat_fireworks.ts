/**
 * Forge block: LM Chat Fireworks AI
 *
 * OpenAI-compatible chat completions against Fireworks AI
...
const API = 'https://api.fireworks.ai/inference/v1/chat/completions';

async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  const apiKey = asString(ctx.options.apiKey);
  if (!apiKey) throw new Error('Fireworks: apiKey is required');
  const prompt = asString(ctx.options.prompt);
  if (!prompt) throw new Error('Fireworks: prompt is required');
  const model = asString(ctx.options.model) || 'accounts/fireworks/models/llama-v3p1-8b-instruct';
  ...
  return {
    ...
    logs: [`Fireworks chat → ${model}`],
  };
}

const block: ForgeBlock = {
  id: 'forge_lm_chat_fireworks',
  name: 'LM Chat Fireworks',
  description: 'Send a chat completion request to Fireworks AI.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against Fireworks AI chat completions API.',
      fields: [
        { id: 'apiKey', label: 'API key', type: 'password', required: true },
        { id: 'model', label: 'Model', type: 'text', placeholder: 'accounts/fireworks/models/llama-v3p1-8b-instruct' },
