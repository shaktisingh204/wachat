/**
 * Forge block: LM Chat OctoAI
 *
 * OpenAI-compatible chat completions against OctoAI
...
const API = 'https://text.octoai.run/v1/chat/completions';

async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  const apiKey = asString(ctx.options.apiKey);
  if (!apiKey) throw new Error('OctoAI: apiKey is required');
  const prompt = asString(ctx.options.prompt);
  if (!prompt) throw new Error('OctoAI: prompt is required');
  const model = asString(ctx.options.model) || 'meta-llama-3.1-8b-instruct';
  ...
  return {
    ...
    logs: [`OctoAI chat → ${model}`],
  };
}

const block: ForgeBlock = {
  id: 'forge_lm_chat_octoai',
  name: 'LM Chat OctoAI',
  description: 'Send a chat completion request to OctoAI.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against OctoAI chat completions API.',
      fields: [
        { id: 'apiKey', label: 'API key', type: 'password', required: true },
        { id: 'model', label: 'Model', type: 'text', placeholder: 'meta-llama-3.1-8b-instruct' },
