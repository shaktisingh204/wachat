/**
 * Forge block: LM Chat Cerebras
 *
 * OpenAI-compatible chat completions against Cerebras inference
...
async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  ...
  if (!apiKey) throw new Error('Cerebras: apiKey is required');
  const prompt = asString(ctx.options.prompt);
  if (!prompt) throw new Error('Cerebras: prompt is required');
  const model = asString(ctx.options.model) || 'llama3.1-8b';
  ...
  return {
    ...
    logs: [`Cerebras chat → ${model}`],
  };
}

const block: ForgeBlock = {
  id: 'forge_lm_chat_cerebras',
  name: 'LM Chat Cerebras',
  description: 'Send a chat completion request to Cerebras inference.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against Cerebras chat completions API.',
      fields: [
        { id: 'apiKey', label: 'API key', type: 'password', required: true },
        { id: 'model', label: 'Model', type: 'text', placeholder: 'llama3.1-8b' },
