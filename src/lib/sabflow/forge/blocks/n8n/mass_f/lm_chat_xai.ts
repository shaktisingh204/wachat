/**
 * Forge block: LM Chat xAI (Grok)
 *
 * OpenAI-compatible chat completions against xAI
...
async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  ...
  if (!apiKey) throw new Error('xAI: apiKey is required');
  const prompt = asString(ctx.options.prompt);
  if (!prompt) throw new Error('xAI: prompt is required');
  const model = asString(ctx.options.model) || 'grok-2-latest';
...
const block: ForgeBlock = {
  id: 'forge_lm_chat_xai',
  name: 'LM Chat xAI (Grok)',
  description: 'Send a chat completion request to xAI Grok.',
  iconName: 'LuBrain',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against xAI Grok chat completions API.',
      fields: [
        { id: 'apiKey', label: 'API key', type: 'password', required: true },
        { id: 'model', label: 'Model', type: 'text', placeholder: 'grok-2-latest' },
