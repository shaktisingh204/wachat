/**
 * Forge block: LM Chat DeepSeek
 *
 * OpenAI-compatible chat completions against DeepSeek's API
...
async function chat(ctx: ForgeActionContext): Promise<ForgeActionResult> {
  ...
  const model = asString(ctx.options.model) || 'deepseek-chat';
  ...
  return {
    ...
    logs: [`DeepSeek chat → ${model}`],
  };
}

const block: ForgeBlock = {
  id: 'forge_lm_chat_deepseek',
  name: 'LM Chat DeepSeek',
  ...
  actions: [
    {
      ...
      description: 'Single-turn chat against DeepSeek chat completions API.',
      fields: [
        { id: 'apiKey', label: 'API key', type: 'password', required: true },
        { id: 'model', label: 'Model', type: 'text', placeholder: 'deepseek-chat' },
