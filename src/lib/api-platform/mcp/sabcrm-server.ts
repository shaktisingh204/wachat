/**
 * SabNode MCP — method dispatcher for the SabCRM server.
 *
 ...
 * HTTP framing live in the Route Handler (`/api/mcp/sabcrm`); this module
 * is pure protocol logic over the {@link SABCRM_TOOLS} registry and is
 * unit-testable without a request. Mirrors `./server.ts` (the Ad Manager
...
import { SABCRM_TOOLS, SABCRM_TOOL_MAP, type McpTool } from './sabcrm-tools';

export const SABCRM_MCP_SERVER_INFO = {
  name: 'sabnode-sabcrm',
  title: 'SabNode SabCRM',
  version: '1.0.0',
} as const;

const SERVER_INSTRUCTIONS =
  'Tools for working with this SabNode workspace\'s SabCRM: object metadata, records (list/get/create/' +
  'update/trash), cross-object search, pipelines, gated stage moves, and record activity timelines. ' +
  'Every tool requires a `projectId` — if you do not know it, ask the user for their SabCRM project id. ' +
  'Start with list_objects to discover object slugs and field keys. Stage moves are governed by entry ' +
  'gates: when move_record_stage returns `moved: false`, read `gate` — fill the missing fields with ' +
  'update_record, or tell the user an approval must be decided in the dashboard.';

/** Tools the calling key is allowed to see/use, given its granted scopes. */
function toolsForContext(ctx: ApiAuthContext): McpTool[] {
  return SABCRM_TOOLS.filter((t) => requireScope(t.scope, ctx));
}
...
function handleInitialize(params: unknown) {
  ...
  return {
    ...
    serverInfo: SABCRM_MCP_SERVER_INFO,
...
async function handleToolsCall(ctx: ApiAuthContext, params: unknown) {
  ...
  const tool = SABCRM_TOOL_MAP.get(p.name);
 ...
 * notifications.
 */
export async function dispatchSabcrmMcp(
  msg: JsonRpcRequest,
  ctx: ApiAuthContext,
