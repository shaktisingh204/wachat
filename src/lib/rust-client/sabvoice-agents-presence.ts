...
/**
 * SabVoice agent presence client — wraps `/v1/sabvoice/agents-presence`.
 */
...
export const sabvoiceAgentsPresenceApi = {
  list: (params?: AgentPresenceListParams) =>
    rustFetch<{ items: AgentPresenceDoc[] }>(
      `/v1/sabvoice/agents-presence${buildListQuery(params)}`,
    ),
  getByAgentId: (agentId: string) =>
    rustFetch<AgentPresenceDoc>(
      `/v1/sabvoice/agents-presence/${encodeURIComponent(agentId)}`,
    ),
  upsert: (input: AgentPresenceUpsertInput) =>
    rustFetch<{ entity: AgentPresenceDoc }>('/v1/sabvoice/agents-presence', {
      method: 'POST',
