...
/**
 * SabVoice Calls (CDR) client — wraps `/v1/sabvoice/calls`.
 */
...
export const sabvoiceCallsApi = {
  list: (params?: VoiceCallListParams) =>
    rustFetch<VoiceCallListResponse>(`/v1/sabvoice/calls${buildListQuery(params)}`),
  getById: (id: string) =>
    rustFetch<VoiceCallDoc>(`/v1/sabvoice/calls/${encodeURIComponent(id)}`),
  create: (input: VoiceCallCreateInput) =>
    rustFetch<{ id: string; entity: VoiceCallDoc }>('/v1/sabvoice/calls', {
      method: 'POST',
  ...
  delete: (id: string) =>
    rustFetch<{ deleted: boolean }>(`/v1/sabvoice/calls/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    }),
};
