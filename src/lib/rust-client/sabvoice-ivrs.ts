...
/**
 * SabVoice IVRs client — wraps `/v1/sabvoice/ivrs`.
 */
...
export const sabvoiceIvrsApi = {
  list: (params?: VoiceIvrListParams) =>
    rustFetch<VoiceIvrListResponse>(`/v1/sabvoice/ivrs${buildListQuery(params)}`),
  getById: (id: string) =>
    rustFetch<VoiceIvrDoc>(`/v1/sabvoice/ivrs/${encodeURIComponent(id)}`),
  create: (input: VoiceIvrCreateInput) =>
    rustFetch<{ id: string; entity: VoiceIvrDoc }>('/v1/sabvoice/ivrs', {
      method: 'POST',
  ...
  delete: (id: string) =>
    rustFetch<{ deleted: boolean }>(`/v1/sabvoice/ivrs/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    }),
};
