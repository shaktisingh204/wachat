...
/**
 * SabVoice DIDs client — wraps `/v1/sabvoice/dids`.
 */
...
export const sabvoiceDidsApi = {
  list: (params?: VoiceDidListParams) =>
    rustFetch<VoiceDidListResponse>(`/v1/sabvoice/dids${buildListQuery(params)}`),
  getById: (id: string) =>
    rustFetch<VoiceDidDoc>(`/v1/sabvoice/dids/${encodeURIComponent(id)}`),
  create: (input: VoiceDidCreateInput) =>
    rustFetch<{ id: string; entity: VoiceDidDoc }>('/v1/sabvoice/dids', {
      method: 'POST',
  ...
  delete: (id: string) =>
    rustFetch<{ deleted: boolean }>(`/v1/sabvoice/dids/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    }),
  search: (params: DidSearchParams) => {
    ...
    return rustFetch<{ items: AvailableNumber[] }>(
      `/v1/sabvoice/dids/search?${qs.toString()}`,
    );
  },
};
