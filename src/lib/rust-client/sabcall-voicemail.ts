...
/**
 * SabCall Voicemail client — wraps `/v1/sabcall/voicemail`.
 */
...
export const sabcallVoicemailApi = {
  list: (params?: VoicemailListParams) =>
    rustFetch<VoicemailListResponse>(`/v1/sabcall/voicemail${buildListQuery(params)}`),
  getById: (id: string) =>
    rustFetch<VoicemailDoc>(`/v1/sabcall/voicemail/${encodeURIComponent(id)}`),
  create: (input: VoicemailCreateInput) =>
    rustFetch<{ id: string; entity: VoicemailDoc }>('/v1/sabcall/voicemail', {
      method: 'POST',
  ...
  delete: (id: string) =>
    rustFetch<{ deleted: boolean }>(
      `/v1/sabcall/voicemail/${encodeURIComponent(id)}`,
      { method: 'DELETE' },
    ),
  listen: (id: string, listenerId: string) =>
    rustFetch<VoicemailDoc>(
      `/v1/sabcall/voicemail/${encodeURIComponent(id)}/listen`,
