    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-1">
                {pages.map(page => (
                    <div key={page._id.toString()} className={cn("flex items-center group rounded-md transaction-all duration-200", activePageId === page._id.toString() ? 'bg-[var(--st-text)]/10 text-[var(--st-text)]' : 'hover:bg-[var(--st-bg-muted)]/50')}>
                        <Button variant="ghost" className="flex-1 justify-start font-normal" onClick={() => onSelectPage(page._id.toString())}>
                            {page.isHomepage && <Home className="mr-2 h-4 w-4 text-[var(--st-text)]" />}
