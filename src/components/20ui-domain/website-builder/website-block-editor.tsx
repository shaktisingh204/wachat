...
import { Card, CardBody, CardDescription, CardFooter, CardHeader, CardTitle, Button, Select } from '@/components/sabcrm/20ui';
import {
  WebsiteBlock,
  EcommProduct,
...
class BlockEditorErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean }> {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <div className="p-4 rounded-md bg-[var(--st-text)]/10 text-[var(--st-text)] text-sm">
...
export function WebsiteBlockEditor({ selectedBlock, availableProducts, onUpdate, onRemove }: PropertiesPanelProps) {
    if (!selectedBlock) {
        return (
            <div className="text-center text-[var(--st-text-secondary)] p-8 h-full flex flex-col items-center justify-center animate-in fade-in duration-500">
                <div className="h-20 w-20 bg-[var(--st-bg-muted)]/50 rounded-full flex items-center justify-center mb-4">
                    <Trash2 className="h-8 w-8 text-[var(--st-text-secondary)]/30" />
                </div>
                <h3 className="font-medium text-lg text-[var(--st-text)]">No Block Selected</h3>
    ...
    const renderEditor = () => {
        ...
        switch (selectedBlock.type) {
            ...
            default: return <p className="text-sm text-[var(--st-text-secondary)]">Editor not available for this block type.</p>;
        }
    };
