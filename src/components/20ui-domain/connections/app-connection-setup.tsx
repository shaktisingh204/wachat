...
export function AppConnectionSetup({ app, onConnectionSaved, flowId }: { app: any, onConnectionSaved: () => void, flowId?: string }) {
    ...
    if (app.connectionType === 'webhook') {
        ...
        return <p className="text-sm text-[var(--st-text-secondary)]">Webhook setup instructions for this app are not yet configured.</p>;
    }
    ...
    if (app.connectionType === 'apikey') {
        return (
            <form action={formAction} ref={formRef} className="space-y-4">
                ...
                <p className="text-sm text-[var(--st-text-secondary)]">{app.description}</p>
    ...
    return (
        <p className="text-sm text-[var(--st-text-secondary)]">This app does not require any special connection setup.</p>
    );
}
