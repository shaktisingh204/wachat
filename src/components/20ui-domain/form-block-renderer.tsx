...
import {
  ...
  RadioGroupItem,
  EmptyState,
  Alert } from '@/components/sabcrm/20ui';
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <CardBody className="grid grid-cols-12" style={{gap: `${settings.fieldSpacing || 24}px`}}>
                    {(settings.fields || []).map((field: FormField) => {
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: controllerField }) => {
                                            ...
                                            switch(field.type) {
                                                ...
                                                case 'select': return <Select onValueChange={controllerField.onChange} defaultValue={controllerField.value}><SelectTrigger aria-label={field.label} className={cn('form-field', sizeClasses)}><SelectValue placeholder={field.placeholder || "Select..."} /></SelectTrigger><SelectContent>{fieldOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent></Select>;
                                                case 'checkbox': return <Checkbox id={fieldName} checked={controllerField.value} onChange={(e) => controllerField.onChange(e.target.checked)} label={field.label} />;
                                                case 'acceptance': return <Checkbox id={fieldName} checked={controllerField.value} onChange={(e) => controllerField.onChange(e.target.checked)} label={field.defaultValue || 'I agree to the terms.'} />;
                                                case 'radio': return <RadioGroup onValueChange={controllerField.onChange} defaultValue={controllerField.value} aria-label={field.label} className="pt-2">{fieldOptions.map(opt => <RadioGroupItem key={opt} value={opt} id={`${fieldName}-${opt}`} label={opt} />)}</RadioGroup>;
                                                case 'file': return <SabFileUrlInput value={controllerField.value || ''} onChange={(value) => controllerField.onChange(value)} accept="all" placeholder={field.placeholder || 'Choose a file'} pickerTitle={field.label} />;
                     ...
                     {submissionStatus === 'error' && <div className="col-span-12"><Alert tone="danger" title="Submission failed">{errorMessage}</Alert></div>}
                </CardBody>
                <CardFooter style={{justifyContent: settings.buttonAlign || 'flex-start'}}>
                    <Button id={settings.buttonId} type="submit" size={settings.buttonSize} variant="primary" className="submit-button" disabled={isPending}>
                        {isPending && <LoaderCircle className="mr-2 h-4 w-4 animate-spin" aria-hidden="true"/>}
                        {SubmitIcon && settings.buttonIconPosition === 'left' && <SubmitIcon aria-hidden="true" style={{marginRight: `${settings.buttonIconSpacing || 8}px`, width: settings.buttonIconSize || 16, height: settings.buttonIconSize || 16}}/>}
                        {settings.submitButtonText || 'Submit'}
                        {SubmitIcon && settings.buttonIconPosition === 'right' && <SubmitIcon aria-hidden="true" style={{marginLeft: `${settings.buttonIconSpacing || 8}px`, width: settings.buttonIconSize || 16, height: settings.buttonIconSize || 16}}/>}
                    </Button>
                </CardFooter>
            </form>
        </Card>
