  ...
  premium: [
    {
      ...
      action: "upgrade",
      iconClass: "text-[var(--st-text)]",
      badge: { text: "20% off", className: "bg-[var(--st-text)] text-white text-[11px]" },
    },
...
export const UserDropdown = ({
  ...
  const renderMenuItem = (item: MenuItem, index: number) => (
    ...
    >
      <span className="flex items-center gap-1.5 font-medium">
        <Icon
          icon={item.icon}
          className={`size-5 ${item.iconClass || "text-[var(--st-text-secondary)]"}`}
        />
        {item.label}
      </span>
      {item.badge && (
      ...
      {item.rightIcon && (
        <Icon icon={item.rightIcon} className="size-4 text-[var(--st-text-secondary)]" />
      )}
      {item.showAvatar && (
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      online: "text-[var(--st-text)] bg-[var(--st-bg-muted)] dark:text-[var(--st-text-secondary)] dark:bg-[var(--st-text)]/30",
      offline: "text-[var(--st-text-secondary)] bg-[var(--st-bg-muted)]",
      busy: "text-[var(--st-text)] bg-[var(--st-bg-muted)] dark:text-[var(--st-text-secondary)] dark:bg-[var(--st-text)]/30",
    };
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="no-scrollbar w-[310px] rounded-2xl bg-[var(--st-surface)] p-0 shadow-md"
        align="end"
      >
        <section className="bg-[var(--st-surface)] backdrop-blur-lg rounded-2xl p-1 shadow-sm">
          <div className="flex items-center p-2">
            <div className="flex-1 flex items-center gap-2">
              ...
              <div>
                <h3 className="font-semibold text-sm text-[var(--st-text)]">{user.name}</h3>
                <p className="text-[var(--st-text-secondary)] text-xs">{user.username}</p>
              </div>
            </div>
            <Badge
          ...
          <DropdownMenuGroup>
            <DropdownMenuSub>
              <DropdownMenuSubTrigger className="cursor-pointer p-2 rounded-lg">
                <span className="flex items-center gap-1.5 font-medium text-[var(--st-text-secondary)]">
                  <Icon
                    icon="solar:smile-circle-line-duotone"
                    className="size-5 text-[var(--st-text-secondary)]"
                  />
                  Update status
                </span>
              </DropdownMenuSubTrigger>
              <DropdownMenuPortal>
                <DropdownMenuSubContent className="bg-[var(--st-surface)] backdrop-blur-lg">
                  <DropdownMenuRadioGroup value={selectedStatus} onValueChange={onStatusChange}>
                    {MENU_ITEMS.status.map((status, index) => (
                      <DropdownMenuRadioItem className="gap-2" key={index} value={status.value}>
                        <Icon icon={status.icon} className="size-5 text-[var(--st-text-secondary)]" />
                        {status.label}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
