...
export function Accordion03() {
  return (
    <div className="w-full max-w-3xl shadow-sm rounded-xl bg-[var(--st-surface)]">
      <Accordion type="single" defaultValue="02" collapsible className="w-full">
        {items.map((item) => (
          <AccordionItem className="relative" value={item.id} key={item.id}>
            ...
            <AccordionContent className="space-y-6 text-[var(--st-text-secondary)] w-full h-full md:h-full grid md:grid-cols-2">
              <div className="px-6 space-y-6">
                <p> {item.content}</p>
                <Button>View More</Button>
              </div>
              <img
                className="h-full md:absolute md:w-1/2 object-cover right-0 top-0"
                src={item.img}
                alt={item.title}
                aria-hidden="true"
              />
