...
export function Crm360Timeline({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-4 border border-zoru-line bg-zoru-surface">
        <div className="flex gap-2 border-b border-zoru-line pb-3 mb-4">
          <Button
            size="sm"
        ...
        {activeTab === 'whatsapp' && onSendWhatsApp && (
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              />
              <select
                className="flex h-8.5 w-full rounded-md border border-zoru-line bg-zoru-bg px-2 text-[12.5px] text-zoru-ink focus-visible:outline-none"
                value={waTemplate}
      ...
      <div className="relative border-l border-zoru-line pl-6 ml-4 space-y-6">
        {items.length > 0 ? (
          items.map((item) => (
            <div key={item.id} className="relative group">
              
              {/* Timeline Connector Icon */}
              <span className="absolute -left-[35px] top-1.5 flex h-6.5 w-6.5 items-center justify-center rounded-full border border-zoru-line bg-zoru-surface shadow-[var(--zoru-shadow-sm)]">
                {iconMap[item.type]}
              </span>

              {/* Card Container */}
              <Card className="p-4 border border-zoru-line bg-zoru-surface hover:shadow-[var(--zoru-shadow-sm)] transition-shadow">
                ...
                <div className="flex items-center justify-between border-b border-zoru-line pb-2 mb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12.5px] font-semibold text-zoru-ink">{item.actorName}</span>
                    <Badge variant={badgeVariants[item.type] as any} className="text-[9px] uppercase px-1 py-0 h-4">
                      {item.type}
                    </Badge>
                    {item.status && (
                      <Badge variant="ghost" className="text-[9.5px] bg-zoru-surface-2 h-4 text-zoru-ink-muted">
                        {item.status}
                      </Badge>
                    )}
                  </div>
                  <div className="flex items-center gap-3 text-zoru-ink-subtle text-[11px] font-mono">
                    ...
                    {onDeleteItem && (
                      <Button
                        ...
                        className="h-6 w-6 p-0 text-zoru-danger-ink hover:bg-zoru-danger/10 opacity-0 group-hover:opacity-100 transition-opacity"
                ...
                <div className="space-y-2">
                  <div className="text-[13px] font-semibold text-zoru-ink">{item.title}</div>
                  {item.body && (
                    <p className="text-[12.5px] text-zoru-ink-muted leading-relaxed whitespace-pre-wrap">
                      {item.body}
                    </p>
                  )}
                  ...
                  {item.diff && Object.keys(item.diff).length > 0 && (
                    <div className="mt-3 overflow-hidden rounded-md border border-zoru-line bg-zoru-surface-2/10">
                      <table className="min-w-full text-[11px] leading-tight">
                        <thead className="bg-zoru-surface-2/30 border-b border-zoru-line">
                          <tr>
                            <th className="px-2.5 py-1 text-left font-semibold text-zoru-ink-muted">Changed Field</th>
                            <th className="px-2.5 py-1 text-left font-semibold text-zoru-ink-muted">Prior Value</th>
                            <th className="px-2.5 py-1 text-left font-semibold text-zoru-ink-muted">Updated Value</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(item.diff).map(([key, change]) => (
                            <tr key={key} className="border-b border-zoru-line last:border-0 hover:bg-zoru-surface-2/20">
                              <td className="px-2.5 py-1.5 font-medium text-zoru-ink">{key}</td>
                              <td className="px-2.5 py-1.5 text-zoru-ink-subtle italic">{String(change.before ?? '—')}</td>
                              <td className="px-2.5 py-1.5 text-zoru-ink font-semibold">{String(change.after ?? '—')}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          <div className="text-center py-8 text-zoru-ink-subtle text-[13px] flex flex-col items-center gap-1">
