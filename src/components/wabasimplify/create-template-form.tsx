...
export function CreateTemplateForm({ project, bulkProjectIds = [], initialTemplate, isCloning, isAdminForm = false, isBulkForm = false }: CreateTemplateFormProps) {
  ...
  const formAction = (formData: FormData) => {
    ...
    if (templateType === 'CATALOG_MESSAGE') {
      rawData.templateType = 'CATALOG_MESSAGE';
      rawData.catalogId = formData.get('catalogId') as string;
      rawData.carouselHeader = formData.get('carouselHeader') as string;
      rawData.carouselBody = formData.get('carouselBody') as string;
      rawData.carouselFooter = formData.get('carouselFooter') as string;
      rawData.section1Title = formData.get('section1Title') as string;
      rawData.section1ProductIDs = formData.get('section1ProductIDs') as string;
      rawData.section2Title = formData.get('section2Title') as string;
      rawData.section2ProductIDs = formData.get('section2ProductIDs') as string;
    ...
    if (!validationResult.success) {
      // Construct error message from Zod issues
      const errorMessage = validationResult.error.errors.map(e => e.message).join('\n');
      setState({ ...createTemplateInitialState, error: errorMessage });
      return;
    }
        ...
        <div className="lg:col-span-3 space-y-6">
          <Card>
            ...
            <CardContent className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="templateName">Template Name</Label>
                <Input id="templateName" name="name" placeholder="e.g., weekly_promo_carousel" value={templateName} onChange={(e) => setTemplateName(e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="language">Language</Label>
