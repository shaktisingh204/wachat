  ...
  return (
    <defs>
      <marker id="n8n-arrow" {...commonMarkerProps}>
        <path d={arrowPath} fill="var(--st-border)" />
      </marker>
      <marker id="n8n-orange-arrow" {...commonMarkerProps}>
        <path d={arrowPath} fill="var(--st-accent)" />
      </marker>
    </defs>
  );
}
...
export function WorkflowEdges({
  ...
  return (
    <svg className="absolute left-0 top-0 z-0 h-full w-full overflow-visible pointer-events-none">
      <ArrowDefs />
