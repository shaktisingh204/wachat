...
import { SidebarProvider } from '@/components/sabcrm/20ui/compat';
...
export function AdminLayout({ children }: AdminLayoutProps) {
    ...
    return (
        <SidebarProvider defaultOpen={false}>
            <div data-app={activeApp} className={cn("admin-dashboard flex h-screen w-full flex-col relative overflow-hidden bg-[var(--app-bg)]", appRailPosition === 'top' ? 'app-rail-top' : 'app-rail-left')}>
                {/* Ambient theme blobs — color derived from per-app CSS vars */}
                <div className="pointer-events-none absolute inset-0 -z-0 overflow-hidden">
                    <div className="absolute -top-32 -left-32 h-72 w-72 rounded-full blur-[90px] transition-colors duration-700 bg-[var(--app-blob1)]" />
                    <div className="absolute top-1/2 -right-24 h-80 w-80 rounded-full blur-[100px] transition-colors duration-700 bg-[var(--app-blob2)]" />
                    <div className="absolute bottom-0 left-1/3 h-60 w-60 rounded-full blur-[70px] transition-colors duration-700 bg-[var(--app-blob3)]" />
                    <div className="absolute top-1/4 left-1/2 h-48 w-48 rounded-full blur-[80px] transition-colors duration-700 bg-[var(--app-blob4)]" />
                </div>
                ...
                <div className="relative z-10 flex flex-1 overflow-hidden">
                    ...
                    {showSidebar && (
                        <div
                            className="hidden md:flex my-2 mr-0 ml-1 rounded-2xl h-[calc(100%-1rem)] shadow-xl overflow-hidden transition-all duration-500 animate-fade-in shrink-0 bg-[hsl(var(--card)/0.85)] backdrop-blur-[24px] border border-[var(--app-border)]"
                        >
                    ...
                    <main className="flex-1 overflow-hidden relative transition-all duration-300 p-2 pl-1">
                        {isChatPage || isBuilderPage ? (
                            <div
                                className="h-full overflow-hidden rounded-2xl shadow-xl bg-[hsl(var(--card)/0.85)] backdrop-blur-[24px] border border-[var(--app-border)]"
                            >
                                <RouteTransition>{children}</RouteTransition>
                            </div>
                        ) : (
                            <div
                                className="h-full overflow-y-auto rounded-2xl shadow-xl px-5 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8 bg-[hsl(var(--card)/0.85)] backdrop-blur-[24px] border border-[var(--app-border)]"
                            >
