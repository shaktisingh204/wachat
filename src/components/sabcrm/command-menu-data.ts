...
/**
 * Command-menu data layer — extracted VERBATIM from the retired
 * `twenty/twenty-command-menu.tsx` so the 20ui `CommandMenu` component is
 * presentation only. Owns:
 *
 *  - the recently-viewed records localStorage helper (`recordRecents`),
 *  - the theme toggle bridge to the `sabcrm.prefs.v1` prefs blob,
 *  - the object catalogue (live `listSabcrmObjectsTw` with a static fallback),
 *  - favorites loading,
 *  - the debounced fan-out record search across the catalogue,
 *  - record label / avatar derivation,
 *  - the static global-nav / settings command tables + the shortcuts table.
 *
 * Nothing in here renders — keep it that way. The presentation shell lives in
 * `./command-menu.tsx` (20ui `CommandDialog`).
 */
...
import type { AvatarShape } from '@/components/sabcrm/20ui/avatar';
...
export function readRecents(): RecordRecent[] {
...
export function pushRecent(entry: RecordRecent): void {
  if (typeof window === 'undefined') return;
  try {
...
/* =========================================================================
   ...
   ========================================================================= */
export const STANDARD_OBJECT_ICON: Record<string, LucideIcon> = {
  companies: Building2,
  people: Users,
  leads: Briefcase,
  notes: StickyNote,
 ...
 */
const STANDARD_OBJECT_AVATAR_SHAPE: Record<string, AvatarShape> = {
  people: 'round',
  companies: 'square',
  leads: 'square',
  notes: 'square',
...
export function avatarShapeForSlug(slug: string): AvatarShape {
...
export const FALLBACK_OBJECTS: readonly ObjectCatalogueEntry[] = [
...
export function singularLabelForSlug(
  slug: string,
...
export function deriveRecordAvatarUrl(record: SabcrmRustRecord): string | undefined {
  const data = record.data ?? {};
...
export const GLOBAL_NAV_COMMANDS: readonly NavCommand[] = [
  {
    id: 'nav-dashboard',
...
export const SETTINGS_COMMANDS: readonly NavCommand[] = [
  {
    id: 'set-root',
...
/* =========================================================================
   Hooks — the loading effects, moved verbatim out of the old component
   ========================================================================= */
 ...
 */
export function useObjectCatalogue(
  open: boolean,
  projectId?: string,
): readonly ObjectCatalogueEntry[] {
