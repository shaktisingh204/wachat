...
/**
 * 20ui — StPortalPopover: a portal-rendered, anchor-positioned popover.
 *
 * Moved here from `@/components/sabcrm/twenty/st-portal-popover` (that file is
 * now a thin re-export of this one) so the 20ui design system has no imports
 * from the legacy twenty/ kit. The exported names + props are unchanged.
 *
 * Twenty-style dropdowns (the workspace switcher, the notifications bell, the
 * tag pickers, `StSelect`, and the 20ui combobox/select/menu/datepicker/
 * daterange/tooltip) all live inside containers that clip their children —
 * most notably `.st-sidebar { overflow: hidden }`. Rendering a popover as a
 * positioned child of those containers gets it clipped at the container edge
 * regardless of `z-index`. This helper sidesteps that entirely: it renders
 * into `document.body` (so no ancestor `overflow`/`transform` can clip it) and
 * positions itself with `position: fixed` against the live bounding rect of an
 * anchor element, re-measuring on scroll + resize.
 *
...
// The `.st-popover` chrome lives in the 20ui token foundation
// (`:is(.\32 0ui, .ui20).st-popover` in tokens.css — compound, since the
// portal root carries both classes on one element); imported here so the
// popover styles itself even when consumed via a deep import.
import './tokens.css';
...
export function StPortalPopover({
  ...
  return createPortal(
    <div
      ...
      // resolve wherever it mounts: `ui20` for the app-wide 20ui system and
      // `sabcrm-twenty` for the legacy CRM surface (a body-mounted portal is
      // not a descendant of either app frame). Whichever stylesheet is loaded
      // supplies the tokens; the other class is inert. KEEP `sabcrm-twenty`
      // here until the twenty/ kit is fully retired — the twenty CSS still
      // layers `.sabcrm-twenty .st-popover` chrome on top for old pages.
