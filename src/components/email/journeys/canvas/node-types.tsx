...
export function JourneyNodeCard({ node, selected, onSelect, index }: JourneyNodeCardProps) {
  ...
  return (
    <Button
      ...
      onClick={onSelect}
      className={cn(
        'group relative !block w-full max-w-md !h-auto !rounded-xl border !bg-[var(--st-bg-secondary)] !px-4 !py-3 text-left shadow-sm transition',
        'hover:border-[var(--st-text-secondary)]/40',
    ...
    >
      ...
      <span className="flex items-start gap-3">
        <span
          className={cn(
            'mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--st-bg-muted)]',
            meta.accent,
          )}
        >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-2">
            ...
            {node.data.label ? (
              <span className="truncate text-sm font-medium text-[var(--st-text)]">{node.data.label}</span>
            ) : null}
          </span>
          <span className="mt-1 block text-xs text-[var(--st-text-secondary)] line-clamp-2">{summary}</span>
        </span>
      </span>
    </Button>
  );
