  ...
  return (
    <div className="flex items-center gap-1 rounded-2xl rounded-tl-sm px-4 py-3 w-fit shadow-sm bg-[var(--st-bg-secondary)]">
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          className="h-1.5 w-1.5 rounded-full bg-[var(--st-text)] animate-bounce"
...
function HostBubble({ msg }: { msg: HostMsg }) {
  if (msg.kind === 'text') {
    return (
      <div className="max-w-[80%] rounded-2xl rounded-tl-sm px-4 py-2.5 text-[13px] leading-relaxed shadow-sm bg-[var(--st-bg-secondary)] text-[var(--st-text)] animate-in fade-in-0 slide-in-from-left-2 duration-200">
        {msg.text}
      </div>
    );
  }
