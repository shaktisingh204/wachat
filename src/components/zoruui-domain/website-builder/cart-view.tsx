...
import { Button, Card, CardBody, CardFooter, CardHeader, CardTitle, Separator } from '@/components/sabcrm/20ui';
import {
    ...
    if (itemCount === 0) {
        return (
            <div className="text-center py-20">
                <ShoppingCart className="mx-auto h-16 w-16 text-[var(--st-text-secondary)]" />
                <h2 className="mt-4 text-2xl font-semibold">Your cart is empty</h2>
                <p className="mt-2 text-[var(--st-text-secondary)]">Looks like you haven't added anything to your cart yet.</p>
            ...
            <div className="md:col-span-2 space-y-4">
                {cart.map(item => (
                     <Card key={item.productId} className="flex items-center p-4 gap-4">
                         <div className="relative h-24 w-24 bg-[var(--st-bg-muted)] rounded-md overflow-hidden">
                         ...
                         <div className="flex-1">
                            ...
                            <p className="text-[var(--st-text-secondary)]">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(item.price)}</p>
                         </div>
