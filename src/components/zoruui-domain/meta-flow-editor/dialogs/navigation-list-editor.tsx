...
import { Input, Button, Label, ScrollArea, Separator, Badge } from '@/components/sabcrm/20ui';
            ...
            <div className="space-y-4">
                ...
                <ScrollArea className="max-h-[40vh] space-y-3 pr-4">
                    {listItems.map((item: any, index: number) => (
                        <div key={item.id || index} className="p-4 border rounded-lg space-y-3 bg-[var(--st-bg-muted)]/50 mb-3 relative">
                             <Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1 h-7 w-7" onClick={() => handleRemoveItem(index)}><Trash2 className="h-4 w-4 text-[var(--st-text)]"/></Button>
                            ...
                            <div className="p-3 border rounded-md bg-[var(--st-bg-secondary)]">
