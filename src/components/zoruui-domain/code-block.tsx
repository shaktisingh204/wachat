...
export function CodeBlock({ code, language, className, wrap }: CodeBlockProps) {
  ...
  return (
    <div className={cn("relative rounded-lg bg-[var(--st-bg-muted)]/50 p-4 font-mono text-sm", className)}>
      <Button
        size="icon"
        variant="ghost"
        ...
        onClick={onCopy}
      >
        {hasCopied ? <Check className="h-4 w-4 text-[var(--st-text)]" /> : <Copy className="h-4 w-4" />}
