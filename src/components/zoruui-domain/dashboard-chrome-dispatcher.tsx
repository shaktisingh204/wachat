...
import { DashboardClientLayout } from '@/components/zoruui-domain/dashboard-client-layout';
import {
  ClayDashboardLayout,
  type ClayLayoutUser,
  type ClayLayoutPlan,
...
export function DashboardChromeDispatcher({
  ...
  // ── Settings branch: Clay chrome for account, billing & workspace config.
  if (onSettings) {
    if (!wachatData) {
      return <ClayBootSkeleton />;
    }
    return (
      ...
      >
        <AdManagerProvider>
          <ClayDashboardLayout context="settings" user={user} plan={plan}>
            {children}
          </ClayDashboardLayout>
        </AdManagerProvider>
      </ProjectProvider>
