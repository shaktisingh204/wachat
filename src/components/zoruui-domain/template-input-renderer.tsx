...
function SmartVariableInput({ id, name, placeholder, required, variableOptions = [], value, onChange }: SmartVariableInputProps) {
    ...
    return (
        <div className="relative">
            ...
            {variableOptions.length > 0 && <p className="text-[10px] text-[var(--st-text-secondary)] mt-1">Type manually or select a variable.</p>}
        </div>
    );
}
        ...
        <div className="space-y-6">
                                ...
                                <div key={`header-${idx}`} className="space-y-2">
                                    ...
                                    {vars.map(v => (
                                        <div key={`header-var-${v}`} className="space-y-1">
                                            <Label htmlFor={`variable_header_${v}`} className="text-xs text-[var(--st-text-secondary)]">Variable {'{{'}{v}{'}}'}</Label>
                                            <SmartVariableInput
                                ...
                                {headerMediaSource === 'file' ? (
                                    <div className="space-y-1">
                                        ...
                                        <Input
                                            ...
                                            required
                                            className="file:text-[var(--st-text)] file:font-medium"
                                        />
                        ...
                        <div className="grid gap-3">
                            {vars.map(v => (
                                <div key={`body-var-${v}`} className="space-y-1">
                                    <Label htmlFor={`variable_body_${v}`} className="text-xs text-[var(--st-text-secondary)]">Variable {'{{'}{v}{'}}'}</Label>
                                    <SmartVariableInput
                                        id={`variable_body_${v}`}
                            ...
                            {interactiveButtons.map((btn: any) => (
                                <div key={`btn-${btn.index}`} className="space-y-1">
                                    ...
                                    <SmartVariableInput
                                        ...
                                        variableOptions={variableOptions}
                                    />
                                    {btn.type === 'URL' && <p className="text-[10px] text-[var(--st-text-secondary)]">Appended to: {btn.url.split('{{1}}')[0]}</p>}
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {template.type === 'MARKETING_CAROUSEL' && (
                <div className="space-y-4 border-t pt-4">
                    ...
                    <p className="text-xs text-[var(--st-text-secondary)]">Upload media for each card.</p>
                    ...
                    <div className="grid gap-4">
                        {template.components?.find(c => c.type === 'CAROUSEL')?.cards?.map((card: any, index: number) => {
                            ...
                            return (
                                <div key={index} className="p-4 border rounded-md bg-[var(--st-bg-muted)]/20 space-y-3">
