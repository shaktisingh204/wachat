...
const NodePreview = ({ node }: { node: FlowNode }) => {
    const renderTextWithVariables = (text?: string) => {
        ...
        return parts.map((part, i) =>
            part.match(/^{{.*}}$/) ? (
                <span key={i} className="font-semibold text-[var(--st-text)]/90 bg-[var(--st-text)]/10 rounded-sm px-1">
                    {part}
                </span>
            ) : (
                part
            ...
            case 'image':
                return (
                     <div className="space-y-1">
                        <div className="aspect-video bg-[var(--st-bg-secondary)]/50 rounded-md flex items-center justify-center">
                            <ImageIcon className="h-8 w-8 text-[var(--st-text)]/20" />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="space-y-1 mt-2 border-t border-muted-foreground/20 pt-2">
                            {(node.data.buttons || []).map((btn: any, index: number) => (
                                <div key={btn.id || index} className="text-center text-[var(--st-text)] font-medium bg-[var(--st-bg-secondary)]/50 py-1.5 rounded-md text-xs">
    ...
    return (
        <ZoruCardContent className="p-2 pt-0">
            <div className="bg-[var(--st-bg-muted)] p-2 rounded-lg text-sm text-[var(--st-text)]/80">
                {content}
            </div>
        </ZoruCardContent>
    );
...
const NodeComponent = ({ 
    ...
    const Handle = ({ position, id, style }: { position: 'left' | 'right' | 'top' | 'bottom', id: string, style?: React.CSSProperties }) => (
        <div 
            ...
            style={style}
            className={cn(
                "absolute w-4 h-4 rounded-full bg-[var(--st-bg-secondary)] border-2 border-primary hover:bg-[var(--st-text)] transition-colors z-10",
    ...
    return (
        ...
        >
            <Card className={cn("w-64 hover:shadow-xl hover:-translate-y-1 bg-[var(--st-bg-secondary)]", isSelected && "ring-2 ring-primary shadow-2xl")}>
                ...
                <NodePreview node={node} />
                 {node.type === 'condition' && ( <ZoruCardContent className="p-3 pt-0 text-xs text-[var(--st-text-secondary)]"><div className="flex justify-between items-center"><span>Yes</span></div><Separator className="my-1"/><div className="flex justify-between items-center"><span>No</span></div></ZoruCardContent>)}
            </Card>
...
export function FlowBuilder() {
    ...
    return (
        <div className="flex h-[calc(100vh-theme(spacing.20))] bg-[var(--st-bg-muted)]/30">
             <div className={cn("flex-col gap-4 p-2 bg-[var(--st-bg-secondary)] border-r transition-all duration-300", isSidebarCollapsed ? 'hidden' : 'hidden md:flex md:w-64')}>
                 ...
                 <main className="flex-1 grid grid-cols-12 overflow-hidden">
                    ...
                    >
                        ...
                        <div className="relative w-full h-full" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: 'top left' }}>
                            {isLoading && !currentFlow ? (<div className="absolute inset-0 flex items-center justify-center"><LoaderCircle className="h-8 w-8 animate-spin text-[var(--st-text)]" /></div>) : (
                                <>
