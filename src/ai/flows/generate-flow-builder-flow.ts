...
const FlowNodeDataSchema = z.object({
    ...
    buttons: z.array(z.object({ id: z.string(), type: z.enum(['QUICK_REPLY']).describe('The type of the button, always "QUICK_REPLY".'), text: z.string() })).optional().describe('An array of buttons for `buttons` nodes.'),
    ...
    operator: z.enum(['equals', 'not_equals', 'contains', 'is_one_of', 'is_not_one_of', 'greater_than', 'less_than']).optional().describe('For `condition` nodes, the comparison operator (e.g., "equals", "contains").'),
