...
- **2026-06-14 — Batch 7 (V3.7 Event Streams + Sinks, COMPLETE) DONE & verified.** Added `sabsms_event_sinks` + `sabsms_sink_deliveries` collections (+ `SabsmsSinkDelivery` type); `sinks/consume.ts` (`deliverEventToSinks` — envelope→match→fanOut→persist, injected IO, 4 tests); `sinks/handlers.ts` (`registerSinkEventHandlers` wildcard, wired into `events/consumer.ts` `createDefaultRouter` mirroring `registerWebhookOutEventHandlers`); CRUD UI: `settings/sinks-actions.ts` (RBAC, secret minted once for HTTP kinds) + `settings/sinks-card.tsx` (list/add/toggle/delete) wired into the settings page.
- **Test status: 52/52 passing** (`npx tsx --test` across channels/governance/verify/sinks/consume). The 2 failures seen when globbing all `__tests__` are PRE-EXISTING v2 tests (`routing-policy`, `idempotency-replay`) that import `server-only` (unresolvable under raw tsx) — unmodified by this work. API routes + settings cards + the worker hook match proven repo patterns but need the running app (live Mongo + engine + render) to verify end-to-end.
